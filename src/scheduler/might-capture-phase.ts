/**
 * Daily might-snapshot phase.
 *
 * Runs at most once per game day per clan, and only ever AFTER the chest scan
 * has been finalised — see the call site in scheduler/loop.ts. That ordering is
 * the whole safety argument: by the time this runs, the gift and triumphal
 * sweeps have committed their rows and written their scan_sessions record, so
 * nothing this phase can do (or fail to do) is able to cost chest data.
 *
 * The other half of the isolation is what this phase deliberately does NOT do:
 *
 *   - it never creates a scan_sessions row, touches the state machine, or sets
 *     lastScanError, so a might failure can't show up as a scan failure;
 *   - it never throws out of the call site (the caller swallows), so the worst
 *     case is a logged no-op and a missing day on a growth chart.
 *
 * It DOES write to `members` — that part of the original isolation is gone, and
 * the two lines that used to claim otherwise ("never an upsert, a name that
 * doesn't resolve is reported, not inserted") were left behind by the change that
 * ended it. The in-game member list IS the clan roster, so a name on it is better
 * evidence of membership than a chest, and someone who joins between the roster
 * build and their first chest would otherwise stay invisible. What keeps that
 * safe is not silence but review: a created row has first_seen = now, which puts
 * it in the New Members queue with the screenshot crop of its row, and every
 * reading is resolved against the clan's player merge rules and the roster before
 * anything is created.
 *
 * Unrelated to the ChestTracker integration — different source, different
 * tables, no shared code path.
 */
import type { Page } from 'playwright';
import type { AppConfig, ClanMember } from '../models/types.js';
import { childLogger } from '../utils/logger.js';
import {
  getMemberListCropRevision,
  isMemberListCropRecalibrated,
  isMemberListCropSet,
} from '../config/calibration.js';
import { currentGameDate } from '../utils/game-day.js';
import { exactMatchMember, fuzzyMatchMember } from '../utils/fuzzy.js';
import * as memberRepo from '../data/repositories/member-repo.js';
import * as mightRepo from '../data/repositories/might-repo.js';
...
export interface MightCapturePhaseContext {
  config: AppConfig;
  clanId: number;
  reportProgress?: (message: string) => void;
  /**
   * Ignore the once-a-day gate for this run.
   *
   * Set by the superadmin "Re-capture now" action. Without it, any fix to the
   * capture or matching code can't be exercised until the next 17:00 UTC
   * rollover, which makes iterating on a feature that can only be tested against
   * the live game needlessly slow. Safe because saveSnapshots upserts on
   * (member, game day): a forced re-run corrects today's readings rather than
   * duplicating them.
   */
  force?: boolean;
}

export interface MightCapturePhaseOutcome {
  ran: boolean;
  ...
  gameDate?: string;
  matched?: number;
  /** Names seen on the in-game list that weren't on the roster, so were created
   *  and put into the New Members review queue. */
  created?: string[];
  rowsWritten?: number;
}
...
export function resolveMember(
  name: string,
  activeMembers: ClanMember[],
  inactiveMembers: ClanMember[],
  candidatesFor: (name: string) => string[],
  canonicalise: (name: string) => string,
): { member: ClanMember; viaInactive: boolean; exact: boolean } | null {
  ...
  if (canonical !== name) {
    const member = exactMatchMember(canonical, activeMembers)
      ?? exactMatchMember(canonical, inactiveMembers);
  ...
  for (const candidate of candidates) {
    const member = exactMatchMember(candidate, activeMembers)
      ?? exactMatchMember(candidate, inactiveMembers);
    if (member) return { member, viaInactive: !member.isActive, exact: true };
  }

  // Only now fall back to distance, active members first — a contested near-miss
  // should land on someone currently in the clan.
  for (const [viaInactive, pool] of [[false, activeMembers], [true, inactiveMembers]] as const) {
    for (const candidate of candidates) {
      const member = fuzzyMatchMember(candidate, pool);
      if (member) return { member, viaInactive, exact: false };
    }
  }
  return null;
}
 ...
 */
export async function runMightCapturePhase(
  ctx: MightCapturePhaseContext,
  page: Page,
): Promise<MightCapturePhaseOutcome> {
  const { config, clanId, reportProgress, force } = ctx;

  if (!config.mightTrackingEnabled) return { ran: false, skipped: 'disabled' };
  ...
  if (!isMemberListCropRecalibrated()) {
    log.info(
      'Might capture skipped — the member-list rectangle predates might tracking ' +
      `(Stage 4 save count: ${getMemberListCropRevision()}). Re-run Admin → Scanner Mode → ` +
  ...
  if (mightRepo.hasSnapshotForGameDate(clanId, gameDate)) {
    if (!force) {
      ...
      log.info(
        `Might capture skipped for clan #${clanId} — already captured for game day ${gameDate}. ` +
    ...
    log.info(
      `Might re-capture forced for clan #${clanId} — overwriting today's readings (game day ${gameDate}).`,
    );
  }
  ...
  const {
    captureMemberMight, nameMatchCandidates, sameOcrSkeleton,
  } = await import('../browser/might-capture.js');
  ...
  const resolve = (name: string): ReturnType<typeof resolveMember> =>
    resolveMember(
      name, activeMembers, inactiveMembers, nameMatchCandidates, canonicalisePlayerName,
    );
  ...
  if (capture.navigationFailed) {
    log.warn(
      `Might capture for clan #${clanId}: could not open the Members panel — skipping today. ` +
  ...
  if (capture.coordRowsSeen > 0 && capture.rowsWithoutMight === capture.coordRowsSeen) {
    log.warn(
      `Might capture for clan #${clanId}: read ${capture.coordRowsSeen} member row(s) but found no ` +
  ...
  if (capture.rows.length === 0) {
    log.warn(
      `Might capture for clan #${clanId}: no member rows were read at all ` +
  ...
  const snapshots: mightRepo.MightSnapshotInput[] = [];
  ...
  const resolvedByRow = new Map<typeof capture.rows[number], ReturnType<typeof resolve>>();
  for (const row of capture.rows) resolvedByRow.set(row, resolve(row.name));
  ...
  for (const name of capture.unresolvedNames) {
    ...
    if (hit) seenMemberIds.add(hit.member.id);
  }
  ...
  const rowsWritten = mightRepo.saveSnapshots(
    clanId,
    gameDate,
  ...
  if (snapshots.length > 0) {
    log.debug(
      `Might recorded for clan #${clanId} (${gameDate}): `
      + snapshots.map((s) => `#${s.memberId}=${s.might.toLocaleString('en-US')}`).join(', '),
    );
  }

  log.info(
    `Might capture for clan #${clanId} (${gameDate}): ${rowsWritten} member(s) recorded from ` +
  ...
  if (collapsedPairs.length > 0) {
    log.warn(
      { noAlert: true },
      `Might capture for clan #${clanId}: ${collapsedPairs.length} reading(s) matched a member ` +
  ...
  if (promoted.length > 0) {
    ...
    log.warn(
      { noAlert: true },
      `Might capture for clan #${clanId}: ${promoted.length} reading(s) were close to an existing ` +
  ...
  if (missed.length > 0) {
    log.warn(
      { noAlert: true },
      `Might capture for clan #${clanId}: ${missed.length} active member(s) got no reading today — ` +
  ...
  if (created.length > 0) {
    ...
    log.warn(
      { noAlert: true },
      `Might capture for clan #${clanId}: created ${created.length} member(s) seen on the in-game ` +
  ...
  if (failed.length > 0) {
    ...
    log.warn(
      `Might capture for clan #${clanId}: failed to create ${failed.length} member(s) — ` +
  ...
  logSystemAction(clanId, 'member.might_capture', {
    clanId,
    gameDate,
  ...
  // later.
  if (created.length > 0) invalidateReviewQueueCount(clanId);
