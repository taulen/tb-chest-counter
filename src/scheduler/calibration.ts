...
import { navigateToGame, checkLoginStatus } from '../browser/auth.js';
import { getCanvasBounds } from '../browser/navigator.js';
import { captureForVision } from '../browser/screenshotter.js';
import { humanClick, randomDelay } from '../utils/human-delay.js';
import { keyPress } from '../browser/input.js';
import { getUiPosition, isUiPositionSet } from '../config/calibration.js';
import type { StateMachine } from './state-machine.js';

const log = childLogger('calibration');
...
/**
 * In-flight position overrides the calibration wizard sends with a
 * Capture request. The operator may have marked a sidebar in the
 * browser-side wizard state but not yet saved it via Save Stage —
 * this lets the same Capture click use that in-flight value for
 * navigation. Otherwise Stage 2 would need an intermediate save
 * between marking sidebars and marking the gifts-panel-only
 * targets (Open button, card crop, top tabs), since the saved
 * sidebar position is what the auto-nav reads.
 *
 * Each entry is canvas-relative `xPct`/`yPct` in [0, 1]. Anything
 * outside that range, or a zero pair, is ignored — the auto-nav
 * falls back to the saved config value (or skips the click if
 * neither is set).
 */
export type CalibrationNavOverrides = {
  clanButton?: { xPct: number; yPct: number };
  giftsSidebar?: { xPct: number; yPct: number };
  membersSidebar?: { xPct: number; yPct: number };
  /** Stage 1's MAP button, needed in-flight because the operator marks it in the
   *  same stage they'd then want to capture Stage 5 from. */
  worldMapButton?: { xPct: number; yPct: number };
  /** Stage 5's capital mark, needed in-flight so Stage 6 can be captured without a
   *  save in between — the same reason the sidebars are overridable. */
  clanCapitalMarker?: { xPct: number; yPct: number };
  /**
   * Stage 5's show-clan-capital mark, fed back into its OWN capture as well as
   * into Stage 6's.
   *
   * Stage 5 has the same two-visible-at-once problem Stage 6 does: the icon lives
   * in the minimap strip, but the capital can only be marked reliably AFTER
   * clicking that icon has recentred the camera on it. So the stage is captured
   * twice — first the map as-is to mark the icon, then again with the icon clicked
   * so the capital is where it will actually be at runtime.
   */
  clanCapitalButton?: { xPct: number; yPct: number };
  /**
   * Stage 6's own History mark, fed back into its own capture.
   *
   * Stage 6 has two targets that can't be visible at once: "History" lives in
   * the Clan Capital dialog's left rail (present on every sub-section) while the
   * rows to draw the rectangle around only exist once History is open. So the
   * stage is captured twice — the first capture lands on the dialog's default
   * sub-section and the operator marks History, the second one clicks that mark
   * and lands on the list. Same shape as the Stage 2 → Stage 3 hand-off, just
   * folded into one stage because the operator thinks of it as one screen.
   */
  capitalHistorySidebar?: { xPct: number; yPct: number };
};
...
export interface CalibrationCaptureResult {
  ...
  error?: string;
  /**
   * Non-fatal note shown alongside a screenshot that WAS saved.
   *
   * Distinct from `error` on purpose: when auto-nav can't confirm it landed where
   * it meant to, the screenshot is still the most useful thing an operator can be
   * given — they can see what blocked it. Refusing to save it (which is what an
   * `error` does) leaves them with nothing to diagnose from, which is exactly how
   * a promo overlay on Stage 5 presented as a mystery.
   */
  warning?: string;
...
export async function captureCalibrationScreenshot(
  ...
  stateMachine.transition(AppState.CHECKING_AUTH);
  ...
  const warmPage = stageCanReuseWarmPage && (() => {
    try {
      ...
      return current.host === new URL(TB_GAME_URL).host;
    } catch {
  ...
  if (warmPage) {
    ...
    loggedIn = await checkLoginStatus(page).catch(() => false);
    if (loggedIn) {
  ...
  stateMachine.transition(AppState.NAVIGATING);
  ...
  const isValidPct = (p: { xPct: number; yPct: number } | undefined) =>
    !!p && p.xPct > 0 && p.xPct < 1 && p.yPct > 0 && p.yPct < 1;
  const resolvePosition = (
    name: keyof CalibrationNavOverrides,
    ...
    if (isUiPositionSet(name)) return getUiPosition(name);
    return null;
  };
  ...
  if (stage === 'worldmap' || stage === 'capital') {
    ...
    const clickPct = async (
      ...
      await randomDelay(settleMinMs, settleMaxMs);
    };
    ...
    const probeWorldMap = async (): Promise<boolean | null> => {
      try {
        const { isOnWorldMap } = await import('../browser/resource-history-capture.js');
        return await isOnWorldMap(await captureForVision(page));
      } catch (err) {
    ...
    if (stage === 'worldmap') {
      ...
      if (capitalButtonPct) {
        ...
        log.info(
          "Calibration capture (stage=worldmap): the show-clan-capital icon isn't marked yet, so "
    ...
    if (stage === 'capital') {
      ...
      if (historyPct) {
        ...
        log.info(
          "Calibration capture (stage=capital): History isn't marked yet, so this capture stops "
    ...
    await humanClick(page, clanX, clanY);
