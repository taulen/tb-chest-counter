...
import { MaintenanceModeError, SessionKickedError } from '../browser/navigator.js';
...
import { UNKNOWN_PLAYER_NAME, type TabScanResult } from './scan-pipeline.js';
 ...
 */
export const MAINTENANCE_BUFFER_MS = 5 * 60_000;

export interface ScanFinalizeContext {
  clanId: number;
  isRunning: boolean;
  stateMachine: StateMachine;
  /** Latest progressMessage at the moment the catch fires — used as
   *  the errorPhase fallback for SessionKickedError. */
  progressMessage: string;
  /** Used by the success path: ScanLoop refreshes this after each
   *  scheduleNextCycle call, so reading it from the host keeps the
   *  helper free of timing decisions. */
  recordScanError: (err: unknown) => void;
  getLastScanError: () => { message: string; phase: string } | null;
  clearLastScanError: () => void;
  setMaintenanceBlock: (durationMs: number) => void;
  clearMaintenanceBlock: () => void;
  scheduleNextCycle: (delayMs: number) => void;
  flagSkipNextScheduling: () => void;
  reportProgress: (phase: 'scan', message: string) => void;
  config: { screenshotRetentionDays: number };
  onScanComplete?: (result: ScanResult, clanId: number) => void;
}
...
export async function finalizeScanSuccess(
  ctx: ScanFinalizeContext,
  result: ScanResult,
  scanSessionId: number,
  giftsResult: TabScanResult,
  triumphalWarning: string | null,
): Promise<ScanResult> {
  const unknownMsg = giftsResult.unknownNames > 0
    ? `OCR failed to read the player name for ${giftsResult.unknownNames} chest(s). ` +
      `They were saved under "${UNKNOWN_PLAYER_NAME}" and need manual review. ` +
  ...
  result.success = true;
  ctx.stateMachine.transition(AppState.IDLE);
 ...
 */
function persistedChestCount(sessionId: number, clanId: number): { gifts: number; total: number } {
  try {
...
export interface ScanFinalizeFailureOutcome {
  /** When 'rethrow', the caller MUST re-throw the original error so
   *  the outer runCycle catch can apply its own cooldown logic. When
   *  'return', the caller returns `result` as-is — failure has
   *  already been recorded and the session row updated. */
  action: 'return' | 'rethrow';
}
 ...
 */
export function finalizeScanFailure(
  err: unknown,
  ctx: ScanFinalizeContext,
  result: ScanResult,
  scanSessionId: number,
): ScanFinalizeFailureOutcome {
  ...
  if (persisted.total > result.newChests) {
    log.info(
      `Session ${scanSessionId} has ${persisted.gifts} gift + ${persisted.total - persisted.gifts} triumphal ` +
  ...
  if (err instanceof MaintenanceModeError) {
    ...
    if (durationMs && durationMs > 0) {
      ...
      if (ctx.isRunning) {
        const waitMs = durationMs + MAINTENANCE_BUFFER_MS;
  ...
  if (deleted > 0) {
    log.warn(
      `Rolled back ${deleted} chest(s) from failed session ${scanSessionId} that the persisted-row ` +
  ...
  ctx.stateMachine.transition(AppState.ERROR);
