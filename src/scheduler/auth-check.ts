...
import { closeBrowser, launchBrowser } from '../browser/launcher.js';
import { clanStorageStatePath, clanBrowserProfileDir } from '../config/clan-paths.js';
import {
  BrowserTargetCrashedError,
  LoggedOutError,
  checkLoginStatus,
  isCrashLikeError,
  navigateToGame,
 ...
 * write-free).
 */
function noteSessionHealthy(clanId: number): void {
  const { recovered } = clearClanNeedsReauth(clanId);
  if (!recovered) return;
  log.info(
    `Clan #${clanId} no longer needs re-authentication — the saved session loaded the game on this ` +
  ...
  void postReauthResolvedNotice(clanId).catch(() => {});
}
...
export interface AuthCheckContext {
  ...
  clanId: number;
  reportProgress: (phase: 'auth', message: string) => void;
}

export interface AuthCheckResult {
  ok: boolean;
  /** The (possibly relaunched) session. Caller MUST assign this back
   *  into its own session field whether ok or not — a crash recovery
   *  may have rotated the browser even when auth ultimately failed. */
  session: BrowserSession;
}
 ...
 */
export async function performAuthCheck(ctx: AuthCheckContext): Promise<AuthCheckResult> {
  let { session } = ctx;
  ...
  for (let authAttempt = 1; authAttempt <= 2; authAttempt++) {
    try {
      ...
      const crashed = err instanceof BrowserTargetCrashedError || isCrashLikeError(err) || page.isClosed();
      ...
      if (crashed) {
        ...
        log.error(
          `Clan #${ctx.clanId} scan aborted — the browser crashed twice while loading the game, ` +
      ...
      if (ctx.config.headless) {
        ...
        const { firstTime } = markClanNeedsReauth(ctx.clanId);
        const cause = err instanceof LoggedOutError
        ...
        if (firstTime) {
          ...
          void postReauthRequiredNotice(ctx.clanId).catch(() => {});
        }
        return { ok: false, session };
      }
 ...
 * crash mid-attempt.
 */
export async function relaunchBrowserSession(
  config: AppConfig,
  session: BrowserSession,
  clanId: number,
  reason: string,
