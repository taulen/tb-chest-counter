...
import { containsKnownChestName } from './chest-names.js';
 ...
 */
export function parseMaintenanceDuration(text: string): number | null {
 ...
 */
export function looksLikeStoreOverlayText(rawText: string): boolean {
  const text = rawText.toLowerCase();
  const compact = text.replace(/\s+/g, '');
  const has = (phrase: string): boolean =>
    text.includes(phrase) || compact.includes(phrase.replace(/\s+/g, ''));
 ...
 */
export function describeSessionKickText(rawText: string): string | null {
  const text = rawText.toLowerCase();
  const compact = text.replace(/\s+/g, '');
  const has = (phrase: string): boolean =>
    text.includes(phrase) || compact.includes(phrase.replace(/\s+/g, ''));
...
export function looksLikeSessionKickedText(rawText: string): boolean {
  return describeSessionKickText(rawText) !== null;
}

export interface ScreenStateClassification {
  state: ScreenState;
  /** Non-null only when state is MAINTENANCE and a duration was parseable. */
  maintenanceDurationMs: number | null;
}
 ...
 */
export function classifyScreenStateText(rawText: string): ScreenStateClassification {
  const text = rawText.toLowerCase();
  ...
  if (has('undergoing maintenance')
    ...
    return { state: ScreenState.MAINTENANCE, maintenanceDurationMs: parseMaintenanceDuration(text) };
  }
  ...
  if (describeSessionKickText(rawText) !== null) {
    return { state: ScreenState.SESSION_KICKED, maintenanceDurationMs: null };
  }

  if (has('no gifts') || has('no gift')) {
    return { state: ScreenState.NO_GIFTS, maintenanceDurationMs: null };
  }
  ...
  if (hasGifts && hasClanContext) return { state: ScreenState.GIFT_TAB, maintenanceDurationMs: null };
  if (hasGifts && (has('chest') || has('time left') || has('from'))) {
    return { state: ScreenState.GIFT_TAB, maintenanceDurationMs: null };
  }

  // Panel-content fallback: chest token + a known chest name.
  if (has('chest') && containsKnownChestName(text)) {
    return { state: ScreenState.GIFT_TAB, maintenanceDurationMs: null };
  }
  ...
  if (giftLayoutSignals >= 2) return { state: ScreenState.GIFT_TAB, maintenanceDurationMs: null };

  if (has('login') || has('password') || has('sign in')) {
    return { state: ScreenState.LOGIN_REQUIRED, maintenanceDurationMs: null };
  }
  if (has('loading')) return { state: ScreenState.LOADING, maintenanceDurationMs: null };

  return { state: ScreenState.UNKNOWN, maintenanceDurationMs: null };
}
