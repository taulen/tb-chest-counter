...
export function parseGiftCards(text: string): GiftEntry[] {
  ...
  let pendingName = '';
  let current: GiftEntry | null = null;

  const setName = (raw: string): void => {
    ...
    if (/[a-z]{3,}/i.test(cleaned)) pendingName = cleaned;
  };
  ...
  for (const line of lines) {
    ...
    if (fromMatch) {
      ...
      if (!pendingName) {
        ...
        if (fromIdx > 0) setName(line.slice(0, fromIdx));
      }
      ...
      if (pendingName && validPlayer) {
        current = {
          ...
          chestName: correctChestName(pendingName),
          chestType: getChestRarity(pendingName) as ChestType,
          source: '',
          timeLeft,
          giftTab: 'gifts',
          quantity: 1,
    ...
    if (/^(?:time|tome|delete|claim|open|oper)\b/i.test(line)) continue;
    setName(line);
  }
