...
export function transliterateCyrillicHomoglyphs(s: string): string {
  let out = '';
 ...
 */
export function despaceKeepingCase(s: string): string {
 ...
 */
function identityDigits(s: string): string {
...
function altKey(s: string): string {
 ...
 */
export function namesDifferByAltSuffix(a: string, b: string): boolean {
  ...
  if (identityDigits(ka) !== identityDigits(kb)) return true;
...
export function ocrNormalize(s: string): string {
 ...
 */
export function normalizeNonLatin(s: string): string {
  return s
    .normalize('NFKC')
    .toLowerCase()
