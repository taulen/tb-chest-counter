...
const log = childLogger('might-route');
...
function clampDays(raw: unknown, fallback = DEFAULT_DAYS): number {
  const n = Number.parseInt(String(raw ?? ''), 10);
  if (!Number.isFinite(n) || n <= 0) return fallback;
  return Math.min(MAX_DAYS, n);
}

export function createMightRouter(scanLoop?: ScanLoop): Router {
  const router = Router();
  ...
  router.get('/overview', (req, res) => {
    ...
    const rows = getAllMembers(true, clanId).map((m) => {
  ...
  router.get('/member/:memberId', (req, res) => {
    ...
    const member = getMemberById(memberId, clanId);
    if (!member) {
  ...
  router.get('/events', async (req, res) => {
    ...
    try {
      ...
      const feedBacked = EVENT_CATALOG.filter(
        (e) => Array.isArray(e.calendarNames) && e.calendarNames.length > 0,
      );
      ...
      for (const def of feedBacked) {
        ...
        for (const occ of occurrences) {
          if (Date.parse(occ.to) < cutoffMs) continue;
          const startMs = Date.parse(occ.from);
          const resetMs = Date.parse(occ.resetAt);
          if (!Number.isFinite(startMs) || !Number.isFinite(resetMs)) continue;
          windows.push({
            key: def.key,
            name: def.name,
            from: occ.from,
            to: occ.to,
            fromDay: gameDateFor(startMs, rolloverHr),
            ...
            toDay: gameDateFor(resetMs - 1, rolloverHr),
            label: occ.label,
          });
        }
      }
      ...
      log.warn(
        { noAlert: true },
        `Could not build might chart event annotations (chart still renders): ${String(err instanceof Error ? err.message : err)}`,
      );
   ...
   */
  router.post('/recapture', requireSuperAdmin, (req, res) => {
    if (!scanLoop) {
    ...
    const clanIds = listClans({ activeOnly: true }).map((c) => c.id);
    ...
    logAction(req.user!.id, 'might_recapture_requested', { clanIds });
  ...
  router.get('/settings', requireSuperAdmin, (req, res) => {
    ...
    res.json({
      ...
      calibrated: isMemberListCropSet(),
      cropIncludesMight: isMemberListCropRecalibrated(),
      cropRevision: getMemberListCropRevision(),
  ...
  router.put('/settings', requireSuperAdmin, (req, res) => {
    ...
    updateEnvValue('MIGHT_TRACKING_ENABLED', enabled ? 'true' : 'false');
    resetConfig();
    scanLoop?.setScannerSettings({ mightTrackingEnabled: enabled });
    logAction(req.user!.id, 'update_might_tracking', { enabled });
