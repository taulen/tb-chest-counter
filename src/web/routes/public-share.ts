 ...
 */
function applyShareHeaders(res: Response): void {
...
function paramAsString(p: string | string[] | undefined): string {
 ...
 * clan's current one.
 */
function resolvePublicShareCode(
  clanId: number,
  currentCode: string,
  requested: unknown,
): string {
  const asked = typeof requested === 'string' ? requested.trim() : '';
  if (!asked || asked === currentCode) return currentCode;
...
export function publicShareTokenHandler(req: Request, res: Response, next: NextFunction): void {
  ...
  if (segs.length !== 1 || !SHARE_TOKEN_REGEX.test(segs[0])) {
    next();
    return;
  }
 ...
 */
export function createPublicShareApiRouter(): Router {
  const router = Router();
  ...
  router.get('/:token/clan', (req, res) => {
    ...
    res.json({
      ...
      leaderboardWeeklyGoalPoints: resolveWeeklyGoalPoints(clan),
    });
  });
  ...
  router.get('/:token/external/latest', (req, res) => {
    ...
    const latest = getLatestSnapshot({ clanId: clan.id, shareCode });
    if (!latest) {
      res.json({
        snapshot: null,
        shareCode,
        isArchived: shareCode !== clan.ctShareCode,
        lastCheckedAt: getLatestPollAt({ clanId: clan.id, shareCode }),
      });
      return;
    }
    const detail = getSnapshot(latest.id);
    res.json({
      snapshot: detail,
      shareCode,
      isArchived: shareCode !== clan.ctShareCode,
  ...
  router.get('/:token/external/share-codes', (req, res) => {
    ...
    const rows = listClanShareCodes(clan.id).map((r) => ({
      ...r,
  ...
  router.get('/:token/external/snapshots', (req, res) => {
    ...
    const shareCode = resolvePublicShareCode(clan.id, clan.ctShareCode, req.query.shareCode);
    const result = listSnapshots({ limit, offset, clanId: clan.id, shareCode });
