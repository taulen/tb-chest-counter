...
import { createSetupRouter, isSetupFlowActive } from './routes/setup.js';
import { createExternalRouter } from './routes/external.js';
import { createPublicShareApiRouter, publicShareTokenHandler } from './routes/public-share.js';
import { PUBLIC_SHARE_ASSETS } from './public-share-assets.js';
import { SETUP_MODE_ASSETS } from './setup-mode-assets.js';
...
import { validateSession } from '../data/repositories/user-repo.js';
import { loginBridge } from './login-bridge.js';
import { clientAddressFor } from './login-bridge/link-quality.js';
import { BUILD_INFO } from '../utils/build-info.js';
import { startDailyBackupSchedule } from '../utils/db-backup.js';
import type { ScanLoop } from '../scheduler/loop.js';
import type { ExternalLoop, MultiClanExternalLoop } from '../scheduler/external-loop.js';
...
const BUILD_VERSION = (BUILD_INFO.fingerprint && BUILD_INFO.fingerprint !== 'unknown')
 ...
 */
function readHtmlWithVersion(filename: string): string {
  const html = fs.readFileSync(path.join(PUBLIC_DIR, filename), 'utf8');
  return versionHtmlAssets(html, BUILD_VERSION);
}
 ...
 */
function configureKeepAliveTimeouts(server: http.Server): void {
...
interface WebServerOptions {
  setupMode?: boolean;
  externalLoop?: ExternalLoop | MultiClanExternalLoop;
  /** Public URL the user actually opens in their browser. Mirrors
   *  config.webExternalUrl; when set, the startup log advertises this
   *  URL instead of the misleading "http://localhost:<port>" — useful
   *  for Docker deployments where localhost inside the container is
   *  not what the operator types into a browser. */
  externalUrl?: string;
}
 ...
 */
function setupBridgeAuth(req: Request, res: Response, next: NextFunction): void {
  if (isSetupFlowActive()) {
    const actor = firstSuperadmin();
    if (actor) {
      req.user = actor;
      next();
      return;
...
function mountSetupModeLoginBridge(app: express.Application): void {
  app.post('/api/admin/login-session/start', setupBridgeAuth, async (req, res) => {
    ...
    try {
      const dims = await loginBridge.start(1);
      logAction(req.user!.id, 'login_session_start', { clanId: 1, fromSetup: true });
 ...
 */
function attachLoginBridgeUpgradeHandler(httpServer: http.Server, setupMode: boolean): void {
  ...
  httpServer.on('upgrade', (req, socket, head) => {
    ...
    const result = token ? validateSession(token) : null;
    ...
    // that never paints.
    const duringSetup = setupMode && isSetupFlowActive();
    if (!result && duringSetup) {
      const clientAddress = clientAddressFor(req.socket.remoteAddress, req.headers['x-forwarded-for']);
      wss.handleUpgrade(req, socket, head, (ws) => {
        void loginBridge.attachSocket(ws, clientAddress);
      });
      return;
    }
...
export function startWebServer(port: number, scanLoop?: ScanLoop, options: WebServerOptions = {}): http.Server {
  const app = express();
  ...
  const restoreBodyLimit = express.json({ limit: '110mb' });
  ...
  app.use('/api/resources/upload', express.json({ limit: '110mb' }));
  ...
  if (setupMode) {
    ...
    const ensureSetupCompleted = (req: Request, res: Response, next: NextFunction): void => {
      if (isSetupFlowActive()) {
    ...
    for (const asset of SETUP_MODE_ASSETS) {
      app.get([`/${asset}`, `/v/:buildVersion/${asset}`], (_req, res) => {
        res.setHeader('Cache-Control', 'no-store');
        res.sendFile(path.join(PUBLIC_DIR, asset));
      });
    }
    ...
    app.use('/api', ensureSetupCompleted, requireAuth, requireClanContext, createApiRouter(scanLoop));
    app.use(ensureSetupCompleted, requireAuth, express.static(PUBLIC_DIR, { index: false }));
    ...
    const setupHttp = http.createServer(app);
  ...
  app.get(['/base.css', '/v/:buildVersion/base.css'], (_req, res) => {
    res.sendFile(path.join(PUBLIC_DIR, 'base.css'));
  });
  ...
  for (const asset of PUBLIC_SHARE_ASSETS) {
  ...
  app.use('/api/clans', createClansRouter(scanLoop));
  ...
  if (scanLoop) {
    loginBridge.setTeardownHook(() => scanLoop.resume());
  }
  ...
  app.use('/api', requireAuth, requireClanContext, createApiRouter(scanLoop));
  ...
  if (options.externalLoop) {
    app.use('/api/external', requireAuth, requireClanContext, createExternalRouter(options.externalLoop));
  }
  ...
  app.use('/api/resources', requireAuth, requireClanContext, createResourcesDataRouter(scanLoop));
  ...
  app.get(['/vendor/chart.umd.js', '/v/:buildVersion/vendor/chart.umd.js'], requireAuth, (_req, res) => {
    res.setHeader('Cache-Control', 'public, max-age=86400');
    res.type('application/javascript');
    res.sendFile(path.resolve('node_modules/chart.js/dist/chart.umd.js'));
  });
  ...
  app.use('/v/:buildVersion', requireAuth, (req, res, next) => {
    res.setHeader('Cache-Control', 'public, max-age=31536000, immutable');
    next();
  ...
  app.get('/{*path}', requireAuth, (_req, res) => {
...
function maskDockerBridge(addr: string): string {
  const octets = addr.split('.').map((n) => Number.parseInt(n, 10));
  if (octets.length !== 4 || octets.some((n) => !Number.isFinite(n))) return addr;
