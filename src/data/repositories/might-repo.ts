...
export interface MightSnapshotInput {
  memberId: number;
  might: number;
  /**
   * Account level from the avatar badge, or null/omitted when it wasn't in the
   * crop. Stored as 0 for "unknown" because the column is NOT NULL from the v30
   * bootstrap — read it as `level > 0` rather than trusting a zero.
   */
  level?: number | null;
  /**
   * Path to a saved crop of the member-list row this reading came from. Written
   * only for rows that introduced a NEW member, as the evidence an admin needs
   * to acknowledge them — see migration v58.
   */
  cropPath?: string;
}

export interface MightPoint {
  gameDate: string;
  might: number;
  /**
   * Hero level on that game day, or null when it wasn't read.
   *
   * Carried on the same point as the might value because they come from the same
   * snapshot row, so the two series are always aligned on the same day — no join, no
   * separate query, and no chance of charting them against different dates.
   *
   * Null rather than 0: the column is NOT NULL so "not read" is stored as 0, and a
   * series that reported that literally would draw a level collapsing to zero on any
   * day the crop happened to clip the avatars.
   */
  heroLevel: number | null;
}

export interface MemberMightRow {
  memberId: number;
  name: string;
  might: number;
  /** Hero level at the latest reading, or null when it has never been captured
   *  (the crop has to include the avatars — see memberListCrop). */
  heroLevel: number | null;
  gameDate: string;
  /** Snapshot the delta is measured from — null when history is too short. */
  baselineMight: number | null;
  baselineDate: string | null;
  /** might - baselineMight, or null when there's no baseline yet. */
  delta: number | null;
}

export interface ClanMightTotal {
  gameDate: string;
  totalMight: number;
  memberCount: number;
}

export interface MemberMightSeries {
  memberId: number;
  name: string;
  points: MightPoint[];
}

function dropCaches(clanId: number): void {
 ...
 */
function longestNonDecreasingIndices(values: number[]): Set<number> {
  ...
  for (let k = tails.length > 0 ? tails[tails.length - 1] : -1; k >= 0; k = prev[k]) keep.add(k);
  return keep;
}
 ...
 */
function trustedHeroLevels(clanId: number, memberIds?: number[]): Map<number, Map<string, number>> {
  const db = getDb();
  ...
  `).all(clanId, ...(memberIds ?? [])) as Array<{
    memberId: number; gameDate: string; level: number;
  }>;

  const byMember = new Map<number, Array<{ gameDate: string; level: number }>>();
  for (const r of rows) {
  ...
  for (const [memberId, series] of byMember) {
    ...
    series.forEach((s, i) => { if (keep.has(i)) kept.set(s.gameDate, s.level); });
...
function latestTrusted(kept: Map<string, number> | undefined): number | null {
  if (!kept || kept.size === 0) return null;
  let last: number | null = null;
  for (const level of kept.values()) last = level;
  return last;
}
...
export function saveSnapshots(
  ...
  const tx = db.transaction((batch: MightSnapshotInput[]) => {
    let n = 0;
    for (const row of batch) {
      stmt.run(
        row.memberId, clanId, row.level ?? 0, row.might,
        capturedAtIso, gameDate, row.cropPath ?? null,
      );
      n++;
    }
    return n;
  ...
  const written = tx(rows) as number;
  dropCaches(clanId);
  return written;
}
 ...
 */
export function hasSnapshotForGameDate(clanId: number, gameDate: string): boolean {
  const db = getDb();
  const row = db.prepare(
    `SELECT 1 AS hit FROM member_snapshots
     WHERE clan_id = ? AND game_date = ? LIMIT 1`,
  ).get(clanId, gameDate) as { hit: number } | undefined;
  return row !== undefined;
}

/** When this clan's most recent might capture ran, or null if never. */
export function getLastCaptureAt(clanId: number): { capturedAt: string; gameDate: string } | null {
  const db = getDb();
  const row = db.prepare(
    `SELECT captured_at AS capturedAt, game_date AS gameDate
     FROM member_snapshots
     WHERE clan_id = ? AND game_date != ''
     ORDER BY game_date DESC, captured_at DESC LIMIT 1`,
  ).get(clanId) as { capturedAt: string; gameDate: string } | undefined;
  return row ?? null;
}

/** Distinct game days this clan has data for, oldest first. */
export function getSnapshotDates(clanId: number): string[] {
  const db = getDb();
  const rows = db.prepare(
    `SELECT DISTINCT game_date AS d FROM member_snapshots
     WHERE clan_id = ? AND game_date != '' ORDER BY d ASC`,
  ).all(clanId) as Array<{ d: string }>;
 ...
 * readings.
 */
export function getMightWithDelta(clanId: number, deltaDays = 7): MemberMightRow[] {
  return cached(`might:${clanId}:delta:${deltaDays}`, CHART_CACHE_TTL_MS, () => {
    const db = getDb();
    const rows = db.prepare(`
      WITH ranked AS (
    ...
    `).all(
      clanId, clanId, `-${deltaDays} days`, clanId, `-${deltaDays} days`, clanId,
    ) as Array<{
      memberId: number; name: string; might: number; gameDate: string;
      baselineMight: number | null; baselineDate: string | null;
    }>;
 ...
 */
export function getLatestMightByMember(
  clanId: number,
): Map<number, { might: number; heroLevel: number | null; gameDate: string }> {
  return cached(`might:${clanId}:latest-by-member`, CHART_CACHE_TTL_MS, () => {
    const db = getDb();
    const rows = db.prepare(`
      WITH ranked AS (
    ...
    `).all(clanId) as Array<{ memberId: number; might: number; gameDate: string }>;

    const trusted = trustedHeroLevels(clanId);
    const out = new Map<number, { might: number; heroLevel: number | null; gameDate: string }>();
    for (const r of rows) {
      out.set(r.memberId, {
        might: r.might,
        heroLevel: latestTrusted(trusted.get(r.memberId)),
 ...
 */
export function getLatestForMember(
  memberId: number,
  clanId: number,
): { might: number; heroLevel: number | null; gameDate: string } | null {
  const db = getDb();
  const row = db.prepare(
    `SELECT power AS might, game_date AS gameDate
     FROM member_snapshots
     WHERE member_id = ? AND clan_id = ? AND game_date != ''
     ORDER BY game_date DESC LIMIT 1`,
  ).get(memberId, clanId) as { might: number; gameDate: string } | undefined;
  if (!row) return null;
 ...
 */
export function getMemberHistory(memberId: number, clanId: number, days = 0): MightPoint[] {
  const db = getDb();
  const clause = days > 0
  ...
  if (days > 0) params.push(clanId, `-${days} days`);
  ...
  `).all(...params) as Array<{ gameDate: string; might: number }>;
 ...
 */
export function getClanTotals(clanId: number, days = 90): ClanMightTotal[] {
  return cached(`might:${clanId}:totals:${days}`, CHART_CACHE_TTL_MS, () => {
    const db = getDb();
    return db.prepare(`
      ...
      GROUP BY game_date
      ORDER BY game_date ASC
    `).all(clanId, clanId, `-${days} days`) as ClanMightTotal[];
  });
}
 ...
 */
export function getSeriesForMembers(
  clanId: number,
  memberIds: number[],
  days = 90,
): MemberMightSeries[] {
  ...
  const names = db.prepare(
    `SELECT id, name FROM members WHERE clan_id = ? AND id IN (${placeholders})`,
  ).all(clanId, ...memberIds) as Array<{ id: number; name: string }>;
  ...
  const rows = db.prepare(`
    ...
    WHERE clan_id = ? AND game_date != '' AND member_id IN (${placeholders})
      AND game_date >= date((SELECT MAX(game_date) FROM member_snapshots
                             WHERE clan_id = ? AND game_date != ''), ?)
    ORDER BY game_date ASC
  `).all(clanId, ...memberIds, clanId, `-${days} days`) as Array<{
    memberId: number; gameDate: string; might: number;
  }>;

  const trusted = trustedHeroLevels(clanId, memberIds);
  const byMember = new Map<number, MightPoint[]>();
  for (const r of rows) {
    const list = byMember.get(r.memberId) ?? [];
    list.push({
      gameDate: r.gameDate,
