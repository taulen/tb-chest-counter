...
import { clanRewardExclusionSql } from '../clan-reward-chests.js';
import * as chestSummaryRepo from './chest-summary-repo.js';
import { invalidateScanStats } from './session-repo.js';
import { invalidateReviewQueueCount } from './review-queue-repo.js';
import { invalidateSourceKeySummary } from './source-points-repo.js';
import { sourceSpellingKey } from '../../vision/source-names.js';
import { childLogger } from '../../utils/logger.js';
import { cached, invalidate } from '../../utils/ttl-cache.js';
import type { ChestRecord, MemberStats, LeaderboardEntry } from '../../models/types.js';
import type { ChestType } from '../../models/enums.js';

const log = childLogger('chest-repo');
...
export const ANALYTICS_CACHE_TTL_MS = 15000;
...
export function invalidateUnknownChestsCount(clanId?: number): void {
  invalidate(clanId === undefined ? 'unknownChestsCount:' : `unknownChestsCount:${clanId}`);
}
...
function confFloatToInt(f: number): number {
  return Math.round(f * 100);
}

function confIntToFloat(i: number): number {
  return i / 100;
}
 ...
 */
export function slugifyChestName(name: string): string {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
 ...
 */
export function resolveChestName(input: string, clanId: number): string {
  const trimmed = input.trim();
  if (!trimmed) return trimmed;
  const db = getDb();

  const exact = db.prepare(
    `SELECT 1 FROM chest_records_v WHERE clan_id = ? AND chest_name = ? LIMIT 1`,
  ).get(clanId, trimmed) as { 1: number } | undefined;
  if (exact) return trimmed;

  const targetSlug = slugifyChestName(trimmed);
  if (!targetSlug) return trimmed;
  ...
  const candidate = db.prepare(`
    ...
    GROUP BY chest_name
    ORDER BY cnt DESC
  `).all(clanId) as Array<{ chest_name: string; cnt: number }>;
...
function rowToChest(row: Record<string, unknown>): ChestRecord {
  return {
    ...
    confidence: confIntToFloat(row.confidence as number),
  };
}
 ...
 */
export function getOrCreateChestId(name: string, defaultType: string): number {
  const db = getDb();
  const row = db.prepare(`
    INSERT INTO chests (name, chest_type) VALUES (?, ?)
    ON CONFLICT(name) DO UPDATE SET name = excluded.name
    RETURNING id
  `).get(name, defaultType) as { id: number };
  return row.id;
}
...
export function getOrCreateChestSourceId(source: string): number | null {
  if (!source) return null;
  const db = getDb();

  // Fast path: exact match / existing row.
  const exact = db.prepare('SELECT id FROM chest_sources WHERE source = ?').get(source) as { id: number } | undefined;
  ...
  const canon = sourceSpellingKey(source);
  if (canon) {
    const candidates = db.prepare('SELECT id, source FROM chest_sources').all() as { id: number; source: string }[];
    let bestId: number | null = null;
    let bestCnt = -1;
    for (const c of candidates) {
      if (sourceSpellingKey(c.source) !== canon) continue;
      const cnt = (db.prepare('SELECT COUNT(*) AS n FROM chest_records WHERE chest_source_id = ?').get(c.id) as { n: number }).n;
      if (cnt > bestCnt) { bestCnt = cnt; bestId = c.id; }
    }
...
export function insertChest(
  chest: Omit<ChestRecord, 'id'> & {
    ...
    clanId: number;
    /** Raw OCR'd player name BEFORE matchKnownPlayer rewriting.
     *  Forensic-only: set by the scan pipeline when ENABLE_RAW_OCR_CAPTURE
     *  is on, NULL otherwise. */
    rawPlayerOcr?: string | null;
    /** Best-effort in-game received time (epoch ms) from the gift's "time
     *  left" countdown; see src/utils/gift-time.ts. NULL for non-scan insert
     *  paths (imports/backfills) and historical rows. */
  ...
  try {
    ...
    `).run(
      ...
      confFloatToInt(chest.confidence),
    ...
    if (err instanceof Error && err.message.includes('UNIQUE constraint')) {
      log.debug(
        `insertChest UNIQUE skip: session=${chest.sessionId} player="${chest.playerName}" chest="${chest.chestName}" source="${chest.chestSource}" capturedAt=${chest.capturedAt}`,
      );
      return null;
    }
    throw err;
...
export function purgeRawPlayerOcr(): { chestRecords: number; triumphalChestRecords: number } {
  const db = getDb();
...
export function getRawPlayerOcrCounts(): { chestRecords: number; triumphalChestRecords: number } {
  const db = getDb();
  const a = db.prepare('SELECT COUNT(*) AS c FROM chest_records WHERE raw_player_ocr IS NOT NULL').get() as { c: number };
  const b = db.prepare('SELECT COUNT(*) AS c FROM triumphal_chest_records WHERE raw_player_ocr IS NOT NULL').get() as { c: number };
 ...
 */
export function deleteChestsBySession(sessionId: number, clanId: number): number {
  const db = getDb();
  const result = db.prepare(
    'DELETE FROM chest_records WHERE session_id = ? AND clan_id = ?',
  ).run(sessionId, clanId);
  if (result.changes > 0) chestSummaryRepo.notifyChestDataChanged(clanId);
 ...
 */
export function countChestsBySession(sessionId: number, clanId: number): number {
  const db = getDb();
  const row = db.prepare(
    'SELECT COUNT(*) AS c FROM chest_records WHERE session_id = ? AND clan_id = ?',
  ).get(sessionId, clanId) as { c: number };
  return row.c;
}

export function getChestsBySession(sessionId: number, clanId: number): ChestRecord[] {
  const db = getDb();
  const rows = db.prepare(
    'SELECT * FROM chest_records_v WHERE session_id = ? AND clan_id = ? ORDER BY captured_at DESC',
 ...
 */
export function getMemberAggregateInRange(
  memberId: number,
  clanId: number,
  from: string,
  to: string,
): { chests: number; points: number } {
  const db = getDb();
  const row = db.prepare(`
    ...
    WHERE member_id = ?
      ...
      AND effective_at >= ?
      AND effective_at < ?
  `).get(memberId, clanId, isoToMs(from), isoToMs(to)) as { chests: number; points: number };
...
export function getChests(filters: {
  from?: string;
  to?: string;
  memberId?: number;
  chestType?: string;
  ...
  const params: unknown[] = [filters.clanId];
...
export function getMemberStats(memberId: number, clanId: number, from?: string, to?: string): MemberStats | null {
  const db = getDb();

  const member = db.prepare(
    'SELECT * FROM members WHERE id = ? AND clan_id = ?',
  ).get(memberId, clanId) as Record<string, unknown> | undefined;
  ...
  const rows = db.prepare(query).all(...params) as { chest_type: string; total: number; points: number }[];

  const chestsByType: Partial<Record<ChestType, number>> = {};
  let totalChests = 0;
  let totalPoints = 0;

  for (const row of rows) {
    chestsByType[row.chest_type as ChestType] = row.total;
  ...
  return {
    ...
    totalPoints,
    chestsByType,
    lastSeen: member.last_seen as string,
  };
}
...
export function getLeaderboard(
  ...
  from?: string,
  to?: string,
  options: { limit?: number; includeAllMembers?: boolean } = {},
): LeaderboardEntry[] {
  const { limit, includeAllMembers = false } = options;
  const key = `leaderboard:${clanId}:${from ?? ''}:${to ?? ''}:${includeAllMembers ? 1 : 0}:${limit ?? ''}`;
  return cached(key, ANALYTICS_CACHE_TTL_MS, () => computeLeaderboard(clanId, from, to, options));
}
...
function computeLeaderboard(
  ...
  const aggConditions: string[] = ['clan_id = ?'];
  const aggParams: unknown[] = [clanId];
  if (from) {
  ...
  const aggWhere = `WHERE ${aggConditions.join(' AND ')}${clanRewardExclusionSql()} `;
  const aggSql = `
    SELECT member_id,
           COUNT(*) AS chests,
  ...
  const rows = db.prepare(query).all(...params) as { member_id: number; member_name: string; total_chests: number; total_points: number }[];
...
export function getTotalChestCount(clanId: number): number {
  const db = getDb();
  const row = db.prepare(
    'SELECT COUNT(*) as total FROM chest_records WHERE clan_id = ?',
  ).get(clanId) as { total: number | null };
  return row.total ?? 0;
}

export function getRecentChests(clanId: number, limit: number = 10): ChestRecord[] {
  const db = getDb();
  const rows = db.prepare(
    'SELECT * FROM chest_records_v WHERE clan_id = ? ORDER BY captured_at DESC LIMIT ?',
  ).all(clanId, limit) as Record<string, unknown>[];
  return rows.map(rowToChest);
}

/**
 * A single-day personal record: "member X had Y chests / Z points on
 * date D". Used for the "Clan Records" card on the Analytics page and
 * the matching badge on the member profile page. The date is in
 * YYYY-MM-DD form (SQLite's DATE() output).
 */
 ...
 */
export function getSingleDayRecords(rolloverUtcHour: number, clanId: number): {
  ...
  return cached(
    `singleDay:${clanId}:${rolloverUtcHour}`,
    ANALYTICS_CACHE_TTL_MS,
    () => chestSummaryRepo.getSingleDayRecords(clanId),
  );
}
 ...
 */
export function getDailyDigestData(fromIso: string, toIso: string, clanId: number): {
  totalChests: number;
  totalPoints: number;
  activePlayers: number;
  scanCount: number;
  topContributors: Array<{ memberId: number | null; memberName: string; chests: number; points: number }>;
} {
  const db = getDb();
  ...
  const rows = db.prepare(`
    ...
    WHERE c.clan_id = ? AND c.effective_at >= ? AND c.effective_at < ?
      ${clanRewardExclusionSql('c.')}
    GROUP BY c.member_id, m.name
    ORDER BY points DESC, chests DESC, LOWER(memberName) ASC
  `).all(clanId, isoToMs(fromIso), isoToMs(toIso)) as Array<{ memberId: number | null; memberName: string; chests: number; points: number }>;
  ...
  `).get(clanId, fromIso, toIso) as { cnt: number };
 ...
 */
export function getMembersByChestName(
  chestName: string,
  clanId: number,
  from?: string,
  to?: string,
  ...
  avgPointsPerChest: number;
  firstSeen: string | null;
  lastSeen: string | null;
  members: Array<{ memberId: number | null; memberName: string; count: number; points: number; lastSeen: string | null }>;
} {
  const db = getDb();
  ...
  const chestRow = db.prepare(
    'SELECT id, chest_type FROM chests WHERE name = ?',
  ).get(chestName) as { id: number; chest_type: string } | undefined;
  ...
  const dateConditions: string[] = ['clan_id = ?', 'chest_id = ?'];
  const dateParams: unknown[] = [clanId, chestRow.id];
  if (from) {
  ...
  const rawSeen = db.prepare(`
    ...
    FROM chest_records
    ${where}
  `).get(...dateParams) as { firstSeen: number | null; lastSeen: number | null };
  const seenRow = {
 ...
 */
export function getChestHistoryForChestAndMember(
  chestName: string,
  memberId: number | null,
  ...
  const db = getDb();
  const params: unknown[] = [chestName, clanId];
  let query = `
 ...
 */
export function getUnknownChests(clanId: number): Array<{
  id: number;
  sessionId: number;
  capturedAt: string;
  effectiveAt: string;
  ...
  `).all(clanId, clanId) as Record<string, unknown>[];
  return rows.map((r) => ({
    id: r.id as number,
 ...
 */
export function countUnknownChests(clanId: number): number {
  return cached(`unknownChestsCount:${clanId}`, UNKNOWN_CHESTS_COUNT_TTL_MS, () =>
    computeUnknownChestsCount(clanId),
  );
}

function computeUnknownChestsCount(clanId: number): number {
  const db = getDb();
  const row = db.prepare(`
    SELECT COUNT(*) AS cnt
    FROM chest_records
  ...
  `).get(clanId, clanId) as { cnt: number };
  return row.cnt;
}
 ...
 */
export function getDebugCropPathForChest(chestId: number, clanId: number): string | null {
  const db = getDb();
  const row = db.prepare(
    'SELECT debug_crop_path FROM chest_records WHERE id = ? AND clan_id = ?',
  ).get(chestId, clanId) as { debug_crop_path: string | null } | undefined;
 ...
 */
export function reassignChestsToMember(
  chestIds: number[],
  toMemberId: number,
  _toMemberName: string,
  clanId: number,
): number {
  if (chestIds.length === 0) return 0;
  const db = getDb();
  ...
  ).run(toMemberId, clanId, ...chestIds);
 ...
 */
export function clearResolvedManualReviewErrors(clanId: number): number {
  const db = getDb();
  const result = db.prepare(`
 ...
 */
export function deleteOrphanedEmptyMembers(clanId: number): number {
  const db = getDb();
 ...
 */
export function getDistinctChestSources(clanId: number): { source: string; count: number }[] {
  const db = getDb();
  const rows = db.prepare(`
  ...
  `).all(clanId) as { chest_source: string; cnt: number }[];
 ...
 */
export function getChestCountsByMember(clanId: number): Record<number, number> {
  const db = getDb();
  const rows = db.prepare(`
    ...
    WHERE clan_id = ?
    GROUP BY member_id
  `).all(clanId) as { member_id: number; cnt: number }[];
  const counts: Record<number, number> = {};
  for (const r of rows) counts[r.member_id] = r.cnt;
  return counts;
}
 ...
 */
export function getDistinctChestNames(clanId: number): { name: string; currentType: string; count: number }[] {
  const db = getDb();
  const rows = db.prepare(`
...
export interface ChestBreakdowns {
  bySource: Array<{ chest_source: string; count: number; points: number }>;
  byType: Array<{ chest_type: string; count: number; points: number }>;
  byName: Array<{ chest_name: string; chest_type: string; count: number; points: number }>;
}
 ...
 */
export function getChestBreakdowns(
  clanId: number,
  fromMs?: number,
  toMs?: number,
): ChestBreakdowns {
  ...
  const params: unknown[] = windowed ? [clanId, fromMs, toMs] : [clanId];
  ...
  const bySource = db.prepare(`
    ...
    ORDER BY count DESC
  `).all(...params) as ChestBreakdowns['bySource'];
  ...
  const byName = db.prepare(`
    ...
    ORDER BY count DESC
  `).all(...params) as ChestBreakdowns['byName'];
  ...
  const typeTotals = new Map<string, { chest_type: string; count: number; points: number }>();
...
export interface ActivityClock {
  /** [weekday 0=Sun][hour 0..23] → chests earned in that slot. */
  grid: number[][];
  /** Chests in the window whose earn time is a real reading. */
  timed: number;
  /** Chests in the window in total, timed or not. */
  total: number;
}
 ...
 * a clock.
 */
export function getActivityClock(
  clanId: number,
  fromMs?: number,
  toMs?: number,
): ActivityClock {
  ...
  const rows = db.prepare(`
    ...
    GROUP BY dow, hour
  `).all(...params) as Array<{ dow: number; hour: number; chests: number }>;
 ...
 */
export function getMemberSourceMix(
  memberId: number,
  clanId: number,
  fromMs?: number,
  toMs?: number,
): Array<{ source: string; chests: number; points: number }> {
  const db = getDb();
  const windowed = Number.isFinite(fromMs) && Number.isFinite(toMs);
  const range = windowed ? ' AND effective_at >= ? AND effective_at < ?' : '';
  ...
  `).all(...params) as Array<{ sourceId: number | null; chests: number; points: number }>;

  if (grouped.length === 0) return [];

  const ids = grouped.map((g) => g.sourceId).filter((id): id is number => id !== null);
  const names = new Map<number, string>();
  if (ids.length > 0) {
    const rows = db.prepare(
      `SELECT id, source FROM chest_sources WHERE id IN (${ids.map(() => '?').join(',')})`,
    ).all(...ids) as Array<{ id: number; source: string }>;
    for (const r of rows) names.set(r.id, r.source);
  }
