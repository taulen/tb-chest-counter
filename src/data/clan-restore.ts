...
import { correctChestName, getChestRarity } from '../vision/chest-names.js';
import { childLogger } from '../utils/logger.js';

const log = childLogger('clan-restore');

/** Schema name the backup is attached under. */
const SRC = 'restore_src';

export interface BackupClanSummary {
  clanId: number;
  name: string;
  slug: string;
  createdAt: string;
  /** Row counts in the backup, for the operator to sanity-check before restoring. */
  counts: {
    members: number;
    chestRecords: number;
    triumphalRecords: number;
    scanSessions: number;
    memberSnapshots: number;
    resourceTransactions: number;
    /** Accounts scoped to this clan in the backup. NOT restored — see module header. */
    users: number;
  };
  /** Newest chest capture in the backup, ISO — i.e. how fresh this copy is. */
  newestChestAt: string | null;
  /** A live clan already holds this id (so the restore would land on a new one). */
  idTakenLive: boolean;
  /** A live clan already holds this name or slug — the restore refuses. */
  nameTakenLive: boolean;
}

export interface BackupInspection {
  schemaVersion: number | null;
  clans: BackupClanSummary[];
}

/** What happened to the clan's user accounts. Every field is worth surfacing. */
export interface RestoredUsers {
  restored: string[];
  /** Usernames already present live, left untouched. */
  skipped: string[];
  /** Restored as `admin` because the backup had them as `superadmin`. */
  demoted: string[];
}

export interface ClanRestoreResult {
  sourceClanId: number;
  clanId: number;
  name: string;
  /** Rows inserted, per table. Zero-count tables are omitted. */
 ...
 */
function materializeBackup(sourcePath: string): { dbPath: string; cleanup: () => void } {
  ...
  const liveDb = path.resolve(process.env.DB_PATH || './data/tb-chests.db');
  const tempDir = path.join(path.dirname(liveDb), 'backups', 'uploads');
  fs.mkdirSync(tempDir, { recursive: true });
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  const dbPath = path.join(tempDir, `clan-restore-${stamp}.db`);
  ...
  if (head[0] === 0x1f && head[1] === 0x8b) {
    fs.writeFileSync(dbPath, zlib.gunzipSync(fs.readFileSync(sourcePath)));
  } else {
 ...
 */
export function inspectBackupClans(sourcePath: string): BackupInspection {
  ...
  try {
    ...
    const schemaVersion = (src.prepare(
      'SELECT MAX(version) AS v FROM schema_version',
    ).get() as { v: number | null } | undefined)?.v ?? null;

    const liveClans = live.prepare('SELECT id, name, slug FROM clans').all() as
    ...
    const rows = src.prepare(
      'SELECT id, name, slug, created_at FROM clans ORDER BY id',
    ).all() as { id: number; name: string; slug: string; created_at: string }[];

    const clans: BackupClanSummary[] = rows.map((c) => {
      const newest = (() => {
        try {
          const r = src!.prepare(
            'SELECT MAX(captured_at) AS m FROM chest_records WHERE clan_id = ?',
          ).get(c.id) as { m: number | null } | undefined;
      ...
      return {
        ...
        counts: {
          ...
          memberSnapshots: safeCount(
            src!,
            'SELECT COUNT(*) AS c FROM member_snapshots WHERE member_id IN (SELECT id FROM members WHERE clan_id = ?)',
            c.id,
          ),
...
function copyableColumns(db: Database.Database, table: string, drop: string[]): string[] {
  const main = (db.prepare(`PRAGMA main.table_info(${table})`).all() as { name: string }[])
    .map((c) => c.name);
  const srcCols = new Set(
    (db.prepare(`PRAGMA ${SRC}.table_info(${table})`).all() as { name: string }[]).map((c) => c.name),
  );
...
interface CopySpec {
  table: string;
  /** WHERE clause over alias `s` (the backup row). */
  where: string;
  /** Column → SQL expression over `s`. Anything unlisted copies as `s.<col>`. */
  map?: Record<string, string>;
  /** Columns to leave out entirely (an AUTOINCREMENT id nothing references). */
  drop?: string[];
  /** Use INSERT OR IGNORE — for tables with a UNIQUE key the live side may hold. */
  orIgnore?: boolean;
}

function copyTable(db: Database.Database, spec: CopySpec): number {
...
function maxId(db: Database.Database, table: string): number {
  const row = db.prepare(`SELECT COALESCE(MAX(id), 0) AS m FROM main.${table}`).get() as { m: number };
  return row.m;
}
...
function buildRefMap(
  ...
  db.prepare(
    `INSERT INTO temp.map_${kind} (old, new)
     SELECT s.id, m.id
       FROM ${SRC}.${table} s
       JOIN main.${table} m ON ${keyExpr}
      WHERE s.id IN (${usedIdsSql})`,
  ).run();
}

export interface ClanRestoreOptions {
  /** Reject the restore unless the backup's clan id is still free. Default false. */
 ...
 */
export function restoreClanFromBackup(
  sourcePath: string,
  sourceClanId: number,
  options: ClanRestoreOptions = {},
): ClanRestoreResult {
  const { dbPath, cleanup } = materializeBackup(sourcePath);
  const db = getDb();
  ...
  try {
    const srcClan = db.prepare(
      `SELECT * FROM ${SRC}.clans WHERE id = ?`,
    ).get(sourceClanId) as Record<string, unknown> | undefined;
    if (!srcClan) {
    ...
    const clash = db.prepare(
      'SELECT id, name FROM main.clans WHERE LOWER(name) = LOWER(?) OR LOWER(slug) = LOWER(?)',
    ).get(name, slug) as { id: number; name: string } | undefined;
    if (clash) {
      throw new Error(
        `Clan "${clash.name}" (id ${clash.id}) is already live under that name. `
    ...
    const users: RestoredUsers = { restored: [], skipped: [], demoted: [] };
    const tables: Record<string, number> = {};
    const record = (table: string, n: number): void => {
      if (n > 0) tables[table] = (tables[table] ?? 0) + n;
    };
    ...
    const tx = db.transaction(() => {
      ...
      db.prepare(
        `INSERT INTO main.clans (${clanCols.join(', ')}) `
        + `SELECT ${clanCols.map((c) => (c === 'id' ? '?' : c === 'created_by' ? 'NULL' : `s.${c}`)).join(', ')} `
        + `FROM ${SRC}.clans s WHERE s.id = ?`,
      ).run(targetId, sourceClanId);
      record('clans', 1);

      // ── 2. Global reference tables, by name ───────────────────────────────
      // Only the rows this clan referenced, canonicalised on the way in.
      const usedChestIds = db.prepare(
        `SELECT DISTINCT chest_id AS id FROM ${SRC}.chest_records WHERE clan_id = @c
         UNION SELECT DISTINCT chest_id FROM ${SRC}.triumphal_chest_records WHERE clan_id = @c`,
      ).all({ c: sourceClanId }) as { id: number }[];

      if (usedChestIds.length > 0) {
        const idList = usedChestIds.map((r) => r.id).join(',');
        const srcChests = db.prepare(
          `SELECT id, name FROM ${SRC}.chests WHERE id IN (${idList})`,
        ).all() as { id: number; name: string }[];
        const insertChest = db.prepare(
          'INSERT OR IGNORE INTO main.chests (name, chest_type) VALUES (?, ?)',
        );
        let added = 0;
        for (const c of srcChests) {
          const canonical = correctChestName(c.name);
          added += insertChest.run(canonical, getChestRarity(canonical)).changes;
        }
        record('chests', added);
        ...
        for (const c of srcChests) {
          const live = liveChestId.get(correctChestName(c.name)) as { id: number } | undefined;
          if (!live) throw new Error(`Could not map chest "${c.name}" into the live chests table`);
      ...
      record('chest_sources', db.prepare(
        `INSERT OR IGNORE INTO main.chest_sources (source)
      ...
      buildRefMap(
        db, 'source', 'chest_sources', 'm.source = s.source',
        `SELECT chest_source_id FROM ${SRC}.chest_records WHERE clan_id = ${sourceClanId} AND chest_source_id IS NOT NULL
      ...
      const usedResourceTypes =
        `SELECT resource_type_id FROM ${SRC}.resource_transactions WHERE clan_id = ${sourceClanId} AND resource_type_id IS NOT NULL
         UNION SELECT resource_type_id FROM ${SRC}.resource_icon_templates WHERE clan_id = ${sourceClanId}`;
      record('resource_types', db.prepare(
        `INSERT OR IGNORE INTO main.resource_types (name, slug)
         SELECT s.name, s.slug FROM ${SRC}.resource_types s WHERE s.id IN (${usedResourceTypes})`,
      ).run().changes);
      buildRefMap(db, 'restype', 'resource_types', 'm.slug = s.slug', usedResourceTypes);

      const usedChestDefs =
        `SELECT chest_definition_ref_id FROM ${SRC}.snapshot_chest_definition
          WHERE snapshot_id IN (SELECT id FROM ${SRC}.snapshot WHERE clan_id = ${sourceClanId})`;
      record('chest_definition_ref', db.prepare(
        `INSERT INTO main.chest_definition_ref (type, name, source, points, override_points)
      ...
      buildRefMap(
        db, 'chestdef', 'chest_definition_ref',
        'm.type = s.type AND m.name = s.name AND m.source = s.source',
        usedChestDefs,
      );
      ...
      if (userCols.length > 0) {
        const srcUsers = db.prepare(
          `SELECT * FROM ${SRC}.users WHERE clan_id = ?`,
        ).all(sourceClanId) as Record<string, unknown>[];

        const usernameTaken = db.prepare('SELECT 1 AS ok FROM main.users WHERE username = ?');
        const insertUser = db.prepare(
          `INSERT INTO main.users (${userCols.join(', ')}) `
        ...
        for (const u of srcUsers) {
          ...
          for (const c of userCols) row[c] = u[c] ?? null;
      ...
      db.prepare(
        `INSERT INTO temp.map_user (old, new)
         SELECT s.id, m.id FROM ${SRC}.users s JOIN main.users m ON m.username = s.username`,
      ).run();

      const userExpr = (col: string): string => `(SELECT new FROM temp.map_user WHERE old = s.${col})`;
    ...
    log.info(
      `Restored clan "${name}" from backup as clan #${targetId}: ${totalRows} row(s) across `
      + `${Object.keys(tables).length} table(s), ${users.restored.length} user account(s)`,
    );
    if (users.skipped.length > 0) {
      log.warn(
        { noAlert: true },
        `Clan restore left ${users.skipped.length} account(s) alone — the username is already in use: `
        + users.skipped.join(', '),
      );
    }
    if (users.demoted.length > 0) {
      log.warn(
        { noAlert: true },
        `Clan restore brought back ${users.demoted.join(', ')} as admin rather than superadmin.`,
      );
    }
