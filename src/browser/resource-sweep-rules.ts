...
/**
 * The fields a row's identity is built from, as a structural type rather than
 * ExtractedRow.
 *
 * Deliberately minimal so the SAME key functions can be applied to a row read off
 * the screen and to a row read back out of the database — which is the whole point
 * of the withhold below, and the reason this module imports nothing at all.
 */
...
export interface DatedRowIdentity extends RowIdentity {
...
/** The fields of a stored row the withhold compares. */
export type RecordedRow = DatedRowIdentity & ResourceField;
 ...
 */
export const CURSOR_ROWS = 12;
export const CURSOR_MIN_RUN = 4;
 ...
 * or renames a player.
 */
export function rowFingerprint(row: RowIdentity & ResourceField): string {
 ...
 * align.
 */
export function fingerprintsAlign(a: string, b: string): boolean {
  if (a === b) return true;
  ...
  if (cut < 0 || cut !== b.lastIndexOf('|')) return false;
  const aResource = a.slice(cut + 1);
  const bResource = b.slice(cut + 1);
  if (aResource !== UNRESOLVED_RESOURCE && bResource !== UNRESOLVED_RESOURCE) return false;
 ...
 */
export function sweepRowKey(row: DatedRowIdentity & ResourceField): string {
...
export function looseSweepKey(row: DatedRowIdentity): string {
 ...
 */
export function looseRowKey(row: RowIdentity): string {
 ...
 */
export function firstSettledRowIndex(
  rows: ReadonlyArray<{ transactionDate: string }>,
  openDates: ReadonlySet<string>,
): number {
  for (let i = 0; i < rows.length; i++) {
    if (!openDates.has(rows[i].transactionDate)) return i;
  }
  return -1;
}
 ...
 */
export function buildCursorFingerprints(
  rows: ReadonlyArray<{ transactionDate: string; fingerprint: string }>,
  openDates: ReadonlySet<string>,
): { fingerprints: string[]; anchorAt: number; anchorDate: string | null } {
  const at = firstSettledRowIndex(rows, openDates);
  if (at < 0) return { fingerprints: [], anchorAt: -1, anchorDate: null };
  const slice = rows.slice(at, at + CURSOR_ROWS);
  if (slice.length < CURSOR_MIN_RUN) return { fingerprints: [], anchorAt: at, anchorDate: null };
  return {
 ...
 */
export const ANCHOR_SAFE_STOP_REASONS: ReadonlySet<string> = new Set([
  'cursor',
  'end-of-list',
  'date-floor',
]);
 ...
 */
export const DATE_BACKSTOP_MARGIN_DAYS = 2;
export const REREAD_OPEN_DAY_DAYS = 1;

export function maxDaysBackFor(markerAgeDays: number): number {
  return markerAgeDays + REREAD_OPEN_DAY_DAYS + DATE_BACKSTOP_MARGIN_DAYS;
}
...
export function readCompleteness(
  ...
  cut: number,
  stopReason: string,
): { completeDates: Set<string>; oldestDateRead: string; monotone: boolean } {
  if (rows.length === 0) return { completeDates: new Set(), oldestDateRead: '', monotone: true };

  let monotone = true;
  for (let i = 1; i < rows.length; i++) {
    if (rows[i].transactionDate > rows[i - 1].transactionDate) { monotone = false; break; }
  }

  const oldestDateRead = rows[rows.length - 1].transactionDate;
  if (!monotone) return { completeDates: new Set(), oldestDateRead, monotone };

  const belowCut = new Set<string>();
  for (let i = Math.max(0, cut); i < rows.length; i++) belowCut.add(rows[i].transactionDate);

  const completeDates = new Set<string>();
  for (let i = 0; i < Math.min(cut, rows.length); i++) {
    const d = rows[i].transactionDate;
    if (belowCut.has(d)) continue;
    if (d === oldestDateRead && stopReason !== 'end-of-list') continue;
    completeDates.add(d);
  }
 ...
 */
export function withholdAlreadyRecorded<T extends DatedRowIdentity & ResourceField>(
  rows: readonly T[],
  recorded: readonly RecordedRow[],
  completeDates: ReadonlySet<string>,
): { rows: T[]; withheld: number; byDate: Record<string, number> } {
  ...
  const buckets = new Map<string, Array<{ row: RecordedRow; used: boolean }>>();
  for (const r of recorded) {
    if (!completeDates.has(r.transactionDate)) continue;
    const k = looseSweepKey(r);
    const bucket = buckets.get(k);
    if (bucket) bucket.push({ row: r, used: false });
    else buckets.set(k, [{ row: r, used: false }]);
  }

  /** Consume one unused stored row for `row`, exactly-matching the resource or not. */
  const take = (row: T, wantExact: boolean): boolean => {
    ...
    for (const entry of bucket) {
      ...
      if (wantExact ? !sameResource : entry.row.resourceTypeId != null) continue;
      entry.used = true;
      return true;
    }
    return false;
  ...
  const withheldAt = new Array<boolean>(rows.length).fill(false);
  ...
  for (let i = 0; i < rows.length; i++) {
    if (!completeDates.has(rows[i].transactionDate)) continue;
    if (take(rows[i], true)) drop(i);
    else pending.push(i);
  }
  ...
  for (const i of pending) {
    if (rows[i].resourceTypeId != null && take(rows[i], false)) drop(i);
  }
