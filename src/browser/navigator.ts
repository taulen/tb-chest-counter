...
import { keyPress, mouseClick } from './input.js';
...
const log = childLogger('navigator');

export { CalibrationMissingError };
...
export class MaintenanceModeError extends Error {
  ...
  constructor(message = 'Game is in maintenance mode', durationMs: number | null = null) {
    super(message);
 ...
 */
async function describeClickTarget(page: Page, x: number, y: number): Promise<string> {
 ...
 */
async function clickNavTarget(page: Page, x: number, y: number, label: string): Promise<string> {
...
async function promptUserToNavigate(instruction: string): Promise<void> {
  console.log('\n========================================');
 ...
 */
async function verifyGiftsViaCardCrop(
  screenshot: Buffer,
  vision: VisionProvider,
  cardCropPcts: { left: number; top: number; right: number; bottom: number },
): Promise<'gifts' | 'empty' | 'no_chest' | 'not_gifts' | null> {
  if (typeof vision.extractCardsFromCrop !== 'function') return null;
  try {
    const meta = await sharp(screenshot).metadata();
    const w = meta.width ?? DEFAULT_VIEWPORT_WIDTH;
    const h = meta.height ?? DEFAULT_VIEWPORT_HEIGHT;
    const region = {
    ...
    if (!(await isOCRBufferUsable(await cropRegion(screenshot, region)))) {
    ...
    // still on the tab.
    if (/no\s*gifts?/i.test(rawText)) return 'empty';
...
function describeCropResult(result: 'gifts' | 'empty' | 'no_chest'): string {
  if (result === 'empty') return 'no gifts';
  if (result === 'no_chest') return 'gifts present, none are chests';
  return 'cards present';
}

export async function ensureOnGiftsTab(
  page: Page,
  vision: VisionProvider,
  tab: 'gifts' | 'triumphal' = 'gifts',
  opts: { cardCropPcts?: { left: number; top: number; right: number; bottom: number } } = {},
): Promise<boolean> {
  ...
  const clanButton = requireUiPosition('clanButton');
  const giftsSidebar = requireUiPosition('giftsSidebar');
  if (tab === 'triumphal' && !isUiPositionSet('triumphalTab')) {
    throw new CalibrationMissingError(
      'triumphalTab',
      2,
      'Triumphal Gifts tab (top of My Clan dialog)',
    );
  }
  ...
  if (state === ScreenState.MAINTENANCE) {
  ...
  if (initialCropCheck === null
      && (state === ScreenState.GIFT_TAB || state === ScreenState.NO_GIFTS)) {
    if (tab === 'triumphal') {
      return await switchToTriumphalTab(page, vision);
    }
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    const afterClanShot = await captureForVision(page).catch(() => null);

    // Step 2: Click "Gifts" in the left sidebar
    let giftsTarget: string;
    if (vision.supportsImages) {
      const freshScreenshot = await captureForVision(page);
    ...
    const verifyScreenshot = await captureForVision(page);
    ...
    let verifyInput: Buffer;
    try {
      verifyInput = await cropPanelHeaderRegion(verifyScreenshot);
    } catch {
      verifyInput = verifyScreenshot;
    }
    verifyState = await vision.detectScreenState(verifyInput);

    if (verifyState === ScreenState.GIFT_TAB || verifyState === ScreenState.NO_GIFTS) {
      log.info('Successfully navigated to Gifts tab (verified)');
      return true;
    }

    if (verifyState === ScreenState.MAINTENANCE) {
      const durationMs = vision.getLastMaintenanceDurationMs?.() ?? null;
      log.warn(`Game is in maintenance mode - aborting scan${durationMs ? ` (~${Math.round(durationMs / 60_000)} min remaining)` : ''}`);
      throw new MaintenanceModeError('Game is in maintenance mode', durationMs);
    }
    ...
    try {
      if (afterClanShot) {
        await saveScreenshot(
          afterClanShot,
          './data/screenshots',
          `nav_verify_fail_attempt${attempt}_after_clan_click`,
          { force: true },
        );
      }
      const savedPath = await saveScreenshot(
        verifyScreenshot,
        './data/screenshots',
        `nav_verify_fail_attempt${attempt}`,
        { force: true },
      );
      log.info(`Saved debug screenshot: ${savedPath}`);
    } catch (err) {
      log.debug(`Failed to save debug screenshot: ${(err as Error).message}`);
    }
...
export async function switchToTriumphalTab(page: Page, vision: VisionProvider): Promise<boolean> {
  // Refuse if the operator hasn't marked the Triumphal tab during stage 2.
  const triumphalTab = requireUiPosition('triumphalTab');

  if (vision.supportsImages) {
    const resized = await captureForVision(page);
    const loc = await vision.findUIElement(resized, '"Triumphal Gifts" tab');
    if (loc) {
      await mouseClick(page, loc.x, loc.y);
  ...
  await mouseClick(page, coords.x, coords.y);
...
export async function clickGiftsTab(page: Page, vision: VisionProvider): Promise<boolean> {
  const giftsTab = requireUiPosition('giftsTab');

  if (vision.supportsImages) {
    const screenshot = await captureForVision(page);
    const loc = await vision.findUIElement(screenshot, '"Gifts" tab (not Triumphal)');
    if (loc) {
      await humanClick(page, loc.x, loc.y);
      await randomDelay(1500, 2500);
