...
import { DEFAULT_VIEWPORT } from '../config/viewport.js';
import type { AppConfig } from '../models/types.js';
import { GPU_ARGS, gpuChannel, isGpuEnabled, logWebglRenderer } from './gpu.js';
import { clearStaleProfileLocks } from './profile-lock.js';
import { gameDayTimezoneId, zoneOffsetMinutesBehindUtc } from '../config/game-timezone.js';
 ...
 */
async function applyCookiesResilient(context: BrowserContext, cookies: PlaywrightCookie[]): Promise<number> {
  try {
    await context.addCookies(cookies);
    return cookies.length;
  } catch (err) {
    log.warn(
      `Batch cookie hydration failed (${String(err)}); retrying cookie-by-cookie so one bad cookie can't drop the whole session.`,
    );
  }
...
async function hydrateContextFromStorageState(context: BrowserContext, storageStatePath: string): Promise<void> {
  ...
  try {
    ...
    parsed = JSON.parse(raw) as PlaywrightStorageState;
  } catch (err) {
  ...
  if (cookies.length === 0) {
    ...
    if (!hasSession) {
      ...
      log.warn(
        `Saved session has no ${TB_SESSION_COOKIE} cookie — the scanner will be treated as logged out. ` +
...
async function seedLocalStorageFromOrigins(
  context: BrowserContext,
  origins: NonNullable<PlaywrightStorageState['origins']>,
): Promise<void> {
  const withData = origins.filter((o) => Array.isArray(o.localStorage) && o.localStorage.length > 0);
  if (withData.length === 0) return;

  try {
    await context.addInitScript((seed: typeof withData) => {
      try {
        const here = window.location.origin;
        const match = seed.find((o) => o.origin === here);
        if (!match || !match.localStorage) return;
    ...
    log.debug(
      `Registered localStorage seed for ${withData.length} origin(s): ${withData.map((o) => o.origin).join(', ')}`,
    );
  } catch (err) {
...
export async function launchBrowser(
  ...
  options: {
    ...
    userDataDir?: string;
    /** Zone to run the browser in. Defaults to the one whose local midnight is
     *  the game reset — see ../config/game-timezone.ts. */
    timezoneId?: string;
  } = {},
  ...
  const userDataDir = options.userDataDir
    ...
    : path.resolve('data', 'browser-profile');
  ...
  const wantedTimezone = options.timezoneId?.trim()
    || gameDayTimezoneId(config.gameDayRolloverUtcHour);
  ...
  const launchOptions = {
    ...
    args: gpuEnabled ? [...baseArgs, ...GPU_ARGS] : baseArgs,
  };
  ...
  clearStaleProfileLocks(userDataDir, log);
  ...
  try {
    ...
    log.debug(
      `Browser render metrics: viewport=${viewport?.width ?? 'n/a'}x${viewport?.height ?? 'n/a'}, inner=${metrics.innerWidth}x${metrics.innerHeight}, outer=${metrics.outerWidth}x${metrics.outerHeight}, screen=${metrics.screenWidth}x${metrics.screenHeight}, dpr=${metrics.devicePixelRatio}`,
    );
    ...
    const expectedOffset = zoneOffsetMinutesBehindUtc(wantedTimezone);
    if (metrics.timezoneOffsetMin !== expectedOffset) {
      log.warn(
        { noAlert: true },
        `Browser timezone did not take: asked for ${wantedTimezone} (UTC`
  ...
  if (gpuEnabled) {
    const renderer = await logWebglRenderer(page, log);
    if (renderer === 'stalled' || renderer === 'software') {
      log.warn(
        `Scanner browser came up with WebGL "${renderer}" — every compositor flush and input ` +
