/**
 * Read the Clan Capital → History list from the live game.
 *
 * This is the automated replacement for the manual screenshot upload. It drives
 * the same navigation a human does — MAP → show-clan-capital → the capital →
 * History — then scrolls the list, OCRs each page through the existing
 * `processResourceScreenshot` pipeline, and stitches the pages back into one
 * ordered list of rows.
 *
 * Two things make this list different from the gifts sweep, and they shape
 * everything below:
 *
 *   - The data is PERSISTENT for about 14 days. A gift disappears the moment it's
 *     opened, so a missed gift is gone forever; a history row stays readable for
 *     roughly a fortnight. So a failed or wrong run costs a retry rather than the
 *     data, and every judgement call here prefers re-reading over guessing.
 *
 *     A rectangle with no rows in it does NOT mean the list has ended — and
 *     treating it as such cut a real sweep off at 503 rows / "2 DAYS AGO" while the
 *     game still had a fortnight to give. Two causes, both handled in the scroll
 *     loop's recovery, which Escapes, jiggles the cursor, waits, and looks again:
 *
 *       - The game's IDLE OVERLAY. On a long sweep it decides nobody is there and
 *         raises a purchase offer over the list. Wheel deltas alone do not count as
 *         activity, so the sweep moves the cursor on every page to keep proving
 *         otherwise — and the old code moved it to the SAME coordinates each time,
 *         which is a zero-delta move that emits no events at all.
 *       - The list LAZY-LOADING the next chunk after a scroll, where waiting is
 *         the right answer.
 *
 *     Only long full-history sweeps are really at risk; a daily incremental run
 *     stops at the cursor within a few pages, well before the idle timer fires.
 *
 *   - The same rows are visible on every run, so the run has to work out where
 *     it got to last time. It cannot do that from a single row: v39 dropped the
 *     UNIQUE key on resource_transactions because a player really can send the
 *     same amount of the same resource twice in one day. What it CAN rely on is
 *     that two ADJACENT rows are never identical — 0 exceptions in 5,873
 *     adjacencies across every real scan batch — so a run of several rows in
 *     order is a reliable marker. That's the cursor: an ordered fingerprint
 *     sequence, re-found by alignment rather than by row equality. See
 *     findCursorIndex.
 *
 *     This used to be justified as "the game merges same-day repeats into one
 *     line", and that is MEASURABLY FALSE as a general rule: on a settled day a
 *     player holds several separate lines for one resource in 33% of Scientific
 *     Tractates groups. Merging happens only on the day still being written to.
 *     The distinction is not pedantry — believing the strong version is what made
 *     it look safe to anchor the cursor on the top of the list.
 *
 *   - THE TOP OF THE LIST IS NOT STABLE, and everything below depends on it.
 *     Rows on the account-calendar day the game is still writing to DO merge: the
 *     amount grows through the day and the merged line moves back up the list.
 *     Measured across five deep re-reads of a production database — rows on a
 *     SETTLED day re-read byte-identical 250 times out of 253 with zero changed
 *     amounts, while 20% of the open day's rows had vanished from their slot by
 *     the next run. So this sweep NEVER writes a row from an open day and NEVER
 *     anchors the cursor on one: a game day is written exactly once, in full, by
 *     the first run after it closes. See buildCursorFingerprints.
 *
 *     What that replaced: the cursor was `stitched.slice(0, CURSOR_ROWS)`, so it
 *     pointed at the twelve most volatile rows in the list. On 2026-09-05 two of
 *     them were still findable, the sweep declared the marker lost, read 601 rows
 *     back to "4 DAYS AGO" and inserted every one of them again. Anchoring on the
 *     newest settled row instead re-found the marker in every measurable case at
 *     11-12 of 12. The same run also wrote the open day as a partial that the
 *     next run either froze (silent under-count) or re-inserted alongside its
 *     grown copy (silent double-count) — 809 duplicate and 317 stale rows in one
 *     clan before this changed.
 *
 * Deliberately DB-free, like might-capture.ts: it takes the roster and resource
 * types in and hands rows back out, so the phase that owns the transaction is
 * the only thing that writes. Navigation failures are reported in the result
 * rather than thrown, because the caller treats every outcome the same way — log
 * it and leave the chest scan alone.
 */
...
import { captureFullPage, saveScreenshot } from './screenshotter.js';
import { getCanvasRect, cropPctToPixels } from './member-capture.js';
import { getCanvasBounds } from './navigator.js';
import { humanClick, randomDelay } from '../utils/human-delay.js';
import { keyPress, mouseMove, mouseWheel } from './input.js';
import { childLogger } from '../utils/logger.js';
import {
  requireResourceHistoryCrop,
  requireUiPosition,
...
import type { ExtractedRow } from '../vision/resource-ocr.js';
...
export interface ResourceHistoryRow extends ExtractedRow {
  /** Fingerprint used for cross-page stitching and for the cursor. */
  fingerprint: string;
}

export interface ResourceHistoryCaptureResult {
  /**
   * Rows to write: above the cursor, below the open day, newest first.
   *
   * "Below the open day" is the part that is easy to miss and is the whole point.
   * Rows on the account-calendar day the game is still writing to are NEVER
   * returned — see the module header on why they cannot be trusted — so a game day
   * is written exactly once, in full, by the first run after it closes.
   */
  rows: ResourceHistoryRow[];
  /**
   * Fingerprints for the new cursor: CURSOR_ROWS rows starting at the newest row
   * the game has finished writing.
   *
   * Empty when the sweep never reached a settled row, when there were too few of
   * them to make a matchable marker, or when the sweep stopped somewhere that
   * leaves unread rows below the anchor (see ANCHOR_SAFE_STOP_REASONS). In every
   * one of those cases the caller must KEEP ITS EXISTING marker rather than store
   * this one.
   */
  cursorRows: string[];
  /** Account-calendar dates that were still open while this sweep ran. Normally
   *  one; two when the sweep crossed a rollover. */
  openDates: string[];
  /** Rows that were above the cursor but withheld because their day is still open.
   *  They are not lost — the next run reads them complete. */
  deferredRows: number;
  /** transaction_date of the row `cursorRows` starts at, or null if there is none. */
  cursorAnchorDate: string | null;
  /** Dates whose entire block this sweep read, so the caller may compare them
   *  against what it already holds. See readCompleteness. */
  completeDates: string[];
  /**
   * The transaction_date the previously-stored marker row reads as THIS run, or null
   * if that row was not located.
   *
   * The caller compares it against the date the same row was stored under. They must
   * be equal, and when they are not, no date-keyed comparison against stored rows is
   * valid this run.
   *
   * Why a row's date can move at all: it is `gameDate(run) - labelDaysAgo`, and those
   * two clocks are different. `gameDate` ticks at 17:00 UTC; the "N DAYS AGO" label
   * ticks at the game ACCOUNT's calendar midnight, which the game never exposes. So
   * the offset between them depends on WHEN the run happened, and a run on the other
   * side of the account midnight from the previous one dates every row it reads one
   * day differently. Scheduled runs all land in the same few hours and are stable;
   * an operator pressing "Collect now" at another time of day is not.
   */
  cursorMatchDate: string | null;
  pagesScanned: number;
  /** Rows read across the whole sweep, before the cursor cut. */
  totalRowsSeen: number;
  /** True when a cursor was supplied but never re-found. The sweep then keeps
   *  everything it read, which risks re-inserting rows the last run already
   *  took — the caller surfaces this rather than hiding it. */
  cursorLost: boolean;
  /** Player names that matched no roster member. Their rows are still returned. */
  unmatchedNames: string[];
  /**
   * Every row-crop file this sweep wrote, referenced or not.
   *
   * A row is read on ~4 consecutive pages, and a read that could not identify the
   * resource gets a crop each time. Most of those reads then pair with a clean one and
   * vanish into the stitch, so their crops end up pointing at nothing — ~200 PNGs a
   * sweep. Which ones is only knowable after the stitch, the cursor cut and the insert,
   * so the sweep reports what it wrote and the caller prunes what nothing kept. Do not
   * try to decide this per row while reading: the rows that stay unknown are exactly the
   * ones whose crop an admin needs, so skipping the write loses the evidence that
   * matters. See pruneUnreferencedCrops.
   */
  cropPathsWritten: string[];
  /** True when navigation never reached the history list. */
  navigationFailed: boolean;
  /** Human-readable reason, set whenever navigationFailed is true. */
  navigationError?: string;
  errors: string[];
  /** Hit MAX_PAGES with the list still advancing, so the read may be truncated. */
  truncated: boolean;
  /**
   * Why the scroll sweep stopped. These are NOT interchangeable, and conflating
   * them cost a diagnosis once already:
   *
   *   'cursor'      — reached the previous run's position. The good ending for a
   *                   daily incremental run.
   *   'date-floor'  — the sweep read further back than a run this recent should ever
   *                   need, without ever re-finding the cursor. A backstop, so it is
   *                   always a symptom: the cursor is what should have stopped it.
   *   'end-of-list' — no new rows AND the view stopped changing. The good ending for
   *                   a full sweep: the list does not blank when it runs out, it
   *                   stops scrolling with the last rows still on screen.
   *   'no-new-rows' — the view is still CHANGING but adds nothing new. Suspicious:
   *                   the list is scrolling yet every row is already held, which
   *                   points at the stitch having stopped aligning.
   *   'blank'       — several pages running contained no history rows at all, and
   *                   Escaping/waiting never brought them back. An overlay that
   *                   would not close, or a panel that stopped rendering.
   *   'page-limit'  — hit maxPages with the list still advancing.
   *   'crashed'     — the browser renderer died mid-sweep. Rows read up to that point
   *                   are kept and written; the read is partial.
   *   'error'       — the crop could not be taken.
   */
  stopReason:
    | 'cursor'
    | 'date-floor'
    ...
    | 'crashed'
    | 'error';
  /** Path to a saved PNG of the final page, when the sweep ended on a condition
   *  worth eyeballing. The only way to tell 'blank' apart from a closed panel. */
  finalPageCropPath?: string;
  /** Oldest date label the sweep reached ('TODAY', '5 DAYS AGO', …). Reveals how
   *  much history the game actually served, which is not a fixed number of days. */
  oldestDateLabel: string;
  /**
   * Directory holding this run's debug frames, when debug saving was on.
   *
   * One directory per run so the whole run is a single self-contained thing to
   * copy off the box (`scp -r`) or attach somewhere — interleaved files in a shared
   * directory make "the frames from that run" a filtering exercise. Logged
 ...
 */
function rowsAlign(a: ResourceHistoryRow, b: ResourceHistoryRow): boolean {
 ...
 */
function rowsAlignIgnoringDate(a: ResourceHistoryRow, b: ResourceHistoryRow): boolean {
  if (`${looseRowKey(a)}|${a.resourceTypeId ?? 'x'}` === `${looseRowKey(b)}|${b.resourceTypeId ?? 'x'}`) return true;
  const oneIsUnresolved = a.resourceTypeId == null || b.resourceTypeId == null;
  return oneIsUnresolved && looseRowKey(a) === looseRowKey(b);
}
 ...
 */
function requiredMatches(overlap: number): number {
  if (overlap <= 4) return overlap;
  return Math.max(4, Math.ceil(overlap * 0.6));
}

/**
 * Append `page`'s rows onto `acc`, dropping the overlap between them.
 *
 * Scrolling advances by less than a full page on purpose, so consecutive pages
 * share rows. The overlap is found by looking for the longest suffix of `acc`
 * that is also a prefix of `page` — the classic sequence stitch, which is sound
 * here for the same reason the cursor is: adjacent rows are never identical, so
 * a run of matching rows in order is the real overlap and not a coincidence.
 *
 * Falls back to appending everything when no overlap is found. That's the right
 * direction to fail: a duplicate row inside one sweep is caught by the
 * de-duplication below, whereas dropping rows would lose data silently.
 *
 * Two passes: the strict rule, then the date-tolerant one for a long window. See
 * rowsAlign and rowsAlignIgnoringDate.
 */


type AlignRule = (a: ResourceHistoryRow, b: ResourceHistoryRow) => boolean;
...
function findOverlap(
  ...
  for (let overlap = maxOverlap; overlap >= minOverlap; overlap--) {
    ...
    for (let i = 0; i < overlap; i++) {
      if (align(acc[acc.length - overlap + i], page[i])) matched++;
    }
    if (matched >= requiredMatches(overlap)) return overlap;
  }
  return null;
}
...
export function stitchPage(
  acc: ResourceHistoryRow[],
  page: ResourceHistoryRow[],
): { rows: ResourceHistoryRow[]; overlap: number } {
  if (acc.length === 0) return { rows: [...page], overlap: -1 };
  if (page.length === 0) return { rows: acc, overlap: -1 };
  ...
  for (let i = 0; i < overlap; i++) {
    ...
    if (!align(merged[at], page[i])) continue;
 ...
 */
export async function pruneUnreferencedCrops(
  written: readonly string[],
  keptRows: ReadonlyArray<{ rowCropPath?: string | null }>,
): Promise<number> {
  if (written.length === 0) return 0;
  const kept = new Set(
    keptRows.map((r) => r.rowCropPath).filter((p): p is string => !!p),
  );
  let removed = 0;
 ...
 */
export function dedupeNearbyRepeats(
  rows: ResourceHistoryRow[],
  windowRows: number = DEDUPE_ADJACENT_ONLY,
): { rows: ResourceHistoryRow[]; dropped: number } {
  const out: ResourceHistoryRow[] = [];
  ...
  for (const row of rows) {
    ...
    if (at !== undefined && out.length - at <= windowRows) {
      ...
      if (out[at].resourceTypeId == null && row.resourceTypeId != null) out[at] = row;
      dropped++;
      continue;
    }
 ...
 */
export function collapseUnresolvedTwins(
  rows: ResourceHistoryRow[],
  maxPerPage: Map<string, number>,
): { rows: ResourceHistoryRow[]; dropped: number } {
  const byKey = new Map<string, number[]>();
  rows.forEach((r, i) => {
    const key = looseSweepKey(r);
  ...
  for (const [key, indexes] of byKey) {
    ...
    if (indexes.length <= allowed) continue;
    ...
    const unresolved = indexes.filter((i) => {
      if (rows[i].resourceTypeId != null) return false;
      return indexes.some((j) => j !== i && Math.abs(j - i) <= DEDUPE_ADJACENT_ONLY);
    });
    const resolvedCount = indexes.length - indexes.filter(
      (i) => rows[i].resourceTypeId == null,
    ).length;
    const keepUnresolved = Math.max(0, allowed - resolvedCount);
    for (const i of unresolved.slice(keepUnresolved)) drop.add(i);
  }
...
interface CursorCandidate {
  /** Index in the list the stored cursor's head aligns to. */
  at: number;
  /** Marker rows of the window that aligned. */
  matched: number;
  /** Rows compared — the shorter of the cursor tail and what is left of the list. */
  window: number;
  /** Offset into the stored cursor this candidate started from. */
  start: number;
}
 ...
 */
function scanCursorCandidates(
  prints: string[],
  cursor: string[],
): { passing: CursorCandidate | null; best: CursorCandidate | null } {
  let passing: CursorCandidate | null = null;
  let best: CursorCandidate | null = null;

  for (let start = 0; start + CURSOR_MIN_RUN <= cursor.length; start++) {
    const needle = cursor.slice(start);
    for (let at = 0; at + CURSOR_MIN_RUN <= prints.length; at++) {
      const window = Math.min(needle.length, prints.length - at);
      let matched = 0;
      for (let i = 0; i < window; i++) {
        if (fingerprintsAlign(prints[at + i], needle[i])) matched++;
      }
      if (best == null || matched > best.matched) best = { at, matched, window, start };
...
export function findCursorIndex(
  ...
  if (cursor.length === 0 || rows.length === 0) return -1;
  ...
  if (!opts.quiet) {
    log.info(
      `Resource capture: re-found the previous cursor at row ${passing.at} — `
 ...
 */
export function describeCursorNearMiss(rows: ResourceHistoryRow[], cursor: string[]): string {
  if (cursor.length === 0 || rows.length === 0) return 'nothing to compare';
  const prints = rows.map((r) => r.fingerprint);
  const { best } = scanCursorCandidates(prints, cursor);
  if (!best) return 'no window long enough to compare';
  ...
  for (const want of needle) {
    const hit = pool.find((c) => !c.used && fingerprintsAlign(c.p, want));
    if (hit) { hit.used = true; setScore++; }
  }
 ...
 */
export async function isOnWorldMap(screenshot: Buffer): Promise<boolean> {
...
function looksLikeHistory(text: string): boolean {
  return (
    /\b(TODAY|YESTERDAY|DAYS?\s*AGO)\b/i.test(text) ||
 ...
 */
async function navigateToCapitalHistory(
  page: Page,
  reportProgress?: (message: string) => void,
  debugDir?: string,
): Promise<string | null> {
  const mapButton = requireUiPosition('worldMapButton');
  const capitalButton = requireUiPosition('clanCapitalButton');
  const capitalMarker = requireUiPosition('clanCapitalMarker');
  const historySidebar = requireUiPosition('capitalHistorySidebar');
  const cropPct = requireResourceHistoryCrop();
  ...
  const clickAt = async (
    ...
    await randomDelay(settleMin, settleMax);
  };
  ...
  let onMap = await isOnWorldMap(await captureFullPage(page));
  for (let attempt = 0; attempt < 2 && !onMap; attempt++) {
    ...
    onMap = await isOnWorldMap(await captureFullPage(page));
  }
  if (!onMap) {
  ...
  const cropPx = cropPctToPixels(cropPct, canvasRect, screenshotSize);
  ...
  if (!looksLikeHistory(text)) {
    ...
    await saveScreenshot(
      screenshot,
      debugDir ?? './data/screenshots',
      debugDir ? 'nav-fail' : 'resource_nav_fail',
      { force: true },
    ).catch(() => '');
 ...
 */
export function advanceDateLabel(label: string): string {
  const normalized = label.trim().toUpperCase();
  if (normalized === 'TODAY') return 'YESTERDAY';
  if (normalized === 'YESTERDAY') return '2 DAYS AGO';
  const daysAgo = normalized.match(/^(\d+)\s+DAYS?\s*AGO$/);
  if (daysAgo) return `${Number.parseInt(daysAgo[1], 10) + 1} DAYS AGO`;
  return label;
}
 ...
 */
export function daysAgoFromLabel(label: string): number | null {
  const normalized = label.trim().toUpperCase();
  if (normalized === 'TODAY') return 0;
  if (normalized === 'YESTERDAY') return 1;
  const daysAgo = normalized.match(/^(\d+)\s+DAYS?\s*AGO$/);
  if (daysAgo) return Number.parseInt(daysAgo[1], 10);
  return null;
}

export interface ResourceHistoryCaptureOptions {
  members: ClanMember[];
  allTypes: ResourceType[];
  /**
   * Game-day rollover hour, from which the date that "TODAY" means is derived.
   *
   * Taken as the rollover rather than as a finished date because the sweep has
   * to re-derive it PER PAGE: a full backfill can run for over an hour, and if
   * the day turns over mid-sweep the game silently re-labels every row it is
   * still showing. See the re-anchor in the scroll loop.
   */
  rolloverUtcHour: number;
  /** Ordered fingerprints from the previous successful run, newest first. Empty
   *  or absent means "no cursor" — read the whole visible list (first run, or a
   *  deliberate full backfill). */
  cursor?: string[];
  /**
   * The stored marker, for the DATE-MAPPING check only — never as a stop condition.
   *
   * Passed even when `cursor` is deliberately empty (a full backfill or a dry run),
   * because the question it answers is not "where did the last run get to" but "does
   * a row still read back with the date it was stored under". See cursorMatchDate.
   */
  verifyCursor?: string[];
  /**
   * Stop once the sweep is reading rows labelled further back than this many days,
   * having not re-found the cursor.
   *
   * A backstop BEHIND the cursor, not a replacement for it — see the check in the
   * scroll loop for why one is needed and how to size it. Ignored unless a cursor was
   * supplied, so a first run, a full backfill and a dry run still read everything the
   * game will serve.
   */
  maxDaysBack?: number;
  /** Unique-per-run token for unresolved-row crop filenames. */
  cropToken?: string;
  /**
   * Stop after this many pages even if the list is still advancing. Used by the
   * admin "Collect now" button to keep a debugging round-trip short; the
   * scheduled run leaves it at MAX_PAGES.
   */
  maxPages?: number;
  reportProgress?: (message: string) => void;
  /**
   * Save a PNG of EVERY scroll page's crop, not just the first and last.
   *
   * On by default for the interactive "Collect now" run, off for the scheduled
   * one. This feature can only be judged against the live game, and the questions
   * that come up are all about specific frames — did the rectangle clip the
   * resource icon, what did the list look like where the sweep stopped, was that
   * page mid-fetch. A description of a sweep cannot answer those; the pixels can.
   *
   * A full backfill is a few hundred files of ~100KB, which the existing
   * screenshot retention sweep ages out.
   */
 ...
 * the cursor.
 */
export async function captureResourceHistory(
  page: Page,
  opts: ResourceHistoryCaptureOptions,
): Promise<ResourceHistoryCaptureResult> {
  const {
    members, allTypes, rolloverUtcHour, cursor = [], verifyCursor = [], cropToken, reportProgress,
    debugSavePages = false, maxDaysBack,
  } = opts;
  /** The date the list currently calls "TODAY". Re-read every page. */
  const listDateNow = () => gameDateFor(Date.now(), rolloverUtcHour);
  const maxPages = Math.max(1, Math.min(MAX_PAGES, opts.maxPages ?? MAX_PAGES));

  const result: ResourceHistoryCaptureResult = {
    rows: [],
    cursorRows: [],
    openDates: [],
    deferredRows: 0,
  ...
  if (debugSavePages) {
    ...
    debugDir = pathJoin('data', 'screenshots', 'resource-debug', `run_${stamp}`);
    try {
      ...
      const parent = pathJoin('data', 'screenshots', 'resource-debug');
      for (const name of await fsPromises.readdir(parent)) {
        const dir = pathJoin(parent, name);
        if (dir === debugDir) continue;
        try {
      ...
      log.info(
        `Resource capture: saving debug frames to ${debugDir} — one crop per scroll page (what OCR `
  ...
  let stitched: ResourceHistoryRow[] = [];
  ...
  let lastCropped: Buffer | null = null;
   ...
   */
  const maxRowsPerPageByKey = new Map<string, number>();
  /**
   ...
   */
  let anchorRow: ResourceHistoryRow | null = null;

  for (let pageNum = 0; pageNum < maxPages; pageNum++) {
    const rowsBefore = stitched.length;
    let screenshot: Buffer;
    try {
      screenshot = await captureFullPage(page);
    } catch (err) {
      ...
      log.warn(
        { noAlert: true },
        `Resource capture: the browser crashed while screenshotting page ${pageNum + 1} — keeping `
    ...
    if (debugDir) {
      ...
      await saveScreenshot(cropped, debugDir, `${pageLabel}-crop`, { force: true })
        .catch(() => '');
      if (pageNum === 0 || (pageNum + 1) % FULL_SHOT_EVERY === 0) {
        await saveScreenshot(screenshot, debugDir, `${pageLabel}-full`, { force: true })
          .catch(() => '');
      }
    }
    ...
    if (pageDate !== baseDate.toISOString().slice(0, 10)) {
      ...
      log.info(
        `Resource capture: the day rolled over mid-sweep at page ${pageNum + 1} — "today" is now `
    ...
    if (pageNum === 0) {
      await saveScreenshot(
        cropped,
        debugDir ?? './data/screenshots',
        debugDir ? 'p001-crop-firstpage' : 'resource_history_crop_debug',
        { force: true },
      ).catch(() => '');
    }
    ...
    let pageResult;
    try {
      pageResult = await processResourceScreenshot({
        imageBuffer: cropped,
    ...
    if (pageResult) {
      ...
      for (const name of pageResult.unmatchedNames) {
        if (!result.unmatchedNames.includes(name)) result.unmatchedNames.push(name);
      }

      const pageRows: ResourceHistoryRow[] = pageResult.rows.map((row) => ({
        ...row,
        fingerprint: rowFingerprint(row),
      }));

      for (const row of pageRows) {
        if (row.rowCropPath) result.cropPathsWritten.push(row.rowCropPath);
      }

      const seenThisPage = new Map<string, number>();
      for (const row of pageRows) {
        const key = looseSweepKey(row);
        seenThisPage.set(key, (seenThisPage.get(key) ?? 0) + 1);
      }
      for (const [key, count] of seenThisPage) {
        if (count > (maxRowsPerPageByKey.get(key) ?? 0)) maxRowsPerPageByKey.set(key, count);
      }
      ...
      if (stitch.overlap === 0) {
        ...
        log.warn(
          { noAlert: true },
          `Resource capture page ${pageNum + 1} shares no rows with the previous page `
      ...
      const samples = pageRows.slice(0, 3)
        ...
        .join(', ');
      log.info(
        `Resource history page ${pageNum + 1}: ${pageRows.length} row(s) read, `
      ...
      reportProgress?.(
        `Resources: ${stitched.length.toLocaleString('en-US')} row(s), `
      ...
      if (cursor.length > 0 && cursorIndex < 0) {
        ...
        if (cursorIndex >= 0) {
          ...
          log.info(
            `Resource capture: reached the previous run's position after ${pageNum + 1} page(s); `
            + 'stopping the scroll.',
          );
      ...
      if (maxDaysBack != null && cursor.length > 0) {
        ...
        if (daysBack != null && daysBack > maxDaysBack) {
          ...
          if (lastCropped) {
            result.finalPageCropPath = await saveScreenshot(
              lastCropped,
              debugDir ?? './data/screenshots',
              `${debugDir ? '' : 'resource_history_'}final-page`,
              { force: true },
            ).catch(() => '') || undefined;
          }
          log.warn(
            `Resource capture: stopped at the date backstop after ${pageNum + 1} page(s). The sweep `
    ...
    if (rowsThisPage === 0) {
      ...
      if (lastCropped && blankEpisodeShots < 5) {
        ...
        await saveScreenshot(
          lastCropped,
          debugDir ?? './data/screenshots',
          `${debugDir ? '' : 'resource_history_'}blank-p${String(pageNum + 1).padStart(3, '0')}`,
          { force: true },
        ).catch(() => '');
        ...
        await saveScreenshot(
          screenshot,
          debugDir ?? './data/screenshots',
          `${debugDir ? '' : 'resource_history_'}blank-p${String(pageNum + 1).padStart(3, '0')}-full`,
          { force: true },
        ).catch(() => '');
      }
      let recovered = false;
      for (let retry = 1; retry <= BLANK_RETRIES; retry++) {
        log.info(
          `Resource capture: no history rows in the rectangle (page ${pageNum + 1}); dismissing any `
          + `overlay and waiting for rows to come back (${retry}/${BLANK_RETRIES})…`,
        );
        reportProgress?.(
          `Resources: clearing a popup / waiting for more history (${retry}/${BLANK_RETRIES})`,
        );
        ...
        await mouseMove(page, scrollAt.x - 10, scrollAt.y - 6, { steps: 4 }).catch(() => {});
        await mouseMove(page, scrollAt.x, scrollAt.y, { steps: 3 }).catch(() => {});

        await new Promise((r) => setTimeout(r, BLANK_RETRY_WAIT_MS));

        let retryCrop: Buffer;
        try {
          retryCrop = await sharp(await captureFullPage(page)).extract(cropPx).png().toBuffer();
        } catch {
          break;
        }
        ...
        if (debugDir) {
          await saveScreenshot(
            retryCrop,
            debugDir,
            `p${String(pageNum + 1).padStart(3, '0')}-wait${retry}-crop`,
            { force: true },
          ).catch(() => '');
        }
        ...
        if (looksLikeHistory(retryText)) {
          log.info(
            `Resource capture: rows re-appeared after ${retry} attempt(s) — an overlay was in the `
      ...
      log.info(
        `Resource capture: still no history rows after ${BLANK_RETRIES} attempt(s) at dismissing and `
    ...
    if (stitched.length === rowsBefore) {
      ...
      if (dryPages >= DRY_PAGES_TO_STOP) {
        ...
        if (cropUnchanged) {
          ...
          log.info(
            `Resource capture reached the end of the list: ${dryPages} page(s) with no new rows and `
          ...
          log.warn(
            { noAlert: true },
            `Resource capture stopped after ${dryPages} page(s) that still showed ${rowsThisPage} `
    ...
    if (sameCount >= DRY_PAGES_TO_STOP) {
      result.stopReason = 'end-of-list';
      log.info(
        `Resource capture: the view stopped changing after ${pageNum + 1} page(s) at `
    ...
    try {
      await mouseMove(page, jiggleX, jiggleY, { steps: 4 });
      await mouseMove(page, scrollAt.x, scrollAt.y, { steps: 3 });
      ...
      log.warn(
        { noAlert: true },
        `Resource capture: the browser crashed after ${pageNum + 1} page(s) — keeping the `
  ...
  if (collapsed.dropped > 0) {
    ...
    log.info(
      `Resource capture: dropped ${collapsed.dropped} unresolved row(s) that were a second, `
  ...
  if (result.truncated) {
    log.warn(
      `Resource capture hit the ${maxPages}-page limit with ${stitched.length} row(s) read `
   ...
   */
  const anchor = buildCursorFingerprints(stitched, openDates);
  ...
  const anchorSafe = ANCHOR_SAFE_STOP_REASONS.has(result.stopReason);
  result.cursorRows = anchorSafe ? anchor.fingerprints : [];
  if (!anchorSafe && anchor.fingerprints.length > 0) {
    log.warn(
      { noAlert: true },
      'Resource capture: not moving the marker forward — the sweep ended on '
  ...
  result.completeDates = [...readCompleteness(stitched, cut, result.stopReason).completeDates];

  if (result.deferredRows > 0) {
    log.info(
      `Resource capture: holding back ${result.deferredRows} row(s) dated `
  ...
  if (cursor.length > 0 && cursorIndex < 0) {
    ...
    log.warn(
      `Resource capture: could not re-find the previous run's position among ${stitched.length} `
  ...
  log.info(
    `Resource capture finished: ${result.pagesScanned} page(s), ${stitched.length} row(s) read, `
