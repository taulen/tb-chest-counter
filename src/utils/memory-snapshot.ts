...
function readKeyedValue(file: string, key: string): number | null {
  try {
...
export interface MemorySnapshot {
  /** Resident set of THIS Node process. Excludes the browser's processes. */
  rssBytes: number;
  /** Total charged to the container's cgroup — Node + every Chromium process. */
  cgroupCurrentBytes: number | null;
  /** The container's hard ceiling; Infinity when unlimited. */
  cgroupMaxBytes: number | null;
  /**
   * Times the kernel OOM-killed a process in this cgroup since boot. Non-zero
   * next to a "Target crashed" is the diagnosis: the container is over its
   * memory limit and the renderer is what got sacrificed.
   */
...
export function memorySnapshot(): MemorySnapshot {
  ...
  if (cgroupCurrentBytes !== null) {
    const ceiling =
      cgroupMaxBytes === null || cgroupMaxBytes === Infinity ? 'unlimited' : mb(cgroupMaxBytes);
    parts.push(`container=${mb(cgroupCurrentBytes)}/${ceiling}`);
  }
  if (oomKills !== null) parts.push(`oom_kills=${oomKills}`);
