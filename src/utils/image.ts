...
export async function resizeForVision(buffer: Buffer): Promise<Buffer> {
  ...
  const width = metadata.width ?? DEFAULT_VIEWPORT_WIDTH;
  const height = metadata.height ?? DEFAULT_VIEWPORT_HEIGHT;
...
export async function annotateScreenshot(
  ...
  const width = meta.width ?? DEFAULT_VIEWPORT_WIDTH;
  const height = meta.height ?? DEFAULT_VIEWPORT_HEIGHT;
 ...
 */
export async function isOCRBufferUsable(buffer: Buffer): Promise<boolean> {
  const meta = await sharp(buffer).metadata();
 ...
 */
export async function preprocessSmallTextRows(buffer: Buffer): Promise<Buffer> {
  const meta = await sharp(buffer).metadata();
  const width = meta.width ?? 0;
  return sharp(buffer)
    .grayscale()
 ...
 */
export async function cropPanelHeaderRegion(buffer: Buffer): Promise<Buffer> {
  const meta = await sharp(buffer).metadata();
