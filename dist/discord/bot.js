...
exports.startAllClanBots = startAllClanBots;
exports.stopClanBot = stopClanBot;
exports.startClanBot = startClanBot;
exports.reloadClanBot = reloadClanBot;
exports.stopAllBots = stopAllBots;
exports.postScanReport = postScanReport;
exports.sendClanDigestDmTest = sendClanDigestDmTest;
exports.isClanBotConnected = isClanBotConnected;
...
function findClanByChannel(channelId) {
    if (!channelId)
        return undefined;
...
async function ensureClient(token) {
    ...
    const shared = { client, token, ready: false, clanIds: new Set() };
    clients.set(token, shared);
    client.on('clientReady', async () => {
        shared.ready = true;
        const usingThisToken = Array.from(clans.values()).filter((c) => c.token === token);
        log.info(`Discord client ${maskToken(token)} logged in as ${client.user?.tag} (clans: ${usingThisToken.map((c) => `#${c.clanId}`).join(', ') || 'none yet'})`);
    ...
    client.on('interactionCreate', async (interaction) => {
        ...
        if (interaction.isAutocomplete()) {
            ...
            try {
                ...
                const choices = focused.name === 'player'
                    ? (0, commands_js_1.autocompleteMembers)(ctx.clanId, String(focused.value ?? ''))
                    : [];
        ...
        if (!ctx || ctx.token !== token) {
            ...
            log.info(`Slash command in channel ${interaction.channelId} matched no configured clan channel ` +
                `(configured: ${Array.from(clans.values()).map((c) => `#${c.clanId}=${c.channelId || '<unset>'}`).join(', ') || 'none'})`);
            await interaction
                .reply({
                content: "This channel isn't the configured channel for any clan. Slash commands only work in the exact channel set in the clan's Discord settings — note that a thread counts as a different channel from its parent.",
        ...
        if (!ctx.commandsEnabled) {
            await interaction
                .reply({ content: 'Slash commands are disabled for this clan.', flags: discord_js_1.MessageFlags.Ephemeral })
                .catch(() => { });
            return;
        }
        try {
            await (0, commands_js_1.handleCommand)(interaction, ctx.channelId, ctx.clanId);
        }
        catch (err) {
 ...
 * from Discord's side.
 */
async function registerCommandsForGuild(token, guildId) {
    const shared = clients.get(token);
    if (!shared || !shared.ready || !shared.client.user)
        return;
    const applicationId = shared.client.user.id;
    const rest = new discord_js_1.REST().setToken(token);
    const anyEnabled = Array.from(clans.values()).some((c) => c.token === token && c.guildId === guildId && c.commandsEnabled);
    const body = anyEnabled ? commands_js_1.commands.map((c) => c.toJSON()) : [];
    if (guildId) {
        await rest.put(discord_js_1.Routes.applicationGuildCommands(applicationId, guildId), { body });
        try {
            await rest.put(discord_js_1.Routes.applicationCommands(applicationId), { body: [] });
        }
        catch (cleanupErr) {
            log.warn(`Could not clear global command scope for ${maskToken(token)}: ${String(cleanupErr)}`);
        }
        log.debug(`Token ${maskToken(token)}: slash commands ${anyEnabled ? 'registered' : 'cleared'} for guild ${guildId}`);
    }
    else {
        await rest.put(discord_js_1.Routes.applicationCommands(applicationId), { body });
        log.debug(`Token ${maskToken(token)}: slash commands ${anyEnabled ? 'registered globally' : 'cleared globally'}`);
    }
}
async function startBotForClan(clan, gameDayRolloverUtcHour) {
 ...
 */
async function startAllClanBots(gameDayRolloverUtcHour) {
    for (const clan of (0, clan_repo_js_1.listClans)()) {
        try {
 ...
 */
async function stopClanBot(clanId) {
    ...
    if (shared.clanIds.size === 0) {
        ...
        log.info(`Clan #${clanId} Discord bot stopped (token ${maskToken(ctx.token)} still used by clans: ${Array.from(shared.clanIds).map((id) => `#${id}`).join(', ')})`);
    }
}
/**
 ...
 */
async function startClanBot(clanId, gameDayRolloverUtcHour) {
 ...
 */
async function reloadClanBot(clanId, gameDayRolloverUtcHour) {
    const clan = (0, clan_repo_js_1.getClanById)(clanId);
    ...
    if (existing && existing.token === clan.discordToken) {
        ...
        existing.digestShareUserIds = (0, digest_recipients_js_1.parseDigestRecipients)(clan.discordDailyDigestShareUserId);
 ...
 * this is a no-op.
 */
async function postScanReport(clanId, result) {
    const ctx = clans.get(clanId);
    if (!ctx || !ctx.channelId)
        return;
    if (!ctx.scanReportsEnabled)
        return;
    if (ctx.onlyPostWithNewChests && result.newChests === 0)
        return;
    const shared = clients.get(ctx.token);
    if (!shared || !shared.ready)
        return;
    try {
        const channel = (await shared.client.channels.fetch(ctx.channelId));
        if (channel) {
            const message = (0, embeds_js_1.formatScanReportMessage)(result, clanId);
 ...
 * else their digest.
 */
async function dmDigestToRecipients(shared, ctx, text) {
    const outcomes = [];
 ...
 * manual re-send.
 */
async function sendClanDigestDmTest(clanId) {
    const ctx = clans.get(clanId);
    if (!ctx) {
    ...
    const text = `[Test DM]\n${(0, embeds_js_1.formatDailyDigestShareText)(data, { gameDayDate: gameDayKey })}`;
 ...
 */
function isClanBotConnected(clanId) {
    const ctx = clans.get(clanId);
    if (!ctx)
        return false;
    const shared = clients.get(ctx.token);
 ...
 */
async function postReauthRequiredNotice(clanId) {
    const ctx = clans.get(clanId);
    if (!ctx || !ctx.channelId)
        return;
    const shared = clients.get(ctx.token);
    ...
    try {
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setDescription(`The saved login session for **${ctx.clanName}** has expired and scans will fail until an admin re-authenticates.\n\n` +
 ...
 */
async function postReauthResolvedNotice(clanId) {
    const ctx = clans.get(clanId);
    if (!ctx || !ctx.channelId)
        return;
    const shared = clients.get(ctx.token);
    ...
    try {
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setDescription(`The saved login session for **${ctx.clanName}** loaded the game successfully, so no ` +
 ...
 */
async function sendClanTestMessage(clanId) {
    const ctx = clans.get(clanId);
    if (!ctx) {
        return { ok: false, error: 'Discord bot is not running for this clan. Enable it and save first.' };
    }
...
function scheduleNextDigest(clanId) {
    const ctx = clans.get(clanId);
    if (!ctx)
        return;
    if (ctx.digestTimer) {
    ...
    const now = new Date();
    const target = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), ctx.digestUtcHour, 0, 0, 0));
 ...
 */
function lastRolloverInstant(now, rolloverHour) {
    const target = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), rolloverHour, 0, 0, 0));
    if (target.getTime() > now.getTime()) {
        target.setUTCDate(target.getUTCDate() - 1);
    }
    return target;
 ...
 */
async function catchUpMissedDigest(clanId) {
    const ctx = clans.get(clanId);
    ...
    if (clan.lastDigestGameDay === gameDayKey) {
        log.info(`Clan #${clanId}: digest catch-up not needed — game day ${gameDayKey} already sent (at ${clan.lastDigestAt || 'unknown'})`);
        return;
    }
    ...
    if (!clan.lastDigestAt) {
        log.info(`Clan #${clanId}: digest catch-up not needed — no prior digest recorded; leaving the first run to the scheduler (would be game day ${gameDayKey})`);
        return;
    }
    ...
    if (new Date(clan.lastDigestAt).getTime() >= rollover.getTime()) {
        log.info(`Clan #${clanId}: digest catch-up not needed — last digest (${clan.lastDigestAt}) already covers rollover ${rollover.toISOString()}`);
        return;
    }
 ...
 */
function resolveCompletedGameDay(now, rolloverHour) {
...
async function sendDailyDigestForClan(clanId, windowRef) {
    ...
    try {
        ...
        const message = (0, embeds_js_1.formatDailyDigestMessage)(data, {
            webExternalUrl: config.webExternalUrl || undefined,
            gameDayDate: gameDayKey,
        });
        await channel.send({ content: message });
        log.info(`Clan #${clanId} digest posted: ${data.totalChests} chests, ${data.totalPoints} pts, ${data.activePlayers} active players, ${data.scanCount} scans`);
    }
    catch (err) {
    ...
    if (ctx.digestShareUserIds.length > 0) {
        const text = (0, embeds_js_1.formatDailyDigestShareText)(data, { gameDayDate: gameDayKey });
        const outcomes = await dmDigestToRecipients(shared, ctx, text);
        const sent = outcomes.filter((o) => !o.error);
        const failed = outcomes.filter((o) => o.error);
        if (sent.length > 0) {
            log.info(`Clan #${clanId} digest shared via DM to ${sent.length}/${outcomes.length} recipient(s): ${sent.map((o) => o.label).join(', ')}`);
        }
        if (failed.length > 0) {
            ...
            log.warn(`Clan #${clanId}: failed to DM digest to ${failed.length}/${outcomes.length} recipient(s) — ${dmError}`);
        }
    }
    try {
        (0, clan_repo_js_1.setClanLastDigestStatus)(clanId, {
            at: now.toISOString(),
            gameDay: gameDayKey,
            channelError,
            dmError,
