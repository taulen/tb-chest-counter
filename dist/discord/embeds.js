...
exports.formatScanReportMessage = formatScanReportMessage;
exports.formatLeaderboardMessage = formatLeaderboardMessage;
exports.createStatsEmbed = createStatsEmbed;
exports.formatDailyDigestMessage = formatDailyDigestMessage;
exports.formatDailyDigestShareText = formatDailyDigestShareText;
 ...
 */
function formatRankCell(rank, width) {
    const medals = ['🥇', '🥈', '🥉'];
    if (rank >= 1 && rank <= 3) {
...
function formatScanReportMessage(result, clanId) {
    ...
    const table = renderLeaderboardTable(sorted.map((p, i) => ({ rank: i + 1, name: p.name, points: p.points, chests: p.chests })));
    return `${header}\n${summary}\n${table}`;
}
function formatLeaderboardMessage(leaderboard, period) {
    ...
    const table = renderLeaderboardTable(top.map((e) => ({
        rank: e.rank,
        name: e.memberName,
...
function createStatsEmbed(stats) {
    ...
    { name: 'Total Scans', value: stats.totalSessions.toLocaleString(), inline: true }, { name: 'Total Chests', value: stats.totalChests.toLocaleString(), inline: true }, { name: 'Members', value: stats.totalMembers.toLocaleString(), inline: true }, 
    ...
    // onto two rows.
    { name: 'Avg Chests / Scan', value: stats.avgChestsPerScan.toLocaleString(), inline: false }, { name: 'Last Scan', value: formatLastScanValue(stats.lastScanAt, stats.lastScanChests), inline: false })
        .setTimestamp();
}
/**
 ...
 */
function formatDiscordRelative(iso) {
    if (!iso)
        return 'Never';
 ...
 */
function formatLastScanValue(iso, chestCount) {
    const when = formatDiscordRelative(iso);
    if (chestCount === null || chestCount === undefined)
        return when;
 ...
 */
function formatDailyDigestMessage(data, options = {}) {
    ...
    const gameDayDate = options.gameDayDate ?? new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    ...
    lines.push(`**${data.totalChests.toLocaleString()}** chests · **${data.totalPoints.toLocaleString()}** pts · ` +
    ...
    const table = renderLeaderboardTable(data.topContributors.map((p, i) => ({
        rank: i + 1,
        name: p.memberName,
        points: p.points,
        chests: p.chests,
 ...
 */
function formatGameDayLabel(key) {
...
function formatDailyDigestShareText(data, options = {}) {
