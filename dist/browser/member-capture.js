...
exports.COORD_RE = exports.MEMBER_CANONICAL_WIDTH = void 0;
exports.paddleMemberOcr = paddleMemberOcr;
exports.paddleMemberText = paddleMemberText;
exports.positionalMemberName = positionalMemberName;
exports.extractRawNamesPaddle = extractRawNamesPaddle;
exports.stripLikelyPrefixTag = stripLikelyPrefixTag;
exports.splitBadgeAndName = splitBadgeAndName;
exports.cropPctToPixels = cropPctToPixels;
exports.getCanvasRect = getCanvasRect;
exports.ensureOnMembersTab = ensureOnMembersTab;
exports.captureClanMembers = captureClanMembers;
exports.needsMemberCapture = needsMemberCapture;
...
const paddle_lang_js_1 = require("../vision/paddle-lang.js");
...
// reaching the end.
const MAX_MEMBER_PAGES = member_list_sweep_js_1.MEMBER_LIST_MAX_PAGES;
const DRY_PAGES_BEFORE_STOP = member_list_sweep_js_1.MEMBER_LIST_DRY_PAGES_TO_STOP;
const log = (0, logger_js_1.childLogger)('member-capture');
...
exports.MEMBER_CANONICAL_WIDTH = 1000;
...
async function paddleMemberOcr(buffer, opts = {}) {
    ...
    const canonicalWidth = opts.canonicalWidth ?? exports.MEMBER_CANONICAL_WIDTH;
...
async function paddleMemberText(buffer) {
 ...
 */
function positionalMemberName(words, coordIdx) {
    if (coordIdx <= 0)
        return '';
    return words.slice(0, coordIdx).map((w) => w.text).join(' ').trim();
}
async function extractRawNamesPaddle(cropBuffer, langAvailable) {
    ...
    for (const row of rows) {
        ...
        if (worthRecovering) {
            ...
            if (nameWords.length) {
                ...
                const right = Math.min(exports.MEMBER_CANONICAL_WIDTH, Math.max(...nameWords.map((wd) => wd.box.x + wd.box.width)) + 2);
                ...
                if (canonicalPng && right - left >= 8 && bottom - top >= 8) {
                    try {
                        const crop = await (0, sharp_1.default)(canonicalPng).extract({ left, top, width: right - left, height: bottom - top }).png().toBuffer();
                        recovered = await (0, paddle_lang_js_1.recognizeNonLatinName)(crop);
                    }
...
function stripLikelyPrefixTag(name) {
    ...
    if (tokenMatch) {
        ...
        const prefixLooksLikeTag = prefix.length <= 3 || /^[A-Z]{2,4}$/.test(prefix) || /^[a-z]{1,2}$/.test(prefix);
 ...
 */
function splitBadgeAndName(text) {
    ...
    const separator = m[2];
    const name = m[3].trim();
    if (!(level >= 1 && level <= LEVEL_PARSE_MAX))
        return null;
    if (separator.length === 0)
        return null;
    if (level < LEVEL_NEEDS_SPACE_BELOW && !/\s/u.test(separator))
        return null;
...
function pctToScreenshotPoint(pct, size) {
    return {
...
function cropPctToPixels(crop, canvasBounds, screenshotSize) {
...
function pctToCanvasPoint(pct, canvas) {
    return {
 ...
 */
async function ensureOnMembersTab(page, recognizeText, screenshotSize, canvasRect, saveScreenshot, onProgress) {
    ...
    const clanButtonPct = (0, calibration_js_1.requireUiPosition)('clanButton');
    const membersSidebarPct = (0, calibration_js_1.requireUiPosition)('membersSidebar');
    const memberListCropPct = (0, calibration_js_1.requireMemberListCrop)();
    ...
    let lastVerifyText = '';
    const isMembersVisible = async () => {
        verifyStep++;
        const verifyShot = await (0, screenshotter_js_1.captureFullPage)(page);
        ...
        const verifyCrop = await (0, sharp_1.default)(verifyShot)
            .extract(crop)
            .png()
            .toBuffer();
        await saveScreenshot(verifyCrop, './data/screenshots', `member_nav_crop_${verifyStep}`, { force: true });
        const verifyProcessed = await (0, sharp_1.default)(verifyCrop)
            .grayscale()
            .normalize()
            .sharpen({ sigma: 1.3 })
    ...
    await (0, human_delay_js_1.randomDelay)(150, 300);
    const beforeClanShot = await (0, screenshotter_js_1.captureFullPage)(page);
    ...
    await (0, human_delay_js_1.randomDelay)(1500, 2200);
    const afterClanShot = await (0, screenshotter_js_1.captureFullPage)(page);
    ...
    await (0, input_js_1.mouseClick)(page, membersClick.x, membersClick.y);
    await (0, human_delay_js_1.randomDelay)(1800, 2500);
    const afterMembersShot = await (0, screenshotter_js_1.captureFullPage)(page);
    ...
    await (0, input_js_1.mouseClick)(page, membersClick.x, membersClick.y);
    await (0, human_delay_js_1.randomDelay)(1800, 2500);
    const retryShot = await (0, screenshotter_js_1.captureFullPage)(page);
    ...
    try {
        ...
        await (0, input_js_1.mouseClick)(page, membersClick.x, membersClick.y);
        await (0, human_delay_js_1.randomDelay)(1800, 2500);
        const afterReloadShot = await (0, screenshotter_js_1.captureFullPage)(page);
    ...
    if ((0, screen_state_js_1.looksLikeStoreOverlayText)(lastVerifyText)) {
        log.error('Members tab still not visible, and the screen reads as an in-game store/offer popup — '
        ...
        log.error(`Members tab still not visible after a retry and a reload. The calibrated Members sidebar `
 ...
 */
async function captureClanMembers(page, clanId, onProgress) {
    ...
    const langAvailable = config.scanNonLatinFallback && (0, paddle_lang_js_1.isLangFallbackAvailable)();
    await (0, paddle_service_js_1.getPaddleOcr)();
    if (langAvailable)
        (0, paddle_lang_js_1.prewarmLangModels)(); // load lang models up front, not mid-capture
    const recognizeText = (buffer) => paddleMemberText(buffer);
    ...
    const screenshotSize = {
        width: firstMeta.width ?? viewport_js_1.DEFAULT_VIEWPORT_WIDTH,
        height: firstMeta.height ?? viewport_js_1.DEFAULT_VIEWPORT_HEIGHT,
    };
    ...
    log.info(`Render metrics: screenshot=${screenshotSize.width}x${screenshotSize.height}, viewport=${viewportSize?.width ?? 'n/a'}x${viewportSize?.height ?? 'n/a'}, inner=${windowMetrics.innerWidth}x${windowMetrics.innerHeight}, dpr=${windowMetrics.devicePixelRatio}`);
    if (canvasRect) {
        log.info(`Canvas bounds: left=${Math.round(canvasRect.left)}, top=${Math.round(canvasRect.top)}, width=${Math.round(canvasRect.width)}, height=${Math.round(canvasRect.height)}`);
    }
    else {
    ...
    log.info(`Member list crop pct=[${memberListCropPct.leftPct.toFixed(3)},${memberListCropPct.topPct.toFixed(3)}–${memberListCropPct.rightPct.toFixed(3)},${memberListCropPct.bottomPct.toFixed(3)}] → px=[${cropPx.left},${cropPx.top}–${cropPx.left + cropPx.width},${cropPx.top + cropPx.height}]`);
    ...
    for (let pageNum = 0; pageNum < MAX_MEMBER_PAGES; pageNum++) {
        ...
        let cropped;
        try {
            cropped = await (0, sharp_1.default)(screenshot)
                .extract(cropPx)
                .png()
                .toBuffer();
        }
        ...
        for (const sample of read.unreadable) {
            if (unreadableSamples.length < 8)
                unreadableSamples.push(sample);
        }
        ...
        log.info(`Members OCR page ${pageNum}: ${read.coordRows} coord row(s) → ${read.names.length} name(s)`
        ...
        for (const raw of rawNames) {
            let name = (0, player_names_js_1.cleanPlayerName)(raw);
            ...
            const isDuplicate = allNames.some((existing) => normalizeMemberKey(stripLikelyPrefixTag(existing)) === normKey);
            if (!isDuplicate) {
                allNames.push(name);
        ...
        await (0, input_js_1.mouseMove)(page, scrollAt.x, scrollAt.y);
        for (let i = 0; i < 4; i++) {
            await (0, input_js_1.mouseWheel)(page, 0, 100);
            await (0, human_delay_js_1.randomDelay)(20, 40);
        }
        await (0, human_delay_js_1.randomDelay)(100, 200);
    }
    if (pagesScanned >= MAX_MEMBER_PAGES) {
        log.warn(`Member capture hit its ${MAX_MEMBER_PAGES}-page ceiling without reaching the end of the `
    ...
    if (droppedShortNameCount > 0) {
        log.warn(`Member capture: dropped ${droppedShortNameCount} candidate(s) as too short after cleaning (length < 2 or letterCount < 2). Samples: ${droppedShortNameSamples.join(', ')}`);
    }
    ...
    log.info(`Member capture read ${namesRead} name(s) from ${coordRowsSeen} member row(s) across the sweep `
        + `(rows repeat between pages; ${allNames.length} distinct members kept).`);
    if (unreadableSamples.length > 0) {
        log.warn({ noAlert: true }, `Member capture could not read a name on some rows. Samples: ${unreadableSamples.join(' | ')}`);
    }
    ...
    for (const member of allMembers) {
        ...
        if (name !== member.name)
            rewritten.push(`"${member.name}" → "${name}"`);
        try {
    ...
    if (rewritten.length > 0) {
        log.info(`Member capture: ${rewritten.length} name(s) resolved through a player merge rule — ${rewritten.join(', ')}`);
    }
    if (failedMembers.length > 0) {
        log.warn(`Member capture: failed to upsert ${failedMembers.length} member(s) — ${failedMembers.join('; ')}`);
    }
 ...
 */
function needsMemberCapture(clanId) {
    return memberRepo.getMemberCount(clanId) === 0;
}
//# sourceMappingURL=member-capture.js.map