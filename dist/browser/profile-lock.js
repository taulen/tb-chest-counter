...
exports.clearStaleProfileLocks = clearStaleProfileLocks;
exports.killProfileOwner = killProfileOwner;
...
function readLockOwner(lockPath) {
    let target;
    try {
        target = fs_1.default.readlinkSync(lockPath);
    }
    catch {
        try {
            target = fs_1.default.readFileSync(lockPath, 'utf8').trim();
        }
        catch {
            return null;
        }
 ...
 */
function clearStaleProfileLocks(userDataDir, log) {
    try {
        ...
        if (!owner) {
            ...
            for (const name of SINGLETON_FILES.slice(1)) {
                unlinkIfPresent(path_1.default.join(userDataDir, name));
            }
            return;
        }
        const sameHost = owner.hostname === os_1.default.hostname();
        if (sameHost && pidIsAlive(owner.pid)) {
            log.warn(`Chromium profile ${userDataDir} is locked by a LIVE process (pid ${owner.pid}) ` +
        ...
        const why = sameHost
            ...
            : `it belongs to container/host "${owner.hostname}", not this one (${os_1.default.hostname()})`;
        let removed = 0;
        for (const name of SINGLETON_FILES) {
            if (unlinkIfPresent(path_1.default.join(userDataDir, name)))
                removed++;
        }
        log.info(`Cleared ${removed} stale Chromium singleton lock file(s) from ${userDataDir} — ${why}. ` +
 ...
 */
function killProfileOwner(userDataDir, log) {
    try {
        const owner = readLockOwner(path_1.default.join(userDataDir, 'SingletonLock'));
        if (!owner) {
        ...
        if (owner.hostname !== os_1.default.hostname()) {
        ...
        log.warn(`Force-killed the wedged Chromium holding ${userDataDir} (pid ${owner.pid}) — it did not ` +
