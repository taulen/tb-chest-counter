...
exports.parseMight = parseMight;
exports.pairNamesWithMight = pairNamesWithMight;
exports.ownerForBadge = ownerForBadge;
exports.fillLevelsFromAvatarStrip = fillLevelsFromAvatarStrip;
exports.sameOcrSkeleton = sameOcrSkeleton;
exports.betterLevel = betterLevel;
exports.resolveLevel = resolveLevel;
exports.nameMatchCandidates = nameMatchCandidates;
exports.captureMemberMight = captureMemberMight;
...
const MAX_PAGES = member_list_sweep_js_1.MEMBER_LIST_MAX_PAGES;
const DRY_PAGES_TO_STOP = member_list_sweep_js_1.MEMBER_LIST_DRY_PAGES_TO_STOP;
/**
 ...
 */
function parseMight(text) {
    const t = text.trim();
    if (!/^\d[\d.,\s]*\d$/.test(t) && !/^\d$/.test(t))
        return null;
    ...
    if (digits.length < MIGHT_MIN_DIGITS || digits.length > MIGHT_MAX_DIGITS)
        return null;
 ...
 * covered directly.
 */
function pairNamesWithMight(rows, canonicalWidth = member_capture_js_1.MEMBER_CANONICAL_WIDTH) {
    const anchorIdx = [];
    rows.forEach((row, i) => {
        if (member_capture_js_1.COORD_RE.test(row.words.map((w) => w.text).join(' ')))
            anchorIdx.push(i);
    });
    const pairs = [];
    ...
    for (let a = 0; a < anchorIdx.length; a++) {
        ...
        let name = (0, player_names_js_1.cleanPlayerName)((m?.[1] ?? '').trim());
        ...
        if (level === null) {
            const merged = (0, member_capture_js_1.splitBadgeAndName)(name);
            if (merged) {
                level = merged.level;
                name = merged.name;
            }
        ...
        for (let i = anchorIdx[a]; i < rows.length; i++) {
            ...
            for (const word of rows[i].words) {
                ...
                if (!best || word.box.x > best.x)
                    best = { value, x: word.box.x };
            }
        }
 ...
 */
function ownerForBadge(pairs, y, canonH, allAnchorYs) {
    if (pairs.length === 0)
        return null;
    ...
    if (bestDist > pitch * BADGE_MAX_REACH_FRACTION)
        return null;
 ...
 */
async function fillLevelsFromAvatarStrip(pageCrop, canonH, pairs, allAnchorYs) {
    const needy = pairs.filter((p) => p.level === null);
    if (needy.length === 0)
        return 0;
    try {
        const meta = await (0, sharp_1.default)(pageCrop).metadata();
        const width = meta.width ?? 0;
        const height = meta.height ?? 0;
        const stripW = Math.round(width * AVATAR_STRIP_FRACTION);
        if (stripW < 20 || height < 20)
            return 0;
        const strip = await (0, sharp_1.default)(pageCrop)
        ...
        const { rows } = await (0, member_capture_js_1.paddleMemberOcr)(strip, { canonicalWidth: AVATAR_STRIP_WIDTH });
        ...
        const toMainCanon = (yStrip) => (yStrip * (stripW / AVATAR_STRIP_WIDTH)) * (member_capture_js_1.MEMBER_CANONICAL_WIDTH / width);
        let filled = 0;
 ...
 */
async function maybeSaveRowCrop(pageCrop, canonH, row, keepCropFor) {
    if (!keepCropFor || !keepCropFor(row.name))
        return undefined;
    try {
        const meta = await (0, sharp_1.default)(pageCrop).metadata();
        const width = meta.width ?? 0;
        ...
        if (bottom - top < 8)
            return undefined;
        const slice = await (0, sharp_1.default)(pageCrop)
        ...
        const saved = await saveScreenshot(slice, MIGHT_NEW_MEMBER_CROP_DIR, `might_new_member_${normalizeKey(row.name) || 'row'}`, { force: true });
 ...
 */
function nameMatchCandidates(name) {
    const forms = [name];
    const stripped = (0, member_capture_js_1.stripLikelyPrefixTag)(name);
    ...
    const badgeLeftover = (0, member_capture_js_1.splitBadgeAndName)(name);
    if (badgeLeftover)
        forms.push(badgeLeftover.name);
...
async function captureMemberMight(page, opts = {}) {
    ...
    const save = opts.saveDebugShots
        ? saveScreenshot
        : async () => '';
    const onMembers = await (0, member_capture_js_1.ensureOnMembersTab)(page, (buffer) => (0, member_capture_js_1.paddleMemberText)(buffer), screenshotSize, canvasRect, save, onProgress);
    if (!onMembers) {
        result.navigationFailed = true;
        return result;
    }
    const cropPx = (0, member_capture_js_1.cropPctToPixels)((0, calibration_js_1.requireMemberListCrop)(), canvasRect, screenshotSize);
    log.debug(`Might capture crop px=[${cropPx.left},${cropPx.top}–${cropPx.left + cropPx.width},${cropPx.top + cropPx.height}]`);
    ...
    for (let i = 0; i < REWIND_WHEEL_EVENTS; i++) {
        ...
        await (0, human_delay_js_1.randomDelay)(15, 30);
    }
    await (0, human_delay_js_1.randomDelay)(300, 500);
    ...
    for (let pageNum = 0; pageNum < MAX_PAGES; pageNum++) {
        ...
        const { rows, canonH } = await (0, member_capture_js_1.paddleMemberOcr)(cropped);
        ...
        for (const pair of pairs) {
            ...
            if (!existing) {
                ...
                const levelVotes = new Map();
                if (pair.level !== null)
                    levelVotes.set(pair.level, 1);
            ...
            if (existing.might === pair.might)
                continue;
            result.disagreements++;
            if (digits > existing.digits) {
                log.debug(`Might reread for "${pair.name}": ${existing.might} → ${pair.might} (longer digit run wins)`);
        ...
        const samples = pairs.slice(0, 3)
            ...
            .join(', ');
        log.info(`Might page ${pageNum + 1}: ${anchors} member row(s), ${pairs.length} with a value`
        ...
        if (byKey.size === uniqueBefore) {
            ...
            if (dryPages >= DRY_PAGES_TO_STOP) {
                log.info(`Might capture: no new names for ${dryPages} consecutive page(s) — treating that as the `
        ...
        if (hash === lastHash) {
            ...
            if (sameCount >= DRY_PAGES_TO_STOP) {
                log.info(`Might capture: the member list stopped scrolling after ${pageNum + 1} page(s) `
    ...
    if (result.pagesScanned >= MAX_PAGES) {
        log.warn(`Might capture hit the ${MAX_PAGES}-page backstop with ${byKey.size} member(s) read — the `
    ...
    for (const [key, name] of namesSeen) {
        if (!byKey.has(key))
            result.unresolvedNames.push(name);
    }
    if (result.unresolvedNames.length > 0) {
        log.warn({ noAlert: true }, `Might capture: ${result.unresolvedNames.length} member(s) were read but never yielded a `
    ...
    if (contested.length > 0) {
        ...
        log.debug(`Hero-level votes split for ${contested.length} member(s): `
            + contested.slice(0, 10).map((v) => `${v.name} {${[...v.levelVotes].map(([lv, n]) => `${lv}×${n}`).join(' ')}} → ${resolveLevel(v.levelVotes)}`).join(', '));
    }
...
//# sourceMappingURL=might-capture.js.map