...
exports.LoggedOutError = exports.BrowserTargetCrashedError = void 0;
exports.isCrashLikeError = isCrashLikeError;
exports.navigateToGame = navigateToGame;
exports.waitForInteractiveGame = waitForInteractiveGame;
exports.reloadGameCleanly = reloadGameCleanly;
exports.dismissPopups = dismissPopups;
...
const log = (0, logger_js_1.childLogger)('auth');
class BrowserTargetCrashedError extends Error {
    constructor(message) {
        super(message);
        this.name = 'BrowserTargetCrashedError';
    }
}
exports.BrowserTargetCrashedError = BrowserTargetCrashedError;
/**
...
exports.LoggedOutError = LoggedOutError;
...
async function navigateToGame(page, gameUrl) {
    ...
    if (!canvasReady) {
        ...
        throw new Error(`Game canvas did not become ready. url=${page.url()} title="${diag.title}" snippet="${diag.text}"`);
    }
 ...
 */
async function detectLoggedOutPage(page) {
    ...
    if (!LOGGED_OUT_SIGNATURES.some((s) => lower.includes(s)))
        return null;
...
async function waitForCanvasStable(page, { pollMs = 2_500, stableRequired = 4, timeoutMs = 120_000 } = {}) {
    const probeTimeoutMs = 8_000;
    const maxTimedOutProbes = 3;
    const probeCanvasDims = async () => {
        ...
        return Promise.race([evalPromise, timeoutPromise]).catch(() => null);
    };
    ...
    while (Date.now() < deadline) {
        ...
        if (probeTimedOut) {
            timedOutProbeCount++;
            log.warn(`Canvas probe timed out after ${probeElapsedMs}ms (${timedOutProbeCount}/${maxTimedOutProbes}); renderer may be stalled`);
        ...
        if (dims && dims.w > 100 && dims.h > 100) {
            ...
            if (dims.w === lastW && dims.h === lastH) {
                stableCount++;
                log.info(`Canvas stable at ${dims.w}x${dims.h} (${stableCount}/${stableRequired}) canvases=${dims.total} visible=${dims.visible}`);
                ...
                if (dims.w !== lastW || dims.h !== lastH) {
                    log.info(`Canvas resizing: ${lastW}x${lastH} → ${dims.w}x${dims.h} canvases=${dims.total} visible=${dims.visible}`);
                }
                stableCount = 1;
                lastW = dims.w;
                lastH = dims.h;
 ...
 */
async function isGameChromeVisible(page) {
    try {
...
async function waitForInteractiveGame(page) {
    ...
    while (Date.now() - startedAt < MAX_READY_WAIT_MS) {
        ...
        if (visible === null) {
            log.info('Game-readiness probe unavailable; falling back to the fixed wait + Escape sequence.');
            await (0, human_delay_js_1.randomDelay)(24_000, 30_000);
        ...
        if (visible) {
            ...
            if (clearProbes >= CLEAR_PROBES_NEEDED && settled >= MIN_SETTLE_MS) {
                log.info(`Game is interactive after ${Math.round((Date.now() - startedAt) / 1000)}s `
        ...
        await (0, human_delay_js_1.randomDelay)(1_500, 2_200);
    }
    ...
    // wait it replaced.
    log.warn({ noAlert: true }, `Game did not settle into a clear, interactive state within ${MAX_READY_WAIT_MS / 1000}s `
 ...
 */
async function reloadGameCleanly(page, gameUrl) {
    try {
        await page.goto('about:blank', { waitUntil: 'load', timeout: 15_000 });
        await (0, human_delay_js_1.randomDelay)(SOCKET_DRAIN_MS, SOCKET_DRAIN_MS + 1_000);
    }
    catch (err) {
        ...
        log.debug(`about:blank teardown before reload failed: ${err.message}`);
    }
    await navigateToGame(page, gameUrl);
    await waitForInteractiveGame(page);
}
async function dismissPopups(page, maxAttempts = 12) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
        ...
        await (0, human_delay_js_1.randomDelay)(800, 1500);
    }
    log.info('Popup dismissal complete');
    // Brief settle for any remaining fade animations
    await (0, human_delay_js_1.randomDelay)(2_000, 3_000);
    log.info('UI settle complete');
}
async function performManualLogin(page, _context, config) {
    ...
    console.log('========================================\n');
    await navigateToGame(page, game_url_js_1.TB_GAME_URL);
