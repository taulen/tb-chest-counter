...
exports.markMembersSeen = markMembersSeen;
exports.findMemberByName = findMemberByName;
exports.getAllMembers = getAllMembers;
exports.getMemberById = getMemberById;
exports.getMemberClanId = getMemberClanId;
exports.addAlias = addAlias;
exports.renameMember = renameMember;
exports.removeMember = removeMember;
exports.restoreMember = restoreMember;
exports.deactivateStaleMembers = deactivateStaleMembers;
exports.deleteMember = deleteMember;
exports.getMemberCount = getMemberCount;
exports.getMemberEvidenceCropPath = getMemberEvidenceCropPath;
 ...
 */
function findByDespacedKey(db, clanId, key) {
    if (!key)
        return undefined;
    return db.prepare(`SELECT * FROM members WHERE clan_id = ? AND despaced_name = ?
     ORDER BY is_active DESC, id ASC LIMIT 1`).get(clanId, key);
}
/**
 ...
 */
function invalidateMemberDerivedCaches(clanId) {
...
function upsertMember(name, clanId) {
    const db = (0, database_js_1.getDb)();
    const normalized = normalizeName(name);
    const despaced = (0, ocr_normalize_js_1.despace)(name);
    const now = new Date().toISOString();
    const existing = db.prepare('SELECT * FROM members WHERE clan_id = ? AND normalized_name = ?').get(clanId, normalized)
    ...
    const allMembers = db.prepare('SELECT * FROM members WHERE clan_id = ?').all(clanId);
    ...
    const result = db.prepare(`INSERT INTO members (clan_id, name, normalized_name, despaced_name, aliases, first_seen, last_seen)
     VALUES (?, ?, ?, ?, ?, ?, ?)`).run(clanId, name.trim(), normalized, despaced, '[]', now, now);
    return {
 ...
 */
function markMembersSeen(clanId, memberIds, seenAtIso) {
    ...
    const tx = db.transaction(() => {
        ...
        const { cnt } = db.prepare(`SELECT COUNT(*) AS cnt FROM members
       WHERE clan_id = ? AND is_active = 0 AND id IN (${placeholders})`).get(clanId, ...memberIds);
        const res = db.prepare(`UPDATE members SET last_seen = ?, is_active = 1, left_at = NULL
       WHERE clan_id = ? AND id IN (${placeholders})`).run(seenAtIso, clanId, ...memberIds);
...
function findMemberByName(name, clanId) {
    const db = (0, database_js_1.getDb)();
    const normalized = normalizeName(name);
    const row = db.prepare('SELECT * FROM members WHERE clan_id = ? AND normalized_name = ?').get(clanId, normalized)
        ?? findByDespacedKey(db, clanId, (0, ocr_normalize_js_1.despace)(name));
    return row ? rowToMember(row) : null;
}
function getAllMembers(activeOnly, clanId) {
    const db = (0, database_js_1.getDb)();
    const query = activeOnly
        ? 'SELECT * FROM members WHERE clan_id = ? AND is_active = 1 ORDER BY name'
        : 'SELECT * FROM members WHERE clan_id = ? ORDER BY name';
    const rows = db.prepare(query).all(clanId);
 ...
 */
function getMemberById(id, clanId) {
    const db = (0, database_js_1.getDb)();
    const row = db.prepare('SELECT * FROM members WHERE id = ? AND clan_id = ?').get(id, clanId);
    return row ? rowToMember(row) : null;
}
/**
 ...
 */
function getMemberClanId(id) {
    const db = (0, database_js_1.getDb)();
    const row = db.prepare('SELECT clan_id FROM members WHERE id = ?').get(id);
    return row?.clan_id ?? null;
}
function addAlias(memberId, alias, clanId) {
    const db = (0, database_js_1.getDb)();
    const row = db.prepare('SELECT aliases FROM members WHERE id = ? AND clan_id = ?').get(memberId, clanId);
    if (!row)
        return;
...
function renameMember(id, newName, clanId) {
    const db = (0, database_js_1.getDb)();
    ...
    db.prepare('UPDATE members SET name = ?, normalized_name = ?, despaced_name = ? WHERE id = ? AND clan_id = ?').run(newName.trim(), normalized, (0, ocr_normalize_js_1.despace)(newName), id, clanId);
 ...
 */
function removeMember(id, clanId) {
    const db = (0, database_js_1.getDb)();
    ...
    db.prepare('UPDATE members SET is_active = 0, left_at = ? WHERE id = ? AND clan_id = ? AND is_active = 1').run(new Date().toISOString(), id, clanId);
    invalidateMemberDerivedCaches(clanId);
}
/**
 ...
 */
function restoreMember(id, clanId) {
    const db = (0, database_js_1.getDb)();
    const now = new Date().toISOString();
    db.prepare('UPDATE members SET is_active = 1, last_seen = ?, left_at = NULL WHERE id = ? AND clan_id = ?').run(now, id, clanId);
    invalidateMemberDerivedCaches(clanId);
}
/**
 ...
 * second time.
 */
function deactivateStaleMembers(clanId, cutoffIso) {
    const db = (0, database_js_1.getDb)();
    const stale = db.prepare('SELECT id, name FROM members WHERE clan_id = ? AND is_active = 1 AND last_seen < ?').all(clanId, cutoffIso);
    if (stale.length === 0)
        return [];
    db.prepare('UPDATE members SET is_active = 0, left_at = ? WHERE clan_id = ? AND is_active = 1 AND last_seen < ?').run(new Date().toISOString(), clanId, cutoffIso);
    invalidateMemberDerivedCaches(clanId);
    return stale;
}
/**
 ...
 * history.
 */
function deleteMember(id, clanId) {
    const db = (0, database_js_1.getDb)();
    db.prepare('DELETE FROM members WHERE id = ? AND clan_id = ?').run(id, clanId);
}
function getMemberCount(clanId) {
    const db = (0, database_js_1.getDb)();
    const row = db.prepare('SELECT COUNT(*) as count FROM members WHERE clan_id = ? AND is_active = 1').get(clanId);
    return row.count;
}
/**
 ...
 */
function getMemberEvidenceCropPath(memberId, clanId) {
    const db = (0, database_js_1.getDb)();
    const mightRow = db.prepare(`SELECT row_crop_path AS p FROM member_snapshots
     WHERE member_id = ? AND clan_id = ? AND row_crop_path IS NOT NULL
     ORDER BY id ASC LIMIT 1`).get(memberId, clanId);
    if (mightRow)
        return mightRow.p;
    const resourceRow = db.prepare(`SELECT row_crop_path AS p FROM resource_transactions
     WHERE member_id = ? AND clan_id = ? AND row_crop_path IS NOT NULL
     ORDER BY id ASC LIMIT 1`).get(memberId, clanId);
    if (resourceRow)
        return resourceRow.p;
    const chestRow = db.prepare(`SELECT debug_crop_path AS p FROM chest_records
     WHERE member_id = ? AND clan_id = ? AND debug_crop_path IS NOT NULL
     ORDER BY captured_at ASC LIMIT 1`).get(memberId, clanId);
