...
exports.invalidateReviewQueueCount = invalidateReviewQueueCount;
exports.getReviewQueue = getReviewQueue;
exports.getReviewQueueCount = getReviewQueueCount;
...
const log = (0, logger_js_1.childLogger)('review-queue-repo');
...
function invalidateReviewQueueCount(clanId) {
    (0, ttl_cache_js_1.invalidate)(clanId === undefined ? 'reviewQueueCount:' : `reviewQueueCount:${clanId}`);
}
function getAck(category, clanId) {
...
function getReviewQueue(clanId) {
    const db = (0, database_js_1.getDb)();
    ...
    const chestNameRows = db
        .prepare(`SELECT ch.name AS value, g.firstSeen AS firstSeen, g.cnt
       FROM (
       ...
       ORDER BY g.firstSeen DESC`)
        .all(...[clanId, ...(chestNameAckMs !== null ? [chestNameAckMs] : [])]);
    const chestSourceRows = db
        .prepare(`SELECT cs.source AS value, g.firstSeen AS firstSeen, g.cnt
       FROM (
       ...
       WHERE cs.source != ''
       ORDER BY g.firstSeen DESC`)
        .all(...[clanId, ...(chestSourceAckMs !== null ? [chestSourceAckMs] : [])]);
    ...
    const memberRows = db
        .prepare(`SELECT m.id AS memberId, m.name AS value, m.first_seen AS firstSeen,
       ...
       ORDER BY m.first_seen DESC`)
        .all(...[clanId, clanId, ...(memberAck ? [memberAck] : [])]);
    const mapMs = (rows) => rows.map((r) => ({
        value: r.value,
        firstSeen: new Date(r.firstSeen).toISOString(),
        count: r.cnt,
    }));
    const mapMembers = (rows) => rows.map((r) => ({
        value: r.value,
    ...
    const filteredChestNameEntries = mapMs(chestNameRows).filter((e) => !(0, chest_names_js_1.isHardcodedChestName)(e.value));
    const filteredChestSourceEntries = mapMs(chestSourceRows).filter((e) => !(0, source_names_js_1.isHardcodedSource)(e.value));
 ...
 */
function getReviewQueueCount(clanId) {
    return (0, ttl_cache_js_1.cached)(`reviewQueueCount:${clanId}`, REVIEW_QUEUE_COUNT_TTL_MS, () => computeReviewQueueCount(clanId));
}
function computeReviewQueueCount(clanId) {
    const db = (0, database_js_1.getDb)();
    const chestNameAck = getAck('chest_name', clanId);
    const chestSourceAck = getAck('chest_source', clanId);
    const memberAck = getAck('member', clanId);
    ...
    const chestNameRows = db
        .prepare(`SELECT ch.name AS value
       FROM (
         SELECT chest_id
         FROM chest_records
         WHERE clan_id = ?
       ...
       WHERE ch.name != ''`)
        .all(...[clanId, ...(chestNameAckMs !== null ? [chestNameAckMs] : [])]);
    const chestSourceRows = db
        .prepare(`SELECT cs.source AS value
       FROM (
    ...
    const memberCountRow = db
       ...
       WHERE m.clan_id = ?
       ${memberAck ? 'AND m.first_seen > ?' : ''}`)
        .get(...[clanId, ...(memberAck ? [memberAck] : [])]);
    const triumphalCountRow = db
        .prepare(`SELECT COUNT(DISTINCT ch.name) AS cnt
       FROM triumphal_chest_records t
       JOIN chests ch ON ch.id = t.chest_id
       WHERE t.clan_id = ?
         AND ch.name NOT IN (SELECT chest_name FROM triumphal_chest_points)`)
        .get(clanId);
    const chestNames = chestNameRows.filter((r) => !(0, chest_names_js_1.isHardcodedChestName)(r.value)).length;
    const chestSources = chestSourceRows.filter((r) => !(0, source_names_js_1.isHardcodedSource)(r.value)).length;
    return {
        chestNames,
        chestSources,
...
function acknowledge(category, clanId) {
    const db = (0, database_js_1.getDb)();
    const now = new Date().toISOString();
    db.prepare(`INSERT INTO review_acknowledgments (clan_id, category, acknowledged_at) VALUES (?, ?, ?)
     ON CONFLICT(clan_id, category) DO UPDATE SET acknowledged_at = ?`).run(clanId, category, now, now);
