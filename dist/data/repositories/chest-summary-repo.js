...
exports.notifyChestDataChanged = notifyChestDataChanged;
...
exports.getMemberDailySeries = getMemberDailySeries;
exports.resetSummaryStateForTests = resetSummaryStateForTests;
exports.verifyClanSummary = verifyClanSummary;
...
const log = (0, logger_js_1.childLogger)('chest-summary');
 ...
 */
function notifyChestDataChanged(clanId) {
    generation.set(clanId, (generation.get(clanId) ?? 0) + 1);
    (0, ttl_cache_js_1.invalidate)(`leaderboard:${clanId}`);
    (0, ttl_cache_js_1.invalidate)(`singleDay:${clanId}`);
    (0, ttl_cache_js_1.invalidate)(`analyticsSummary:${clanId}`);
    (0, ttl_cache_js_1.invalidate)(`analyticsDaily:${clanId}`);
    (0, ttl_cache_js_1.invalidate)(`topContributors:${clanId}`);
    ...
    (0, ttl_cache_js_1.invalidate)(`analyticsWindow:${clanId}:`);
}
/**
 ...
 */
function ensureFresh(clanId) {
    const gen = generation.get(clanId) ?? 0;
    if (builtGeneration.get(clanId) === gen)
        return;
    rebuild(clanId);
    builtGeneration.set(clanId, gen);
}
function rebuild(clanId) {
    const db = (0, database_js_1.getDb)();
    const rolloverModifier = `-${(0, index_js_1.loadConfig)().gameDayRolloverUtcHour} hours`;
 ...
 */
function getSingleDayRecords(clanId) {
    ensureFresh(clanId);
    const db = (0, database_js_1.getDb)();
    const query = (metric) => db.prepare(`
    WITH best AS (
 ...
 */
function getTopContributors(clanId, limit, fromDay, toDay) {
    ensureFresh(clanId);
    const db = (0, database_js_1.getDb)();
    const windowed = !!(fromDay && toDay);
    const range = windowed ? ' AND s.game_day >= ? AND s.game_day <= ?' : '';
    const args = (last) => (windowed ? [clanId, fromDay, toDay, last] : [clanId, last]);
 ...
 */
function getWindowContributors(clanId, limit, fromDay, toDay) {
    ensureFresh(clanId);
    const windowed = !!(fromDay && toDay);
    const range = windowed ? ' AND s.game_day >= ? AND s.game_day <= ?' : '';
    const params = windowed ? [clanId, fromDay, toDay, limit] : [clanId, limit];
 ...
 */
function getWindowTotals(clanId, fromDay, toDay) {
    ensureFresh(clanId);
    const windowed = !!(fromDay && toDay);
    const sql = `
 ...
 */
function getWindowRosterSize(clanId, fromDay, toDay) {
    if (!fromDay || !toDay) {
        return (0, database_js_1.getDb)().prepare('SELECT COUNT(*) AS n FROM members WHERE clan_id = ?').get(clanId).n;
    }
    const shift = `-${(0, index_js_1.loadConfig)().gameDayRolloverUtcHour} hours`;
    return (0, database_js_1.getDb)().prepare(`
    SELECT COUNT(*) AS n
    FROM members
    WHERE clan_id = ?
  ...
  `).get(clanId, shift, toDay, shift, fromDay).n;
}
/**
 ...
 */
function getWindowDailySeries(clanId, fromDay, toDay) {
    ensureFresh(clanId);
    const windowed = !!(fromDay && toDay);
    const sql = `
  ...
  `;
    const params = windowed ? [clanId, fromDay, toDay] : [clanId];
    return (0, database_js_1.getDb)().prepare(sql).all(...params);
}
/**
 ...
 */
function getMemberWindowComparison(clanId, prevFromDay, prevToDay, curFromDay, curToDay) {
    ensureFresh(clanId);
    return (0, database_js_1.getDb)().prepare(`
    SELECT m.id         AS memberId,
           m.name       AS name,
  ...
  `).all(curFromDay, curToDay, curFromDay, curToDay, prevFromDay, prevToDay, prevFromDay, prevToDay, clanId, prevFromDay, curToDay);
}
/**
 ...
 */
function getConcentration(clanId, fromDay, toDay, weeks = 8) {
    ensureFresh(clanId);
    const db = (0, database_js_1.getDb)();
    const windowed = !!(fromDay && toDay);
  ...
  `).all(...params);
    ...
    let running = 0;
    let membersForHalf = null;
    for (let i = 0; i < ranked.length; i += 1) {
        running += ranked[i].points;
        if (membersForHalf === null && running >= totalPoints / 2)
            membersForHalf = i + 1;
    }
  ...
  `).all(clanId);
    ...
    for (const row of weeklyRows) {
        ...
        if (hit)
            ...
            byWeek.set(row.weekStart, [row.points]);
    }
 ...
 */
function getMemberDailySeries(clanId, memberId, fromDay, toDay) {
    ensureFresh(clanId);
    return (0, database_js_1.getDb)().prepare(`
    ...
    ORDER BY game_day
  `).all(clanId, memberId, fromDay, toDay);
}
/**
 ...
 */
function resetSummaryStateForTests() {
    generation.clear();
 ...
 */
function verifyClanSummary(clanId) {
    ensureFresh(clanId);
    const db = (0, database_js_1.getDb)();
    const raw = db.prepare('SELECT COUNT(*) AS chests, COALESCE(SUM(point_value), 0) AS points FROM chest_records WHERE clan_id = ? AND member_id IS NOT NULL').get(clanId);
    const summary = db.prepare('SELECT COALESCE(SUM(chests), 0) AS chests, COALESCE(SUM(points), 0) AS points FROM chest_daily_summary WHERE clan_id = ?').get(clanId);
    const rawEarned = db.prepare('SELECT COUNT(*) AS chests, COALESCE(SUM(point_value), 0) AS points FROM chest_records'
        + ` WHERE clan_id = ? AND member_id IS NOT NULL${(0, clan_reward_chests_js_1.clanRewardExclusionSql)()}`).get(clanId);
    const summaryEarned = db.prepare('SELECT COALESCE(SUM(earned_chests), 0) AS chests, COALESCE(SUM(earned_points), 0) AS points FROM chest_daily_summary WHERE clan_id = ?').get(clanId);
    const ok = raw.chests === summary.chests
        && raw.points === summary.points
        && rawEarned.chests === summaryEarned.chests
        && rawEarned.points === summaryEarned.points;
    if (!ok) {
        log.warn(`chest_daily_summary drift for clan ${clanId}: raw={${raw.chests},${raw.points}} summary={${summary.chests},${summary.points}}`
...
//# sourceMappingURL=chest-summary-repo.js.map