...
exports.InvalidMergeRuleError = void 0;
exports.addMergeRule = addMergeRule;
exports.getMergeRules = getMergeRules;
exports.deleteMergeRule = deleteMergeRule;
exports.applyMergeRules = applyMergeRules;
exports.loadMergeRules = loadMergeRules;
exports.applyMergeRulesCached = applyMergeRulesCached;
exports.loadPlayerNameCanonicaliser = loadPlayerNameCanonicaliser;
exports.loadChestTypeOverrides = loadChestTypeOverrides;
exports.setChestTypeOverride = setChestTypeOverride;
exports.getChestTypeOverrides = getChestTypeOverrides;
exports.getChestTypeOverride = getChestTypeOverride;
exports.deleteChestTypeOverride = deleteChestTypeOverride;
 ...
 */
function invalidateMergeDerivedCaches(clanId) {
 ...
 */
function isDegenerateNorm(norm) {
 ...
 */
function isUnsafeRuleFrom(norm) {
...
class InvalidMergeRuleError extends Error {
    constructor(message) {
        super(message);
        this.name = 'InvalidMergeRuleError';
    }
}
exports.InvalidMergeRuleError = InvalidMergeRuleError;
// --- Merge Rules ---
function addMergeRule(type, fromValue, toValue, clanId) {
    ...
    if (isUnsafeRuleFrom(normFrom)) {
        throw new InvalidMergeRuleError(`Merge rule "From" value "${fromValue}" is too generic — after normalization it is ` +
    ...
    const redundant = db.prepare('SELECT id, from_value, to_value FROM merge_rules WHERE clan_id = ? AND type = ? AND from_value != ?').all(clanId, type, fromValue);
    ...
    const tx = db.transaction(() => {
      ...
      `).run(clanId, type, fromValue, toValue, now, toValue, now);
            ...
            db.prepare('UPDATE merge_rules SET to_value = ?, created_at = ? WHERE id = ?').run(toValue, now, covering.id);
        ...
        if (type === 'player') {
            ...
            const fromMember = db.prepare('SELECT id FROM members WHERE clan_id = ? AND name = ? COLLATE NOCASE').get(clanId, fromValue);
            if (!fromMember) {
                log.debug(`Player merge: source player "${fromValue}" not in members — no rows to remap`);
            }
            else {
                const toMember = db.prepare('SELECT id FROM members WHERE clan_id = ? AND name = ? COLLATE NOCASE').get(clanId, toValue);
          ...
          `).run(clanId, fromMember.id, toMember.id);
                    ...
                    const result = db.prepare('UPDATE chest_records SET member_id = ? WHERE clan_id = ? AND member_id = ?').run(toMember.id, clanId, fromMember.id);
                    ...
                    db.prepare('UPDATE triumphal_chest_records SET member_id = ? WHERE clan_id = ? AND member_id = ?').run(toMember.id, clanId, fromMember.id);
          ...
          `).run(fromMember.id, toMember.id);
                    const snapResult = db.prepare('UPDATE member_snapshots SET member_id = ? WHERE member_id = ?').run(toMember.id, fromMember.id);
                    ...
                    const txResult = db.prepare('UPDATE resource_transactions SET member_id = ? WHERE clan_id = ? AND member_id = ?').run(toMember.id, clanId, fromMember.id);
                    ...
                    db.prepare('UPDATE members SET name = ?, normalized_name = ?, despaced_name = ? WHERE id = ?').run(toValue.trim(), toValue.trim().toLowerCase().replace(/\s+/g, ' '), (0, ocr_normalize_js_1.despace)(toValue), fromMember.id);
                }
            }
            ...
            const destination = db.prepare('SELECT id, name, aliases FROM members WHERE clan_id = ? AND name = ? COLLATE NOCASE').get(clanId, toValue);
            if (destination) {
                const alias = fromValue.trim();
                const aliases = JSON.parse(destination.aliases || '[]');
                const known = new Set([destination.name, ...aliases].map((a) => a.trim().toLowerCase()));
            ...
            const fromRow = db.prepare('SELECT id FROM chests WHERE name = ? COLLATE NOCASE').get(fromValue);
            if (!fromRow) {
        ...
        `).run(clanId, fromRow.id, toRow.id);
                ...
                const result = db.prepare('UPDATE chest_records SET chest_id = ? WHERE clan_id = ? AND chest_id = ?').run(toRow.id, clanId, fromRow.id);
        ...
        `).get(fromRow.id, fromRow.id);
                ...
                if (fromRow.id !== toRow.id) {
                    db.prepare('UPDATE OR IGNORE chest_type_overrides SET chest_name = ? WHERE clan_id = ? AND chest_name = ?').run(toValue, clanId, fromValue);
                    db.prepare('DELETE FROM chest_type_overrides WHERE clan_id = ? AND chest_name = ?').run(clanId, fromValue);
                }
            }
        }
        else if (type === 'source') {
            // Same shape as chest merge but operates on chest_sources.
            const fromRow = db.prepare('SELECT id FROM chest_sources WHERE source = ? COLLATE NOCASE').get(fromValue);
            if (!fromRow) {
          ...
          RETURNING id
        `).get(toValue);
                const result = db.prepare('UPDATE chest_records SET chest_source_id = ? WHERE clan_id = ? AND chest_source_id = ?').run(toRow.id, clanId, fromRow.id);
                ...
                db.prepare('UPDATE triumphal_chest_records SET chest_source_id = ? WHERE clan_id = ? AND chest_source_id = ?').run(toRow.id, clanId, fromRow.id);
                const stillReferenced = db.prepare(`
...
function getMergeRules(clanId, type) {
    const db = (0, database_js_1.getDb)();
...
function deleteMergeRule(id, clanId) {
    const db = (0, database_js_1.getDb)();
...
function normalize(s) {
    return (0, ocr_normalize_js_1.foldDiacritics)((0, ocr_normalize_js_1.transliterateCyrillicHomoglyphs)(s))
        .replace(/^[\d\W]*\]\s*/, '')
        .replace(/^["|'™|=]+\s*/, '')
        .replace(/[.'",|]+$/, '')
        .toLowerCase()
 ...
 */
function applyMergeRules(type, value, clanId) {
    const db = (0, database_js_1.getDb)();
    const exact = db.prepare('SELECT to_value FROM merge_rules WHERE clan_id = ? AND type = ? AND from_value = ?').get(clanId, type, value);
    if (exact)
        return exact.to_value;
    const rules = db.prepare('SELECT from_value, to_value FROM merge_rules WHERE clan_id = ? AND type = ?').all(clanId, type);
 ...
 */
function loadMergeRules(type, clanId) {
    const db = (0, database_js_1.getDb)();
    const rules = db.prepare('SELECT from_value, to_value FROM merge_rules WHERE clan_id = ? AND type = ?').all(clanId, type);
    const exact = new Map();
    const fuzzy = [];
...
function applyMergeRulesCached(cache, value) {
    ...
    for (const rule of cache.fuzzy) {
        ...
        if (!isDegenerateNorm(rule.normFrom) && normInput.includes(rule.normFrom))
            return rule.toValue;
        if (normInput === rule.normTo)
            return rule.toValue;
    }
    return value;
}
 ...
 */
function loadPlayerNameCanonicaliser(clanId) {
    const cache = loadMergeRules('player', clanId);
    return (rawName) => applyMergeRulesCached(cache, rawName);
}
function applyRulesInMemory(value, rules) {
    const normInput = normalize(value);
    for (const rule of rules) {
        const normRule = normalize(rule.from_value);
        if (normInput === normRule)
            return rule.to_value;
        if (!isDegenerateNorm(normRule) && normInput.includes(normRule))
            return rule.to_value;
 ...
 */
function loadChestTypeOverrides(clanId) {
    const db = (0, database_js_1.getDb)();
    const rows = db.prepare('SELECT chest_name, chest_type FROM chest_type_overrides WHERE clan_id = ?').all(clanId);
    const map = new Map();
...
function setChestTypeOverride(chestName, chestType, clanId) {
    const db = (0, database_js_1.getDb)();
    const now = new Date().toISOString();
    db.prepare(`
...
function getChestTypeOverrides(clanId) {
    const db = (0, database_js_1.getDb)();
    const rows = db.prepare('SELECT * FROM chest_type_overrides WHERE clan_id = ? ORDER BY chest_name').all(clanId);
    return rows.map((r) => ({
        id: r.id,
...
function getChestTypeOverride(chestName, clanId) {
    const db = (0, database_js_1.getDb)();
    const row = db.prepare('SELECT chest_type FROM chest_type_overrides WHERE clan_id = ? AND chest_name = ?').get(clanId, chestName);
    return row?.chest_type ?? null;
}
function deleteChestTypeOverride(id, clanId) {
    const db = (0, database_js_1.getDb)();
    db.prepare('DELETE FROM chest_type_overrides WHERE id = ? AND clan_id = ?').run(id, clanId);
}
//# sourceMappingURL=merge-repo.js.map