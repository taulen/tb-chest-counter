...
exports.reportConfigIntegrity = reportConfigIntegrity;
...
const log = (0, logger_js_1.childLogger)('config-integrity');
function foldKey(s) {
 ...
 */
function checkConfigIntegrity(database) {
    const anyRecords = database.prepare('SELECT EXISTS (SELECT 1 FROM chest_records) AS n').get().n;
    if (!anyRecords)
        return [];
    const findings = [];
    const add = (f) => { findings.push(f); };
    const chests = database.prepare('SELECT id, name FROM chests').all();
    const chestNames = new Set(chests.map((c) => c.name));
    const chestByFold = new Map();
    for (const c of chests) {
        const k = foldKey(c.name);
        if (k && !chestByFold.has(k))
            chestByFold.set(k, c.name);
    }
    const sources = database.prepare('SELECT id, source FROM chest_sources').all();
    const sourceTexts = sources.map((s) => (0, ocr_normalize_js_1.foldDiacritics)(s.source || '').toLowerCase());
    // ── Event catalog ────────────────────────────────────────────────────
    for (const def of event_catalog_js_1.EVENT_CATALOG) {
        for (const rule of def.rules) {
            const declared = [...(rule.chestName ? [rule.chestName] : []), ...(rule.chestNames || [])];
            for (const name of declared) {
                if (chestNames.has(name))
                    continue;
                const live = (chestNames.has((0, chest_names_js_1.correctChestName)(name)) ? (0, chest_names_js_1.correctChestName)(name) : undefined)
            ...
            if (rule.sourceContains) {
                const needle = (0, ocr_normalize_js_1.foldDiacritics)(rule.sourceContains).toLowerCase();
    ...
    for (const s of sources) {
        const k = (0, source_names_js_1.getSourceKey)(s.source);
        if (k)
            liveSourceKeys.add((0, source_names_js_1.canonicalSourceKey)(k));
    }
    const overrides = database.prepare('SELECT source_key, chest_name FROM source_point_overrides').all();
    for (const o of overrides) {
        ...
        if (o.source_key && !liveSourceKeys.has((0, source_names_js_1.canonicalSourceKey)(o.source_key))) {
            add({
    ...
    const anyTriumphal = database.prepare('SELECT EXISTS (SELECT 1 FROM triumphal_chest_records) AS n').get().n;
    if (anyTriumphal) {
        const tp = database.prepare('SELECT chest_name FROM triumphal_chest_points').all();
        for (const t of tp) {
            if (chestNames.has(t.chest_name))
                continue;
            add({
    ...
    const typeOverrides = database.prepare('SELECT clan_id, chest_name FROM chest_type_overrides').all();
    ...
    // ── Merge rules ──────────────────────────────────────────────────────
    const rules = database.prepare("SELECT clan_id, type, from_value, to_value FROM merge_rules WHERE type IN ('chest','source')").all();
    ...
    // admin acts on it.
    const playerRules = database.prepare("SELECT clan_id, from_value, to_value FROM merge_rules WHERE type = 'player'").all();
    const memberName = database.prepare('SELECT id FROM members WHERE clan_id = ? AND name = ? COLLATE NOCASE');
    ...
    for (const s of sources) {
        const k = (0, source_names_js_1.sourceSpellingKey)(s.source);
        if (!k)
            continue;
 ...
 */
function reportConfigIntegrity(database) {
    let findings = [];
    try {
        ...
        log.warn({ noAlert: true }, `Config integrity check failed to run: ${err.message}`);
        return [];
    }
    ...
    if (warnings.length > 0) {
        ...
        log.warn({ noAlert: true }, `${warnings.length} configuration reference(s) point at something that no longer exists (${kinds}).`);
    }
    return findings;
}
