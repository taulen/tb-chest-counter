...
const config_integrity_js_1 = require("./config-integrity.js");
const log = (0, logger_js_1.childLogger)('database');
let db = null;
/**
...
const MIGRATIONS = [
    ...
    `,
        run: (database) => {
            ...
            const insertType = database.prepare('INSERT OR IGNORE INTO resource_types (name, slug) VALUES (?, ?)');
            for (const [name, slug] of types)
                insertType.run(name, slug);
            ...
            const insertTriumphal = database.prepare(`INSERT OR IGNORE INTO triumphal_chest_points (chest_name, package_points, updated_at)
         VALUES (?, ?, ?)`);
            for (const [name, points] of Object.entries(chest_names_js_1.TRIUMPHAL_PACKAGE_POINTS)) {
    ...
    },
    {
        ...
        run: (database) => {
            ...
            const orphans = database
                .prepare(`SELECT id, public_share_token AS token FROM clans
            WHERE public_share_token != ''
              AND public_share_token NOT IN (SELECT token FROM share_links)`)
                .all();
            const insertLink = database.prepare(`INSERT INTO share_links (clan_id, token, created_at, created_by)
         VALUES (?, ?, ?, NULL)`);
            const now = new Date().toISOString();
            for (const o of orphans)
                insertLink.run(o.id, o.token, now);
            ...
            database
                .prepare(`UPDATE share_links SET revoked_at = NULL, revoked_by = NULL, revoke_reason = ''
 ...
 */
function ensureSeedClan(database) {
    database.prepare(`INSERT OR IGNORE INTO clans (id, name, slug, game_url, is_active, created_at)
     VALUES (1, ?, ?, ?, 1, ?)`).run('Clan #1', 'clan-1', 'https://totalbattle.com', new Date().toISOString());
}
  ...
  `);
    ...
    for (const migration of MIGRATIONS) {
        if (migration.version > version) {
            ...
            database.prepare('INSERT INTO schema_version (version, applied_at) VALUES (?, ?)').run(migration.version, new Date().toISOString());
        }
    }
}
/**
 ...
 */
function cleanupChestNames(database) {
    const rows = database.prepare('SELECT id, name FROM chests').all();
    let fixed = 0;
    for (const row of rows) {
        const corrected = (0, chest_names_js_1.correctChestName)(row.name);
        if (corrected === row.name)
            continue;
        const rarity = (0, chest_names_js_1.getChestRarity)(corrected);
        const existing = database.prepare('SELECT id FROM chests WHERE name = ? AND id != ?').get(corrected, row.id);
        if (existing) {
            ...
            for (const table of ['chest_records', 'triumphal_chest_records']) {
                moved += database.prepare(`UPDATE OR IGNORE ${table} SET chest_id = ? WHERE chest_id = ?`).run(existing.id, row.id).changes;
                dropped += database.prepare(`DELETE FROM ${table} WHERE chest_id = ?`).run(row.id).changes;
            }
            if (dropped > 0) {
                log.info(`Chest merge "${row.name}" → "${corrected}": ${moved} record(s) repointed, `
            ...
            database.prepare('UPDATE chests SET name = ?, chest_type = ? WHERE id = ?').run(corrected, rarity, row.id);
        }
        ...
        for (const table of ['source_point_overrides', 'chest_type_overrides', 'triumphal_chest_points']) {
            database.prepare(`UPDATE OR IGNORE ${table} SET chest_name = ? WHERE chest_name = ?`).run(corrected, row.name);
        ...
        database.prepare("UPDATE merge_rules SET to_value = ? WHERE type = 'chest' AND to_value = ?").run(corrected, row.name);
        database.prepare("DELETE FROM merge_rules WHERE type = 'chest' AND from_value = to_value").run();
        fixed++;
...
function initDatabase(dbPath) {
    ...
    (0, config_integrity_js_1.reportConfigIntegrity)(db);
