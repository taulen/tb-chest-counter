...
exports.flushScreencastWarnings = flushScreencastWarnings;
...
const log = (0, logger_js_1.childLogger)('login-bridge-screencast');
...
function flushScreencastWarnings() {
    warner.flush();
}
const SCREENCAST_FORMAT = 'jpeg';
 ...
 */
async function startScreencast(cdp, opts) {
    cdp.on('Page.screencastFrame', (event) => {
        ...
        void cdp.send('Page.screencastFrameAck', { sessionId: event.sessionId }).catch((err) => {
            ...
            if (!msg.includes('detached') && !(0, relay_js_1.isTargetGoneError)(err)) {
    ...
    const apply = (quality, everyNthFrame) => cdp.send('Page.startScreencast', {
    ...
    return {
        ...
        setProfile: async (quality, everyNthFrame) => {
            try {
