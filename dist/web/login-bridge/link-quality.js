...
exports.CONSERVATIVE_STEP = exports.STREAM_LADDER = void 0;
exports.isDefinitelyLocalAddress = isDefinitelyLocalAddress;
exports.clientAddressFor = clientAddressFor;
exports.isStreamProfileForced = isStreamProfileForced;
exports.initialStreamStep = initialStreamStep;
 ...
 */
function isDefinitelyLocalAddress(raw) {
    if (!raw)
        return false;
    ...
    if (addr.startsWith('fe80:'))
        return true; // v6 link-local
    if (/^f[cd][0-9a-f]{0,2}:/.test(addr))
 ...
 */
function clientAddressFor(remoteAddress, forwardedFor) {
...
function isStreamProfileForced() {
 ...
 */
function initialStreamStep(obs = {}) {
    const forced = profileOverride();
    if (forced) {
        return {
            step: forced === 'local' ? 0 : exports.CONSERVATIVE_STEP,
    ...
    if (typeof rtt === 'number' && Number.isFinite(rtt)) {
        ...
        return {
            step: step === -1 ? exports.STREAM_LADDER.length - 1 : step,
            reason: `${rounded} ms round-trip`,
        };
    }
    return { step: exports.CONSERVATIVE_STEP, reason: 'round-trip could not be measured' };
}
/** Clamp an arbitrary index onto the ladder. */
function ladderStep(index) {
    const i = Math.min(exports.STREAM_LADDER.length - 1, Math.max(0, index));
    return exports.STREAM_LADDER[i];
}
//# sourceMappingURL=link-quality.js.map