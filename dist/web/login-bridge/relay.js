...
exports.flushRelayWarnings = flushRelayWarnings;
exports.resetRelayWarnings = resetRelayWarnings;
exports.isTargetGoneError = isTargetGoneError;
exports.dispatchClientMessage = dispatchClientMessage;
exports.parseClientMessage = parseClientMessage;
const logger_js_1 = require("../../utils/logger.js");
const log_throttle_js_1 = require("../../utils/log-throttle.js");
const log = (0, logger_js_1.childLogger)('login-bridge-relay');
...
const warner = (0, log_throttle_js_1.createCoalescedWarner)((msg) => log.warn(msg));
/** Emit any pending "repeated N×" summaries. Call on session teardown. */
function flushRelayWarnings() {
    warner.flush();
}
/** Test seam — drop suppression state without emitting summaries. */
function resetRelayWarnings() {
    warner.reset();
}
/**
...
function resolveVirtualKeyCode(msg) {
    if (typeof msg.keyCode === 'number' && msg.keyCode > 0)
        return msg.keyCode;
    if (msg.key && NAMED_KEY_CODES[msg.key] !== undefined)
        return NAMED_KEY_CODES[msg.key];
    ...
    if (msg.key && msg.key.length === 1)
        return msg.key.toUpperCase().charCodeAt(0);
    return 0;
}
 ...
 */
async function dispatchClientMessage(cdp, page, msg) {
    try {
        switch (msg.kind) {
            case 'mouse': {
                if (msg.type === 'wheel') {
                ...
                const cdpType = msg.type === 'mousedown' ? 'mousePressed'
