...
exports.loginBridge = void 0;
exports.resolveBridgeViewport = resolveBridgeViewport;
exports.resolveBridgeDisplay = resolveBridgeDisplay;
...
const xvfb_js_1 = require("./login-bridge/xvfb.js");
const relay_js_1 = require("./login-bridge/relay.js");
const screencast_js_1 = require("./login-bridge/screencast.js");
const deadline_js_1 = require("../utils/deadline.js");
const link_quality_js_1 = require("./login-bridge/link-quality.js");
const rtt_js_1 = require("./login-bridge/rtt.js");
const gpu_js_1 = require("../browser/gpu.js");
const profile_lock_js_1 = require("../browser/profile-lock.js");
const log = (0, logger_js_1.childLogger)('login-bridge');
...
function resolveBridgeViewport(raw) {
...
const { width: VIEWPORT_WIDTH, height: VIEWPORT_HEIGHT } = resolveBridgeViewport(process.env.LOGIN_BRIDGE_VIEWPORT);
function resolveBridgeDisplay(raw) {
...
class LoginBridge {
    ...
    streamStep = link_quality_js_1.CONSERVATIVE_STEP;
    ...
    async start(clanId = 1) {
        ...
        try {
            ...
            // isolated.
            const gameUrl = game_url_js_1.TB_GAME_URL;
            const profileDir = (0, clan_paths_js_1.clanLoginProfileDir)(clanId);
            const targetStorageState = (0, clan_paths_js_1.clanStorageStatePath)(clanId);
            ...
            log.info(`Login bridge rendering at ${VIEWPORT_WIDTH}x${VIEWPORT_HEIGHT} ` +
                `(display: ${display === 'xvfb' ? 'Xvfb, software-only' : 'new headless'}).`);
            let headed = false;
            if (display === 'xvfb') {
                this.xvfb = await (0, xvfb_js_1.tryStartXvfb)(VIEWPORT_WIDTH, VIEWPORT_HEIGHT);
            ...
            let gpuEnabled = (0, gpu_js_1.isBridgeGpuEnabled)() && !headed;
            if ((0, gpu_js_1.isBridgeGpuEnabled)() && headed) {
                log.warn('GPU rendering is requested but the display is Xvfb, which is a pure software ' +
            ...
            if (gpuEnabled) {
                ...
                const status = await (0, gpu_js_1.logWebglRenderer)(page, log);
                if (status === 'stalled') {
                    ...
                    log.warn('Abandoning the GPU attempt for this login session and relaunching with ' +
                    ...
                    const discarded = await (0, deadline_js_1.settleWithDeadline)(context.close(), CONTEXT_CLOSE_TIMEOUT_MS, 'discarding the GPU attempt');
                    if (discarded === 'timeout') {
                        ...
                        (0, profile_lock_js_1.killProfileOwner)(profileDir, log);
                    }
                    gpuEnabled = false;
                ...
                log.info('Login bridge is software-rendering the game. That is the main limit on ' +
            ...
            await context.addInitScript(() => {
                ...
                Object.defineProperty(window, 'open', {
                    configurable: true,
                    writable: true,
                    value: function patchedOpen(url) {
                        if (url) {
            ...
            await page.goto(gameUrl, { waitUntil: 'domcontentloaded', timeout: 60_000 }).catch((err) => {
                ...
                if ((0, relay_js_1.isTargetGoneError)(err)) {
            ...
            if (!(await this.confirmAlive(context, page))) {
                throw new Error('The browser stopped responding while the login session was starting (most likely out of ' +
     ...
     * whole option set.
     */
    async launchBridgeBrowser(profileDir, headed, gpuEnabled) {
        ...
        (0, profile_lock_js_1.clearStaleProfileLocks)(profileDir, log);
        const context = await playwright_1.chromium.launchPersistentContext(profileDir, {
            ...
            timezoneId: (0, game_timezone_js_1.gameDayTimezoneId)((0, index_js_1.getConfig)().gameDayRolloverUtcHour),
    ...
    async attachSocket(ws, clientAddress) {
        ...
        ws.on('message', (raw) => {
            const parsed = (0, relay_js_1.parseClientMessage)(raw.toString());
            if (!parsed)
                return;
            const activeCdp = this.cdp;
            const activePage = this.page;
            if (!activeCdp || !activePage)
                return;
            void (0, relay_js_1.dispatchClientMessage)(activeCdp, activePage, parsed).then((result) => {
     ...
     */
    async streamTo(ws, clientAddress) {
        ...
        const rttMs = await (0, rtt_js_1.measureSocketRtt)(ws);
        const start = (0, link_quality_js_1.initialStreamStep)({ address: clientAddress, rttMs });
        this.streamStep = start.step;
        const profile = (0, link_quality_js_1.ladderStep)(this.streamStep);
        log.info(`Login session client at ${clientAddress ?? 'unknown address'} ` +
            `starting at "${profile.label}" (${start.reason}); quality ${profile.quality}, ` +
            `every ${profile.everyNthFrame} frame(s).`);
        const screencast = await (0, screencast_js_1.startScreencast)(cdp, {
     ...
     */
    async confirmAlive(context, page) {
        if (this.browserGone || page.isClosed())
            return false;
        const browser = context.browser();
        if (browser && !browser.isConnected())
            return false;
        try {
            await (0, deadline_js_1.withDeadline)(context.cookies(), LIVENESS_PROBE_TIMEOUT_MS, 'browser liveness probe');
            return true;
        }
        catch (err) {
     ...
     */
    handleBrowserGone(reason) {
        if (this.tearingDown || this.browserGone)
            return;
    ...
    async stopActiveScreencast() {
        ...
        this.cdp = null;
        if (stopScreencast) {
            await (0, deadline_js_1.settleWithDeadline)(stopScreencast(), TEARDOWN_STEP_TIMEOUT_MS, 'stopping the screencast');
        }
        if (cdp) {
            await (0, deadline_js_1.settleWithDeadline)(cdp.detach(), TEARDOWN_STEP_TIMEOUT_MS, 'detaching the CDP session');
        }
    }
    ...
    startAdaptiveLoop() {
        ...
        if ((0, link_quality_js_1.isStreamProfileForced)())
            return;
        ...
        if (typeof this.adaptTimer.unref === 'function')
    ...
    moveStream(next, why) {
        const clamped = Math.min(link_quality_js_1.STREAM_LADDER.length - 1, Math.max(0, next));
        if (clamped === this.streamStep)
            return;
        const direction = clamped > this.streamStep ? 'down' : 'up';
        this.streamStep = clamped;
        const profile = (0, link_quality_js_1.ladderStep)(clamped);
        log.info(`Login session stream stepped ${direction} to "${profile.label}" ` +
     ...
     */
    async hydrateFromStorageState(context, storageStatePath) {
    ...
    async save(adminUserId) {
        ...
        let state;
        try {
            state = await (0, deadline_js_1.withDeadline)(this.context.storageState(), STORAGE_STATE_TIMEOUT_MS, 'reading the browser session state');
        }
        catch (err) {
            log.warn('Could not read the login session state: ' + String(err).split('\n')[0]);
            await this.teardown();
            throw new Error('The browser stopped responding before the session could be captured, so nothing was saved. ' +
        ...
        if (!hasSessionCookie) {
            log.warn(`Captured session is missing the TB session cookie (PTBHSSID) — the scanner will treat this clan as logged out. ` +
        ...
        fs_1.default.mkdirSync(path_1.default.dirname(targetStorageState), { recursive: true });
        if (fs_1.default.existsSync(targetStorageState)) {
            fs_1.default.copyFileSync(targetStorageState, targetStorageState + '.bak');
        }
        fs_1.default.writeFileSync(targetStorageState, JSON.stringify(state, null, 2));
        if (adminUserId !== null) {
            (0, user_repo_js_1.logAction)(adminUserId, 'login_session_save', { cookies: cookies.length, hasTbAuth, clanId });
        }
        ...
        (0, clan_repo_js_1.clearClanNeedsReauth)(clanId);
        await this.teardown();
    ...
    async teardown() {
        ...
        try {
            ...
            if (context) {
                ...
                // it down too.
                const outcome = await (0, deadline_js_1.settleWithDeadline)(context.close(), CONTEXT_CLOSE_TIMEOUT_MS, 'closing the login bridge browser');
                if (outcome === 'timeout') {
                    log.warn(`The login bridge browser did not close within ${CONTEXT_CLOSE_TIMEOUT_MS / 1000}s — it is ` +
        ...
        (0, relay_js_1.flushRelayWarnings)();
        (0, screencast_js_1.flushScreencastWarnings)();
        if (droppedFrames > 0) {
            log.info(`Login session dropped ${droppedFrames} frame(s) to socket backpressure — ` +
...
exports.loginBridge = new LoginBridge();
//# sourceMappingURL=login-bridge.js.map