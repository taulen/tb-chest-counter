...
const log = (0, logger_js_1.childLogger)('clan-resources-route');
/**
...
function debugRunsRoot() {
    return path_1.default.resolve('data', 'screenshots', 'resource-debug');
}
let resourceCollectJob = null;
/**
 ...
 */
function createResourcesRouter(scanLoop) {
    ...
    router.put('/:clanId/resources', auth_js_1.requireClanAdmin, (req, res) => {
        ...
        (0, clan_repo_js_1.setClanResourcesEnabled)(id, enabled);
        ...
        if (autoCapture !== undefined)
            (0, clan_repo_js_1.setClanResourceAutoCapture)(id, !!autoCapture);
        (0, user_repo_js_1.logAction)(req.user.id, 'clan.resources.toggle', { clanId: id, enabled, autoCapture });
        res.json({ ok: true, enabled, autoCapture: (0, clan_repo_js_1.getClanById)(id)?.resourceAutoCapture ?? true });
    });
    /**
     ...
     */
    router.get('/:clanId/resources/debug-shots', auth_js_1.requireClanAdmin, (req, res) => {
        ...
        try {
            ...
            const runs = fs_1.default.readdirSync(root, { withFileTypes: true })
            ...
            const shots = fs_1.default.readdirSync(path_1.default.join(root, run))
                .filter((name) => DEBUG_SHOT_NAME.test(name))
                .sort()
                .map((name) => {
                const stat = fs_1.default.statSync(path_1.default.join(root, run, name));
    ...
    router.get('/:clanId/resources/debug-shots/:run/:name', auth_js_1.requireClanAdmin, (req, res) => {
        ...
        const filePath = path_1.default.join(debugRunsRoot(), run, name);
     ...
     */
    router.post('/:clanId/resources/collect', auth_js_1.requireClanAdmin, async (req, res) => {
        ...
        if (!(0, calibration_js_1.isResourceHistoryCalibrated)()) {
            res.status(400).json({
                error: 'Automated collection is not calibrated yet. Outstanding: '
                    + `${(0, calibration_js_1.missingResourceHistoryTargets)().join('; ')}. Open System → Scanner Calibration and `
                    + 'complete Stages 5 and 6 (and the MAP button in Stage 1).',
            });
            return;
        }
        if (!(0, clan_repo_js_1.getClanById)(clanId)?.resourcesEnabled) {
            res.status(400).json({
        ...
        (0, user_repo_js_1.logAction)(req.user.id, 'resources.collect', { clanId, fullBackfill, dryRun, maxPages });
        resourceCollectJob = {
            status: 'running',
            clanId,
            startedAt: Date.now(),
     ...
     */
    router.get('/:clanId/resources/collect/status', auth_js_1.requireClanAdmin, (req, res) => {
        const clanId = req.parsedClanId;
