...
exports.isLangFallbackAvailable = isLangFallbackAvailable;
exports.prewarmLangModels = prewarmLangModels;
exports.resetLangServices = resetLangServices;
exports.recognizeNonLatinName = recognizeNonLatinName;
/**
...
const log = (0, logger_js_1.childLogger)('paddle-lang');
...
const LANG_DIR = path_1.default.resolve('assets', 'paddle-lang-models');
const DET_PATH = path_1.default.resolve('assets', 'paddle-models', 'det.onnx');
const _services = new Map();
function toAB(b) {
    return b.buffer.slice(b.byteOffset, b.byteOffset + b.byteLength);
}
/** True when the bundled language models are present on disk. */
function isLangFallbackAvailable() {
    return LANGS.every((l) => (0, fs_1.existsSync)(path_1.default.join(LANG_DIR, l.slug, 'rec.onnx')));
}
/**
 ...
 */
function prewarmLangModels() {
    if (!isLangFallbackAvailable())
        return;
    for (const spec of LANGS) {
        getLangService(spec).catch(() => { });
    }
}
/**
 ...
 */
function resetLangServices() {
    _services.clear();
}
function getLangService(spec) {
    const cached = _services.get(spec.slug);
    if (cached)
        return cached;
    const p = (async () => {
        const det = (0, fs_1.readFileSync)(DET_PATH);
        const rec = (0, fs_1.readFileSync)(path_1.default.join(LANG_DIR, spec.slug, 'rec.onnx'));
        const rawDict = (0, fs_1.readFileSync)(path_1.default.join(LANG_DIR, spec.slug, 'dict.txt'), 'utf-8')
            .replace(/\n$/, '').split(/\r?\n/);
        const dict = spec.transform(rawDict);
        log.info(`loading language model "${spec.slug}" (${dict.length} dict entries)`);
        return paddleocr_1.PaddleOcrService.createInstance({
            ort: paddle_service_js_1.ortRuntime,
...
async function recognizeWith(spec, rgb) {
    const svc = await getLangService(spec);
    const regions = (await svc.recognize({ ...rgb, data: new Uint8Array(rgb.data) }));
    if (!regions.length)
        return '';
 ...
 * cross-contamination.
 */
async function recognizeNonLatinName(cropBuffer) {
    ...
    for (const ch of best)
        if (bestSpec.scriptRe.test(ch) || /\s/.test(ch))
            cleaned += ch;
    cleaned = cleaned.replace(/\s+/g, ' ').trim();
    if (bestSpec.rtl)
        cleaned = [...cleaned].reverse().join('');
    return cleaned;
}
//# sourceMappingURL=paddle-lang.js.map