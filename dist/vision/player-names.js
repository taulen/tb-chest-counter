...
exports.cleanPlayerName = cleanPlayerName;
exports.isLikelyNonLatinName = isLikelyNonLatinName;
exports.matchKnownPlayer = matchKnownPlayer;
/**
 ...
 */
function cleanPlayerName(ocrName) {
    return ocrName
 ...
 */
function isLikelyNonLatinName(s) {
    let latin = 0;
    let nonLatin = 0;
    for (const ch of s) {
 ...
 */
function matchKnownPlayer(ocrName, knownNames, exactOnlyNames = []) {
    if (!ocrName || (knownNames.length === 0 && exactOnlyNames.length === 0))
        return ocrName;
    ...
    const normInput = (0, ocr_normalize_js_1.despace)(ocrName);
    ...
    if (!normInput) {
        const nlInput = (0, ocr_normalize_js_1.normalizeNonLatin)(ocrName);
        if (!nlInput)
            return ocrName;
        // Exact normalized match.
        for (const name of exactCandidates) {
            if ((0, ocr_normalize_js_1.normalizeNonLatin)(name) === nlInput)
                return name;
        }
        ...
        for (const name of knownNames) {
            const nlKnown = (0, ocr_normalize_js_1.normalizeNonLatin)(name);
            ...
            if ((0, ocr_normalize_js_1.namesDifferByAltSuffix)(ocrName, name))
                continue;
            if (nlKnown.length >= 3 && nlInput.length >= 3) {
                const maxLen = Math.max(nlInput.length, nlKnown.length);
                const allowedDist = maxLen >= 6 ? 2 : 1;
                const dist = (0, ocr_normalize_js_1.levenshtein)(nlInput, nlKnown);
    ...
    for (const name of exactCandidates) {
        if ((0, ocr_normalize_js_1.despace)(name) === normInput)
            return name;
    }
    ...
    const ocrNormInput = (0, ocr_normalize_js_1.ocrNormalize)(ocrName);
    if (ocrNormInput) {
        for (const name of exactCandidates) {
            if ((0, ocr_normalize_js_1.ocrNormalize)(name) === ocrNormInput && !(0, ocr_normalize_js_1.namesDifferByAltSuffix)(ocrName, name)) {
                return name;
            }
        }
    }
    ...
    for (const name of knownNames) {
        const normKnown = (0, ocr_normalize_js_1.despace)(name);
        ...
        if (normKnown.length >= 3 && normInput.length >= 3) {
            ...
            const allowedDist = maxLen >= 6 ? 2 : 1;
            const dist = (0, ocr_normalize_js_1.levenshtein)(normInput, normKnown);
...
//# sourceMappingURL=player-names.js.map