...
exports.resolveTransactionDate = resolveTransactionDate;
exports.parseHistoryLine = parseLine;
exports.processResourceScreenshot = processResourceScreenshot;
...
const log = (0, logger_js_1.childLogger)('resource-ocr');
 ...
 */
function resolveTransactionDate(dateLabel, uploadDate) {
...
function parseLine(line) {
    ...
    const txMatch = trimmed.match(/^(.+?)(sent|took)\s*resources[.\s]*([+\-−]?[\s]*\d[\d.,'\u2019\u00A0]*)/i);
    if (!txMatch)
        return null;
...
function nccRgb(a, b) {
    return (ncc(a.r.pixels, a.r.mean, a.r.std, b.r.pixels, b.r.mean, b.r.std) +
        ncc(a.g.pixels, a.g.mean, a.g.std, b.g.pixels, b.g.mean, b.g.std) +
        ncc(a.b.pixels, a.b.mean, a.b.std, b.b.pixels, b.b.mean, b.b.std)) / 3;
}
 ...
 */
const CY_RETRY_OFFSETS = [1, -1, 2, -2];
/**
 ...
 */
async function loadTemplates(slug) {
    if (templateCache.has(slug))
        return templateCache.get(slug);
    const dir = path_1.default.resolve('assets', 'resource-icons');
    const results = [];
    const primary = await loadTemplateFile(path_1.default.join(dir, `${slug}.png`));
    if (primary) {
        results.push(primary);
        for (let n = 2; n <= 5; n++) {
            const v = await loadTemplateFile(path_1.default.join(dir, `${slug}-${n}.png`));
            if (!v)
                break;
            results.push(v);
        }
...
async function matchIcon(iconBuffer, allTypes) {
    let cand;
    try {
        const data = await (0, sharp_1.default)(iconBuffer)
            .flatten({ background: { r: 205, g: 184, b: 152 } })
            .resize(48, 48, { fit: 'fill', kernel: 'lanczos3' })
            .raw().toBuffer();
...
function findIconContour(fullImage, canonicalH, stripLeft, rowTop, rowH, darkThreshold) {
    ...
    const invImage = new paddleocr_1.Image(stripW, stripH, 3, inverted);
    ...
    const inThisRow = contours.filter((c) => stripY + c.y < rowBottom && stripY + c.y + c.height > rowTop);
    if (!inThisRow.length)
        return null;
 ...
 */
async function cropAndMatchIconMultiScale(canonicalRgb, canonicalH, fullImage, amountRight, rowTop, rowH, darkThreshold, allTypes) {
    ...
    const scanCandidates = async (cys) => {
                ...
                for (const base of baseSizes)
                    for (const delta of [-4, 0, 4, 8, 12, 16]) {
                        ...
                        try {
                            ...
                            const data = await (0, sharp_1.default)(canonicalRgb, {
    ...
    if (bestId == null) {
        await scanCandidates([...new Set(cyCandidates.flatMap((c) => CY_RETRY_OFFSETS.map((d) => c + d)))]);
    }
    return bestId;
}
 ...
 */
async function saveUnresolvedRowCrop(canonicalBuffer, canonicalH, rowTop, rowH, uploadToken, rowIndex) {
    ...
    const overlay = Buffer.from(`<svg width="${CANONICAL_WIDTH}" height="${height}">`
        + `<rect x="2" y="${boxY}" width="${CANONICAL_WIDTH - 4}" height="${boxH}"`
        + ` fill="none" stroke="#e8562a" stroke-width="4" rx="4"/></svg>`);
    try {
        await fs_1.default.promises.mkdir(crop_dirs_js_1.UNRESOLVED_CROP_DIR, { recursive: true });
        const file = path_1.default.join(crop_dirs_js_1.UNRESOLVED_CROP_DIR, `unresolved_${uploadToken}_r${String(rowIndex).padStart(3, '0')}.png`);
        await (0, sharp_1.default)(canonicalBuffer)
 ...
 */
async function processResourceScreenshot(opts) {
    ...
    const untrackedTypeIds = new Set(allTypes.filter((t) => resource_repo_js_1.UNTRACKED_RESOURCE_SLUGS.has(t.slug)).map((t) => t.id));
    ...
    const canonicalBuffer = await (0, sharp_1.default)(imageBuffer)
    ...
    const { data: rawRgb, info } = await (0, sharp_1.default)(canonicalBuffer)
        .raw()
        .toBuffer({ resolveWithObject: true });
    const fullImage = (0, paddle_service_js_1.normalizeInputToRgb)({
        width: info.width,
        height: info.height,
    ...
    for (const row of rowGroups) {
        ...
        const member = (0, fuzzy_js_1.fuzzyMatchMember)(rawPlayerName, members);
        if (!member) {
        ...
        if (knownResourceSlug) {
            ...
            rows.push({
                ...
                rowCropPath: (found == null || member == null) && cropToken
                    ? await saveUnresolvedRowCrop(canonicalBuffer, canonicalH, rowTop, rowHeight, cropToken, rowIndex++)
                    : null,
            });
            continue;
        }
        ...
        const amountWord = byRight.find(w => isLikelyAmount(w.text)) ??
        ...
        if (amountWord) {
            ...
            if (amountRight >= CANONICAL_WIDTH * 0.7) {
                try {
                    resourceTypeId = await cropAndMatchIconMultiScale(rawRgb, canonicalH, fullImage, amountRight, rowTop, rowHeight, darkThreshold, allTypes);
                }
                catch (err) {
        ...
        if (resourceTypeId != null && untrackedTypeIds.has(resourceTypeId))
            continue;
    ...
    log.info(`resource-ocr: extracted ${rows.length} rows, ${errors.length} errors from screenshot`);
    return { rows, errors, unmatchedNames, finalDateLabel: currentDateLabel };
}
//# sourceMappingURL=resource-ocr.js.map