...
exports.SOURCE_CHEST_POINTS = exports.SOURCE_POINT_KEYS = void 0;
exports.cleanSource = cleanSource;
exports.parseSourceLevel = parseSourceLevel;
exports.parseSourceLevelRange = parseSourceLevelRange;
exports.getSourceKey = getSourceKey;
exports.canonicalSourceKey = canonicalSourceKey;
exports.sourceSpellingKey = sourceSpellingKey;
exports.getDefaultPointsForKey = getDefaultPointsForKey;
exports.getDefaultPointsFor = getDefaultPointsFor;
exports.isKnownSourceKey = isKnownSourceKey;
exports.getPointsFromSource = getPointsFromSource;
exports.isHardcodedSource = isHardcodedSource;
/**
 ...
 */
exports.SOURCE_POINT_KEYS = Object.keys(SOURCE_POINTS);
/**
 ...
 */
function cleanSource(ocrSource) {
    return ocrSource
 ...
 * level the same way.
 */
function parseSourceLevel(source) {
    if (!source)
        return null;
 ...
 */
function parseSourceLevelRange(source) {
    if (!source)
        return null;
...
function deriveSourceKey(source) {
    ...
    if (lower.includes('citadel')) {
        if (lower.includes('cursed'))
            return `cursed citadel ${level}`;
        return `elven citadel ${level}`;
    }
    if (lower.includes('epic') && lower.includes('crypt'))
        return `epic ${level}`;
    if (lower.includes('rare') && lower.includes('crypt'))
        return `rare ${level}`;
    if (lower.includes('crypt'))
        return `common ${level}`;
    ...
    if (lower.includes('jormungandr') && !lower.includes('squad'))
        return 'jormungandr shop';
    ...
    const slug = (0, ocr_normalize_js_1.foldDiacritics)(lower)
        .replace(/[^a-z0-9]+/g, ' ')
        .trim()
        .replace(/\s+/g, ' ');
 ...
 * again.
 */
function canonicalSourceKey(key) {
    return (0, ocr_normalize_js_1.foldDiacritics)(key).toLowerCase().replace(/[^a-z0-9]/g, '');
}
/**
 ...
 */
function sourceSpellingKey(source) {
    return (0, ocr_normalize_js_1.foldDiacritics)(source)
        .toLowerCase()
        .replace(/lvl/g, 'level')
        .replace(/[^a-z0-9]/g, '');
}
...
const SOURCE_POINTS_CANONICAL = new Map(Object.entries(SOURCE_POINTS).map(([k, v]) => [canonicalSourceKey(k), v]));
...
const SOURCE_CHEST_POINTS_CANONICAL = new Map(Object.entries(exports.SOURCE_CHEST_POINTS).flatMap(([key, byName]) => Object.entries(byName).map(([chestName, v]) => [`${canonicalSourceKey(key)}\t${chestName}`, v])));
/**
 ...
 */
function getDefaultPointsForKey(key) {
    if (!key)
        return 0;
 ...
 */
function getDefaultPointsFor(key, chestName) {
    if (!key)
        return 0;
    if (chestName) {
        const exact = exports.SOURCE_CHEST_POINTS[key]?.[chestName];
        if (exact !== undefined)
            return exact;
...
function isKnownSourceKey(key) {
    if (!key)
        return false;
    if (key === 'arena' || key === 'union reward' || key === 'jormungandr shop')
        return true;
    if (/^(common|rare|epic|vault) \d+$/.test(key))
        return true;
    if (/^(elven citadel|cursed citadel) \d+$/.test(key))
        return true;
    return false;
}
function getPointsFromSource(source) {
 ...
 */
function isHardcodedSource(source) {
