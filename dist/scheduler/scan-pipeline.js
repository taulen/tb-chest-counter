...
exports.UNKNOWN_PLAYER_NAME = void 0;
exports.scanCardsPipelined = scanCardsPipelined;
...
const source_points_repo_js_1 = require("../data/repositories/source-points-repo.js");
 ...
 */
exports.UNKNOWN_PLAYER_NAME = '[Unknown]';
...
async function scanCardsPipelined(ctx, page, sessionId, target = 'gifts') {
    ...
    const playerMergeCache = (0, merge_repo_js_1.loadMergeRules)('player', clanId);
    const chestMergeCache = (0, merge_repo_js_1.loadMergeRules)('chest', clanId);
    const sourceMergeCache = (0, merge_repo_js_1.loadMergeRules)('source', clanId);
    const sourcePointOverrides = (0, source_points_repo_js_1.loadOverrides)();
    const chestTypeOverrides = (0, merge_repo_js_1.loadChestTypeOverrides)(clanId);
    ...
    const knownChestNames = new Set(chestRepo.getDistinctChestNames(clanId).map((c) => c.name));
     ...
     */
    const resolvePlayerName = (card) => {
        const cleanedPlayerMulti = (0, player_names_js_1.cleanPlayerName)(card.playerName);
        const cleanedPlayerEng = card.playerNameEnglish ? (0, player_names_js_1.cleanPlayerName)(card.playerNameEnglish) : '';
        if (!cleanedPlayerEng || (0, player_names_js_1.isLikelyNonLatinName)(cleanedPlayerMulti)) {
            return (0, player_names_js_1.matchKnownPlayer)(cleanedPlayerMulti, knownMembers, inactiveMemberNames);
        }
        const matchedEng = (0, player_names_js_1.matchKnownPlayer)(cleanedPlayerEng, knownMembers, inactiveMemberNames);
        if (matchedEng !== cleanedPlayerEng)
            return matchedEng;
        const matchedMulti = (0, player_names_js_1.matchKnownPlayer)(cleanedPlayerMulti, knownMembers, inactiveMemberNames);
    ...
    log.info(`pipelined: starting capture phase. viewportClick=(${viewportClickX},${viewportClickY}) viewportCrop=${JSON.stringify(viewportCrop)}`);
    ...
    let sessionKickReason = null;
    let totalClicks = 0;
    const captureStartedAt = Date.now();
    const oomKillsAtStart = (0, memory_snapshot_js_1.memorySnapshot)().oomKills;
    ...
    const slowWarner = (0, log_throttle_js_1.createCoalescedWarner)((m) => log.warn(m), 60_000);
    /**
    ...
    const captureStallReason = () => {
        ...
        const oomKillsNow = (0, memory_snapshot_js_1.memorySnapshot)().oomKills;
    ...
    for (let batch = 1; batch <= MAX_BATCHES; batch++) {
        ...
        if (stall) {
            ...
            log.warn(`pipelined: abandoning capture at batch ${batch} — ${stall}. ${crops.length} crop(s) from ` +
                ...
                `is left on the Gifts tab. [${(0, memory_snapshot_js_1.memorySnapshot)().summary}]`);
        ...
        try {
            ...
            if (isDebugIteration) {
                fullScreenshot = await (0, screenshotter_js_1.captureFullPage)(page);
                tabResult.screenshots++;
                batchCrop = await (0, image_js_1.cropRegion)(fullScreenshot, viewportCrop);
            }
            else {
                batchCrop = await (0, screenshotter_js_1.captureRegion)(page, {
                    x: viewportCrop.left,
                    y: viewportCrop.top,
            ...
            if (probeOk && sessionKickReason === null) {
                const kickReason = (0, screen_state_js_1.describeSessionKickText)(probeRawText);
                if (kickReason !== null) {
                ...
                try {
                    const ocrSummary = !probeOk
                        ...
                        : probeCards.length > 0
                            ...
                            : '(no cards)';
                    const annotated = await (0, image_js_1.annotateScreenshot)(fullScreenshot, {
                    ...
                    await (0, screenshotter_js_1.saveScreenshot)(annotated, './data/screenshots', `pipelined_iter_${String(batch).padStart(3, '0')}`, { force: true });
                    await (0, screenshotter_js_1.saveScreenshot)(batchCrop, './data/screenshots', `pipelined_crop_${String(batch).padStart(3, '0')}`, { force: true });
                }
                catch (err) {
            ...
            if (probeOk && probeCards.length === 0) {
                ...
                await (0, input_js_1.mouseMove)(page, viewportClickX, viewportClickY);
                for (let c = 0; c < CLICKS_PER_BATCH; c++) {
                    await (0, input_js_1.mouseDown)(page);
                    await (0, input_js_1.mouseUp)(page);
            ...
            tabResult.errors++;
            log.warn(`pipelined: capture iteration ${batch} threw — exiting capture phase with ${crops.length} crop(s) ` +
        ...
        if (batchMs > SLOW_BATCH_MS) {
            ...
            slowWarner.warn('slow-capture-batch', `pipelined: capture batch ${batch} took ${Math.round(batchMs / 1000)}s — healthy is ~4s ` +
                ...
                `[${(0, memory_snapshot_js_1.memorySnapshot)().summary}]`);
        }
        else {
    ...
    slowWarner.flush();
    log.info(`pipelined: capture phase done. ${crops.length} batch crops collected (${totalClicks} clicks), exit=${captureExitReason}`);
    ...
    const newMembersSamples = [];
    let unknownChestNames = 0;
    const unknownChestSamples = [];
    ...
    const newTriumphalNamesSeen = new Set();
    const newTriumphalSamples = [];
    ...
    const cardIdentity = (card) => {
        const player = (0, merge_repo_js_1.applyMergeRulesCached)(playerMergeCache, resolvePlayerName(card));
        const chest = target === 'triumphal'
            ? (0, chest_names_js_1.correctTriumphalChestName)(card.chestName, triumphalKnownNames.length ? triumphalKnownNames : undefined) ?? ''
            : (0, merge_repo_js_1.applyMergeRulesCached)(chestMergeCache, (0, chest_names_js_1.correctChestName)(card.chestName));
        const source = (0, merge_repo_js_1.applyMergeRulesCached)(sourceMergeCache, (0, source_names_js_1.cleanSource)(card.source));
    ...
    const seenCardsOf = (cards, cropMs) => cards.map((card) => {
        ...
        const earnedAt = (0, gift_time_js_1.giftEarnedAtMs)(card.timeLeft, cropMs, -1);
    ...
    for (let i = 0; i < crops.length; i++) {
        ...
        try {
            ...
            const repeated = prevCrop ? (0, crop_overlap_js_1.repeatedLeadingCards)(prevCrop, seenCards, cropCapturedMs) : 0;
            prevCrop = { cards: seenCards, cropMs: cropCapturedMs };
            if (repeated > 0) {
                rereadCards += repeated;
                cards = cards.slice(repeated);
                log.debug(`pipelined: crop ${i + 1}/${crops.length} — dropped ${repeated} card(s) already recorded from the previous crop`);
                if (cards.length === 0)
                    continue;
            }
            let savedCropPathForCrop = null;
             ...
             */
            const ensureCropSaved = async (reason) => {
                if (cropSaveAttempted)
                    return;
                cropSaveAttempted = true;
                try {
                    savedCropPathForCrop = await (0, screenshotter_js_1.saveScreenshot)(crops[i], crop_dirs_js_1.MISSING_NAME_CROP_DIR, `${reason}_s${sessionId}_crop${String(i + 1).padStart(3, '0')}`, { force: true });
                }
                catch (err) {
            ...
            for (const card of cards) {
                ...
                const cleanedSource = (0, merge_repo_js_1.applyMergeRulesCached)(sourceMergeCache, (0, source_names_js_1.cleanSource)(card.source));
                let correctedPlayer = (0, merge_repo_js_1.applyMergeRulesCached)(playerMergeCache, matchedPlayer);
                ...
                let finalType;
                if (target === 'triumphal') {
                    const triumphal = (0, chest_names_js_1.correctTriumphalChestName)(card.chestName, triumphalKnownNames.length ? triumphalKnownNames : undefined);
                    if (!triumphal) {
                    ...
                    finalType = (0, chest_names_js_1.getChestRarity)(triumphal);
                    ...
                    const cleanedChest = (0, chest_names_js_1.correctChestName)(card.chestName);
                    correctedChest = (0, merge_repo_js_1.applyMergeRulesCached)(chestMergeCache, cleanedChest);
                ...
                if (!correctedPlayer || isInactiveLabel) {
                    ...
                    const message = `pipelined: unreadable player name in session ${sessionId}, crop ${i + 1}/${crops.length} ` +
                        ...
                        `— inserting as "${exports.UNKNOWN_PLAYER_NAME}" for manual review`;
                    if (isInactiveLabel) {
                        log.warn(message);
                    }
                    else {
                    ...
                    rowNeedsCrop = true;
                    correctedPlayer = exports.UNKNOWN_PLAYER_NAME;
                }
                ...
                if (!knownMembers.includes(correctedPlayer)) {
                    ...
                    if (correctedPlayer !== exports.UNKNOWN_PLAYER_NAME) {
                        newMembersFromOcr++;
                ...
                if (target !== 'triumphal'
                    && !(0, chest_names_js_1.isKnownChestName)(correctedChest)
                ...
                const pointValue = target === 'triumphal'
                    ? 0
                    : (0, source_points_repo_js_1.getPointsForSourceCached)(sourcePointOverrides, cleanedSource, correctedChest);
            ...
            tabResult.errors++;
            log.warn(`pipelined: OCR phase crop ${i + 1}/${crops.length} threw outside the OCR call — ` +
    ...
    log.info(`pipelined: OCR phase done. ${tabResult.newChests} chests inserted, ${reOcrCount} re-OCRs, ${ocrFailures} OCR failures`);
    ...
    if (rereadCards > 0) {
        ...
        log.info(`pipelined: dropped ${rereadCards} card(s) (${pct}%) that the next screenshot re-read from the previous one — ` +
    ...
    if (newMembersFromOcr > 0 || unknownChestNames > 0 || newTriumphalNamesSeen.size > 0) {
        ...
        if (newMembersFromOcr > 0) {
            log.warn({ noAlert: true }, `pipelined: ${newMembersFromOcr} new member(s) introduced from OCR during this scan into ${clanLabel} — review for legit joiners vs OCR ghosts. Samples: ${newMembersSamples.join(', ')}`);
        }
        if (unknownChestNames > 0) {
            log.warn({ noAlert: true }, `pipelined: ${unknownChestNames} new unrecognised chest name(s) seen for the first time in ${clanLabel} — review for new chest types vs OCR garbage. Samples: ${unknownChestSamples.join(', ')}`);
        }
        if (newTriumphalNamesSeen.size > 0) {
            log.warn({ noAlert: true }, `pipelined: ${newTriumphalNamesSeen.size} new triumphal (Bank) chest(s) seen in ${clanLabel} — stored but scoring 0 until a superadmin assigns a package value on the Triumphal Chest Points page. Samples: ${newTriumphalSamples.join(', ')}`);
        }
    }
    const skippedInserts = tabResult.chestsFound - tabResult.newChests;
    if (skippedInserts > 0) {
        log.warn(`pipelined: ${skippedInserts} insert(s) were skipped as duplicates (chestsFound=${tabResult.chestsFound}, newChests=${tabResult.newChests}). Per-collision details at debug level in chest-repo.`);
    }
    ...
    if (sessionKickReason !== null) {
        throw new navigator_js_1.SessionKickedError(`Session kicked mid-sweep: ${sessionKickReason}`);
    }
    return tabResult;
}
//# sourceMappingURL=scan-pipeline.js.map