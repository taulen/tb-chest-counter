...
exports.MultiClanExternalLoop = exports.ExternalLoop = exports.MAX_BACKFILL_WEEKS = exports.DEFAULTS = exports.CONFIG_KEYS = void 0;
exports.readClanSettings = readClanSettings;
exports.readSettings = readSettings;
exports.writeSettings = writeSettings;
const logger_js_1 = require("../utils/logger.js");
const ingest_js_1 = require("../external/ingest.js");
const chesttracker_client_js_1 = require("../external/chesttracker-client.js");
...
const log = (0, logger_js_1.childLogger)('external-loop');
/**
...
exports.MAX_BACKFILL_WEEKS = 52;
/** Per-share-code "initial backfill done" marker. Keyed by share_code in
 *  ct_config so historical values survive a clan rename. */
function backfillDoneKey(shareCode) {
    return `${exports.CONFIG_KEYS.INITIAL_BACKFILL_DONE}:${shareCode}`;
}
/**
 * Effective settings for one clan, derived from clans table + per-share
 * ct_config marker.
 */
function readClanSettings(clan) {
    const shareCode = clan.ctShareCode || null;
    const initialBackfillDone = shareCode
        ? (0, external_repo_js_1.getConfigValue)(backfillDoneKey(shareCode)) === 'true'
        : true;
    return {
        enabled: !!shareCode,
        shareCode,
        pollIntervalHours: clan.ctPollIntervalHours ?? exports.DEFAULTS.POLL_INTERVAL_HOURS,
        backfillWeeks: clan.ctBackfillWeeks ?? exports.DEFAULTS.BACKFILL_WEEKS,
        initialBackfillDone,
    };
}
/**
...
function readSettings() {
    const clan = (0, clan_repo_js_1.getClanById)(1);
    if (clan)
        return readClanSettings(clan);
    return {
        enabled: false,
        shareCode: null,
        pollIntervalHours: exports.DEFAULTS.POLL_INTERVAL_HOURS,
        backfillWeeks: exports.DEFAULTS.BACKFILL_WEEKS,
...
function writeSettings(patch) {
    if (patch.enabled !== undefined) {
        (0, external_repo_js_1.setConfigValue)(exports.CONFIG_KEYS.ENABLED, patch.enabled ? 'true' : 'false');
    }
    if (patch.shareCode !== undefined) {
        (0, external_repo_js_1.setConfigValue)(exports.CONFIG_KEYS.SHARE_CODE, patch.shareCode ?? '');
    }
    if (patch.pollIntervalHours !== undefined) {
        (0, external_repo_js_1.setConfigValue)(exports.CONFIG_KEYS.POLL_INTERVAL_HOURS, String(patch.pollIntervalHours));
    }
    if (patch.backfillWeeks !== undefined) {
        (0, external_repo_js_1.setConfigValue)(exports.CONFIG_KEYS.BACKFILL_WEEKS, String(patch.backfillWeeks));
    }
    if (patch.initialBackfillDone !== undefined && patch.shareCode) {
        (0, external_repo_js_1.setConfigValue)(backfillDoneKey(patch.shareCode), patch.initialBackfillDone ? 'true' : 'false');
    }
    else if (patch.initialBackfillDone !== undefined) {
        (0, external_repo_js_1.setConfigValue)(exports.CONFIG_KEYS.INITIAL_BACKFILL_DONE, patch.initialBackfillDone ? 'true' : 'false');
    }
}
/**
...
class ExternalLoop {
    ...
    backfill = { running: false, done: 0, total: 0, inserted: 0, empty: 0, failed: 0 };
    constructor(clanId = 1) {
        this.clanId = clanId;
    }
    getClanId() {
        return this.clanId;
    ...
    currentSettings() {
        const clan = (0, clan_repo_js_1.getClanById)(this.clanId);
        if (clan)
            return readClanSettings(clan);
        return readSettings();
    }
    start() {
        ...
        this.running = true;
        log.debug(`External loop started (clan=${this.clanId}, share=${settings.shareCode}, interval=${settings.pollIntervalHours}h)`);
        if (!settings.initialBackfillDone) {
            ...
            const latest = settings.shareCode
                ? (0, external_repo_js_1.getLatestSnapshot)({ clanId: this.clanId, shareCode: settings.shareCode })
                : null;
            if (latest?.fetchedAt) {
                const elapsedMs = Date.now() - new Date(latest.fetchedAt).getTime();
                if (elapsedMs >= 0 && elapsedMs < intervalMs) {
                    const remainingMs = intervalMs - elapsedMs;
                    log.info(`Clan #${this.clanId}: last fetch ${Math.round(elapsedMs / 60_000)} min ago — deferring next fetch by ${Math.round(remainingMs / 60_000)} min`);
    ...
    startBackfill(weeks) {
        ...
        const total = Math.max(1, Math.min(Math.floor(weeks) || 0, exports.MAX_BACKFILL_WEEKS));
    ...
    async runBackfill(shareCode, weeks) {
        const rolloverHour = (0, index_js_1.loadConfig)().gameDayRolloverUtcHour;
        ...
        try {
            ...
            for (let w = 1; w <= weeks; w++) {
                ...
                const window = (0, chesttracker_client_js_1.computeWeekWindowContaining)(anchorMs, rolloverHour);
                try {
            ...
            log.info(`Clan #${this.clanId}: on-demand backfill finished — ${done} week(s) processed, ${inserted} with data, ${empty} empty, ${failed} failed`);
        }
        finally {
            this.backfill.running = false;
            this.inFlight = false;
    ...
    async runInitialBackfill(settings) {
        if (this.inFlight)
            return;
        this.inFlight = true;
        if (!settings.shareCode) {
        ...
        try {
            ...
            for (let w = 1; w <= weeks; w++) {
                ...
                const window = (0, chesttracker_client_js_1.computeWeekWindowContaining)(anchorMs, rolloverHour);
                try {
                    await (0, ingest_js_1.ingestSnapshot)({
                        clanId: this.clanId,
...
exports.ExternalLoop = ExternalLoop;
/**
...
class MultiClanExternalLoop {
    ...
    startBackfillForClan(clanId, weeks) {
...
exports.MultiClanExternalLoop = MultiClanExternalLoop;
//# sourceMappingURL=external-loop.js.map