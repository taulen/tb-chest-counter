...
exports.runMemberCapturePhase = runMemberCapturePhase;
 ...
 */
async function captureMightWhileMembersOpen(ctx, page) {
    try {
        const { runMightCapturePhase } = await import('./might-capture-phase.js');
        const outcome = await runMightCapturePhase({
            config: ctx.config,
            clanId: ctx.clanId,
            reportProgress: (message) => ctx.reportProgress('member-capture', message),
        }, page);
        if (outcome.ran) {
            log.info(`Captured might and hero levels for clan #${ctx.clanId} in the same pass as the roster.`);
        }
        else {
            log.info(`Might capture skipped after the roster build for clan #${ctx.clanId} (${outcome.skipped ?? 'no reason given'}).`);
        }
    }
    catch (err) {
        log.warn(`Might capture after the roster build failed for clan #${ctx.clanId}: `
 ...
 */
async function runMemberCapturePhase(ctx, page) {
    ...
    await (0, input_js_1.mouseClick)(page, clanCoords.x, clanCoords.y);
    await new Promise((r) => setTimeout(r, 2000));
    await captureClanMembers(page, ctx.clanId, (message) => ctx.reportProgress('member-capture', message));
...
//# sourceMappingURL=member-capture-phase.js.map