...
exports.exactMatchMember = exactMatchMember;
exports.fuzzyMatchMember = fuzzyMatchMember;
...
function prepareForMatch(raw) {
    return (0, ocr_normalize_js_1.foldDiacritics)(raw.trim().toLowerCase().replace(/\s+/g, ' '));
}
/**
...
function prepareKey(raw) {
    return (0, ocr_normalize_js_1.despace)(raw);
}
/** Same key minus the case fold, for the case-divergence check below. */
function prepareKeyKeepingCase(raw) {
    return (0, ocr_normalize_js_1.despaceKeepingCase)(raw);
}
/**
 ...
 */
function exactMatchMember(rawName, members) {
    const normalized = prepareForMatch(rawName);
    if (!normalized)
        return null;
    for (const m of members) {
        if (prepareForMatch(m.normalizedName) === normalized)
            return m;
        if (m.aliases.some((a) => prepareForMatch(a) === normalized))
            return m;
    }
    ...
    if (!key)
        return null;
    for (const m of members) {
        if (prepareKey(m.normalizedName) === key)
            return m;
        if (m.aliases.some((a) => prepareKey(a) === key))
            return m;
    }
    return null;
}
 ...
 */
function fuzzyMatchMember(rawName, members) {
    const normalized = prepareForMatch(rawName);
    if (!normalized)
        return null;
    ...
    for (const m of members) {
        ...
        if (casedDist - dist > CASE_DIVERGENCE_ALLOWANCE)
            continue;
        bestDist = dist;
        best = m;
    }
